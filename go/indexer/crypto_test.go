...
func TestDerivePrivateKey(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			privKey, err := indexer.DerivePrivateKey(test.mnemonic, test.hdPath)
			require.Equal(t, err, test.expErr)
			if test.expErr != nil {
				return
			}
...
func TestParsePrivateKeyStr(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			privKey, err := indexer.ParsePrivateKeyStr(test.privKeyStr)
			require.Equal(t, err, test.expErr)
			if test.expErr != nil {
				return
			}
...
func TestGetConfiguredPrivateKey(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			hdPath:     validHDPath,
			privKeyStr: validPrivKeyStr,
			expErr:     indexer.ErrCannotGetPrivateKey,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			privKey, err := indexer.GetConfiguredPrivateKey(
