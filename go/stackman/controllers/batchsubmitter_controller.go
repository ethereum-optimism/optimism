...
func (r *BatchSubmitterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	}, ObjectNamespacedName(crd.ObjectMeta, "batch-submitter"), deployment)
	if err != nil {
		return ctrl.Result{}, err
	}
	if created {
	...
	if deployment.Labels["args_hash"] != argsHash {
		...
		if err != nil {
			lgr.Error(err, "error updating batch submitter deployment")
			return ctrl.Result{}, err
		}
...
func (r *BatchSubmitterReconciler) deploymentArgsHash(crd *stackv1.BatchSubmitter) string {
	h := md5.New()
...
func (r *BatchSubmitterReconciler) deployment(crd *stackv1.BatchSubmitter) *appsv1.Deployment {
	replicas := int32(1)
