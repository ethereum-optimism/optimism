...
func (r *ActorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	created, err := GetOrCreateResource(ctx, r, func() client.Object {
		return r.pod(crd)
	}, ObjectNamespacedName(crd.ObjectMeta, "actor"), pod)
	if err != nil {
		return ctrl.Result{}, err
	}
	if created {
	...
	if pod.Labels["args_hash"] != argsHash {
		err := r.Update(ctx, r.pod(crd))
		if err != nil {
			lgr.Error(err, "error updating actor pod")
			return ctrl.Result{}, err
		}
...
func (r *ActorReconciler) pod(crd *stackv1.Actor) *corev1.Pod {
	command := []string{
		"yarn",
		"test:actor",
		"-f",
	...
	ctrl.SetControllerReference(crd, pod, r.Scheme)
	return pod
}
