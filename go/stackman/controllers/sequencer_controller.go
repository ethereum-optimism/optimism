...
func (r *SequencerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	}, ObjectNamespacedName(crd.ObjectMeta, "sequencer"), deployment)
	if err != nil {
		return ctrl.Result{}, err
	}
	if created {
	...
	if deployment.Labels["args_hash"] != argsHash {
		...
		if err != nil {
			lgr.Error(err, "error updating sequencer deployment")
			return ctrl.Result{}, err
		}
...
func (r *SequencerReconciler) deployment(crd *stackv1.Sequencer) *appsv1.Deployment {
	replicas := int32(1)
