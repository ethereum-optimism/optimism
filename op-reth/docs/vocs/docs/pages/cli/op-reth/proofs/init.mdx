...
```
```txt
Usage: op-reth proofs init [OPTIONS] --proofs-history.storage-path <PROOFS_HISTORY_STORAGE_PATH>
...
Static Files:
      ...
      --proofs-history.storage-path <PROOFS_HISTORY_STORAGE_PATH>
          ...
          This should match the path used when starting the node with `--proofs-history.storage-path`.
