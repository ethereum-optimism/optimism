...
func testImplementations(t *testing.T, forkRPCURL string) {
	...
	deployment1 := deploy()
	require.NotEqual(t, common.Address{}, deployment1.Address("opcmV2"), "OpcmV2 address should be set")
	t.Log("Deploying second implementation contracts bundle")
	deployment2 := deploy()
	require.NotEqual(t, common.Address{}, deployment2.Address("opcmV2"), "OpcmV2 address should be set")
