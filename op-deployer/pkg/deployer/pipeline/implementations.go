...
func DeployImplementations(env *Env, intent *state.Intent, st *state.State) error {
	...
	// Check if ForgeClient is available
	if env.ForgeClient == nil {
		return fmt.Errorf("forge client is required for deploying implementations")
	}

	// Use Forge-based deployment
	lgr.Info("deploying implementations with Forge")
