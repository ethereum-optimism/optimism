...
func WithInteropGen(l1ID stack.L1NetworkID, superchainID stack.SuperchainID,
	...
	return func(setup *stack.Setup) {
		...
		sysL1Net := stack.NewL1Network(stack.L1NetworkConfig{
		...
		sysSuperchain := stack.NewSuperchain(stack.SuperchainConfig{
			CommonConfig: setup.CommonConfig(),
			ID:           superchainID,
		...
		sysCluster := stack.NewCluster(stack.ClusterConfig{
			CommonConfig:  setup.CommonConfig(),
			ID:            clusterID,
		...
		for _, l2ID := range l2IDs {
			...
			sysL2Net := stack.NewL2Network(stack.L2NetworkConfig{
				NetworkConfig: stack.NetworkConfig{
					CommonConfig: setup.CommonConfig(),
