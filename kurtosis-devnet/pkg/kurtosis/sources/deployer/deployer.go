...
type Deployer struct {
	...
	l1ValidatorMnemonicsName string
...
func WithMnemonicsName(name string) DeployerOption {
	return func(d *Deployer) {
		d.l1ValidatorMnemonicsName = name
	}
}
...
func NewDeployer(enclave string, opts ...DeployerOption) *Deployer {
	d := &Deployer{
		...
		l1ValidatorMnemonicsName: defaultMnemonicsName,
...
func (d *Deployer) ExtractData(ctx context.Context) (*DeployerData, error) {
	...
	for id, l1Wallets := range l1WalletsForL2Admin {
	...
	// retrieve L1 genesis file
	genesisArtifact, err := fs.GetArtifact(ctx, d.genesisArtifactName)
	if err != nil {
		return nil, err
	}

	l1ValidatorWallets, err := d.getL1ValidatorWallets(genesisArtifact)
	if err != nil {
		return nil, err
	}

	l1ChainID, err := d.getL1ChainID(genesisArtifact)
	if err != nil {
		return nil, err
	}
