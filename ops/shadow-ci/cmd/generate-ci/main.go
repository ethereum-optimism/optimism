...
func renderShadowCIYAML(cfg *model.Config) ([]byte, error) {
	...
	// Collect unique workspace_paths from all build-group categories.
	// These are the paths that need to be staged for downstream groups.
	buildPathSet := make(map[string]bool)
	for _, cat := range cfg.Scoping.JobCategories {
		if cat.Group == "build" {
			for _, wp := range cat.WorkspacePaths {
				buildPathSet[wp] = true
			}
		}
	}
	var buildWorkspacePaths []string
	for p := range buildPathSet {
		buildWorkspacePaths = append(buildWorkspacePaths, p)
	}
	sort.Strings(buildWorkspacePaths)

	// Collect sorted build category names for per-category caching.
	buildCategoryNames := make([]string, 0)
	for _, cat := range groupCats["build"] {
		buildCategoryNames = append(buildCategoryNames, cat)
	}
	sort.Strings(buildCategoryNames)

	data := templateData{
		Groups:              activeGroups,
		GroupDeps:           groupDeps,
		BuildWorkspacePaths: buildWorkspacePaths,
		BuildCategories:     buildCategoryNames,
	}
...
type templateData struct {
	...
	BuildWorkspacePaths []string // derived from workspace_paths of build-group categories
	BuildCategories     []string // sorted names of build-group categories (for per-category caching)
}
      ...
      - run:
          ...
          command: |
            ...
            if [ -d "$HOME/.local/share/mise/installs" ] && [ -n "$(ls -A $HOME/.local/share/mise/installs 2>/dev/null)" ]; then
              echo "Tools restored from cache — reshimming"
              mise reshim
              exit 0
            fi
  ...
  shadow-ci-{{ .Name }}:
    ...
    steps:
      ...
      - restore_cache:
          keys:
            - shadow-ci-mise-v1-{{ "{{" }} checksum "mise.toml" {{ "}}" }}
{{ range $.BuildCategories }}      - restore_cache:
          keys:
            - shadow-ci-cat-{{ . }}-v1-
{{ end }}      - run:
      ...
      - run:
          ...
          command: |
            /tmp/shadow-ci-workspace/bin/execute \
              ...
              --cache-dir /tmp/shadow-ci-cache
{{ if eq .Name "build" }}{{ range $.BuildCategories }}      - save_cache:
          when: always
          key: shadow-ci-cat-{{ . }}-v1-{{ "{{" }} epoch {{ "}}" }}
          paths:
            - /tmp/shadow-ci-cache/{{ . }}
{{ end }}{{ end }}      - store_test_results:
          path: /tmp/shadow-ci-test-results
      - store_artifacts:
          path: /tmp/shadow-ci-test-results
