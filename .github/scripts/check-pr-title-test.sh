...
expect pass 'op-node!: remove legacy sync mode'
expect pass 'op-node,op-batcher: share event loop metrics'
expect pass 'op-node,op-batcher!: require authenticated control-plane RPC'
...
expect fail 'Fix: capitalized conventional type'
expect fail 'op-node fix things without separator'
expect fail 'op-node:missing space after colon'
expect pass 'OPContractsManager: resolve each proxy admin'
expect pass 'Op-Node: mixed case scope'
expect fail '!op-node: misplaced breaking marker'
expect fail 'op-node!,op-batcher: misplaced breaking marker'
expect fail 'op-node!!: duplicate breaking marker'
