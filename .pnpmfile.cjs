  ...
  for (const [pkg, entry] of Object.entries(pkgs)) {
    ...
    if (repo !== undefined) {
      if (repo.startsWith('git@github.com')) {
        throw new Error(
          `Invalid git ssh specification found for package ${pkg}. Ensure sure that the dependencies do not reference SSH-based git repos before running installing them`
        )
      }
    }
  }
