...
```bash
L1_URL=whatever L2_URL=whatever pnpm test:integration:live
```
...
already. Then, run `pnpm test:actor <args>` to run the tests. Note that it will be **very expensive** to run the actor
