...
describe('Fee Payment Integration Tests', async () => {
  ...
  it('should use the correctly estimated intrinsic gas for transactions of varying lengths', async () => {
    ...
    const Factory__TestLib_IntrinsicGas: ContractFactory = getContractFactory('TestLib_IntrinsicGas', wallet, true)
    ...
    const Factory__SetStorageAndDeployCode = await ethers.getContractFactory('SetStorageAndDeployCode', wallet)
    ...
    expect(
      await wallet.provider.getCode(mockProxyEOA.address)
    ).to.deep.eq(
      proxyEOACode
    )
    ...
    for (const dataSize of [10, 100, 1000, 10000, 25000, 50000, 75000, 100000, 127000]) {
      ...
      const estimatedIntrinsicGas = await TestLib_IntrinsicGas.ecdsaContractAccount(dataSize, { gasLimit: 10_000_000 })
