...
describe('Native ETH value integration tests', () => {
  ...
  describe(`calls between OVM contracts with native ETH value and relevant opcodes`, async () => {
    ...
    describe('WETH functionality', async () => {
      ...
      it('deposit', async () => {
        const initialBalance = await wallet.provider.getBalance(wallet.address)
        ...
        expect(firstTransferEvent.args.from).to.equal(wallet.address)
        ...
        expect(secondTransferEvent.args.to).to.equal(wallet.address)
        expect(secondTransferEvent.args.value).to.equal(value)

        expect(depositEvent.event).to.equal('Deposit')
        expect(depositEvent.args.dst).to.equal(wallet.address)
      ...
      it('withdraw', async () => {
        ...
        expect(depositEvent.args.src).to.equal(wallet.address)
        expect(depositEvent.args.wad).to.equal(value)
      })
    })
  })
