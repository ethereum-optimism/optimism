...
- **Lokahi** — Rust rewrite of the Go `op-supernode` multi-chain consensus-layer host (`rust/lokahi/`, binary `lokahi`). Early development: it hosts N kona chains in one process, sequences the chains configured to sequence, and runs the cross-chain interop verifier over them; the supernode RPC surface is not implemented yet.
