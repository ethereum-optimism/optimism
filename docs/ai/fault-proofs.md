...
the Optimism monorepo: the Cannon MIPS VM, op-program, and the dispute game contracts.
See [go-dev.md](go-dev.md) for Go workflow and [contract-dev.md](contract-dev.md) for
the Solidity dispute game contracts.

## Scope

- `cannon/` — MIPS32 VM that executes the op-program in single-step mode.
- `op-program/` — standalone Go binary that derives and executes L2 state.
- `packages/contracts-bedrock/src/dispute/` — on-chain dispute game contracts.

## Key Concepts

- **Cannon**: a MIPS32 VM that executes the op-program in single-step mode, producing a
  reproducible execution trace.
- **op-program**: a standalone Go binary that derives and executes L2 state.
...
- **op-program determinism**: the same inputs always produce the same output root. The
  op-program must be fully deterministic — no network calls, no filesystem access at runtime.
