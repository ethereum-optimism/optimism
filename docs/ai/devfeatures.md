...
2. **Intent file (`globalDeployOverrides.devFeatureBitmap`)**
   ...
   - The circuit that verifier implements is pinned in `op-deployer/pkg/deployer/standard/sp1-verifier.json` (`VERIFIER_HASH()`). `kona-sp1-proposer`'s release-pin test holds it to the linked sp1-sdk; `TestApplyDefaultsSP1VerifierOnSepolia` holds it to the address on chain; `VerifyOPCM` re-checks it at release. Bump sp1-sdk to a new circuit and the address and JSON must move together.
