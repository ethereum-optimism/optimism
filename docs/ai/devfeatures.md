...
2. **Intent file (`globalDeployOverrides.devFeatureBitmap`)**
   ...
   - `standard.SP1VerifierHashFor` records the `VERIFIER_HASH()` of that verifier, the PLONK circuit it accepts proofs for. `TestApplyDefaultsSP1VerifierOnSepolia` holds it to the chain and `VerifyOPCM` re-checks it at release. `kona-sp1-proposer` derives the hash it needs from the sp1-sdk it links (no copy of the value in Rust) and pauses creation and skips proving for any adapter whose verifier hash differs from it. Bumping sp1-sdk to a new circuit means moving the address and the hash together; until then the proposer reports the mismatch at runtime.
