---
title: Running a Node With Docker (simple-optimism-node)
description: Run an OP Stack node via the simple-optimism-node Docker wrapper. Retained for reference; new deployments should use the direct docker-compose tutorial.
---
...
<Info>
  By default, `simple-optimism-node` uses `op-geth` as the execution client. Support for `Nethermind` as an alternative execution client is also available. **op-reth is not currently supported upstream.**
</Info>
