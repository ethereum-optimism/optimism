...
Now that you have [installed the `kona-node`](/node-operators/kona-node/install/overview),
it's time to run it.
...
-   **[Configuration](/node-operators/kona-node/configuration)** - Configure your node
-   **[Monitoring](/node-operators/kona-node/monitoring)** - Set up logs, metrics, and observability
