...
Visit the [Prerequisites](/node-operators/kona-node/install/overview) for details on installing Rust and `just`.
</Note>
...
binary](https://github.com/ethereum-optimism/optimism/releases).
...
- Read the [design docs](/node-operators/kona-node/design/intro) to understand kona-node's architecture
- Check out the [Binaries](/node-operators/kona-node/run/binary) documentation
- Explore the [examples](https://github.com/ethereum-optimism/optimism/tree/develop/rust/kona/examples)
