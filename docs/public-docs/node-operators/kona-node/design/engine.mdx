...
The `SealedPayload` sent over `result_tx` carries the execution payload, the sealed block as the
engine decoded it while importing, and how long sealing took — fetching and importing the payload,
excluding any wait for it to become ready. That wait is what `ready_deadline` bounds: with a
deadline set, the task first asks the execution layer whether the payload is worth sealing
(`engine_awaitPayloadReadyV1`) and only fetches it once the answer arrives or the deadline passes.
The sequencer sets the deadline on chains that may build several blocks per block time, so a block
is sealed as soon as it is worth sealing rather than at a fixed cadence; without one, the task
fetches the payload immediately.
