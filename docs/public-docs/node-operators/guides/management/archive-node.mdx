...
op-reth rejects `--minimal` at startup: it prunes bodies to a fixed 10,064-block window, which is smaller than the default offset and cannot be widened independently.
