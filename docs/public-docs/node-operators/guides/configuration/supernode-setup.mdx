...
*   **L1 endpoints.** An L1 JSON-RPC endpoint with the `eth` namespace enabled and an L1 beacon-node HTTP endpoint, shared by every chain. A beacon archiver fallback is strongly recommended — see [configure a blob archiver](/node-operators/guides/management/blobs#configure-a-blob-archiver).
