...
<Tabs>
  <Tab title="op-node">
    ...
    * `geth` — for op-geth (*deprecated*, see the [op-geth deprecation notice](/notices/archive/op-geth-deprecation)).
  ...
  <Tab title="kona-node">
    ...
    - [kona-node component hub](/op-stack/components/kona-node)
    - [kona-node CLI reference](/node-operators/kona-node/configuration)
    - [Run a node with kona-node](/node-operators/kona-node/run/overview)
  </Tab>
</Tabs>
