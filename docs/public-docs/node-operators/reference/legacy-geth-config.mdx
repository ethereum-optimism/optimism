...
A filter that can only match post-Bedrock blocks is served locally; this includes any filter whose `fromBlock` follows the chain tip (`latest`, `safe`, `finalized`, `pending`), because the tip is post-Bedrock.

A filter that starts before Bedrock and ends at or after it — including one with no `toBlock`, or a `toBlock` that follows the chain tip, which is where an "all logs for this contract" query lands — is served from both:
...
Forwarded filters are still subject to `--rpc.max-blocks-per-filter`: it is applied to the range op-reth sends to Legacy Geth, so a fully pre-Bedrock range wider than the limit is not forwarded at all, and a split filter whose pre-Bedrock half is wider than the limit is not split.
In both cases op-reth falls back to local handling, which rejects the query — the range asked for is at least as wide.
A skipped split is logged as a warning on the `rpc::historical` target, with the filter's `from_block`, `to_block` and the `bedrock` block.
