...
A filter whose upper bound follows the chain tip (`latest`, `safe`, `finalized`, `pending`, or no `toBlock` at all) is served locally, because the tip is post-Bedrock.

Forwarded filters are still subject to `--rpc.max-blocks-per-filter`: a pre-Bedrock range wider than that limit is not forwarded, and local handling rejects it.
...
<Warning>
  A range that starts before Bedrock and ends at or after it is served locally, so it returns only the post-Bedrock logs and silently omits the pre-Bedrock ones.
  op-reth logs a warning on the `rpc::historical` target when this happens, with the filter's `from_block`, `to_block` and the `bedrock` block.
  To get the pre-Bedrock half, issue separate queries that end at the last pre-Bedrock block, each spanning at most `--rpc.max-blocks-per-filter` blocks.
</Warning>
