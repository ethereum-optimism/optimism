...
A filter whose upper bound follows the chain tip (`latest`, `safe`, `finalized`, `pending`, or no `toBlock` at all) is served locally.

Forwarded filters are still subject to `--rpc.max-blocks-per-filter`: a pre-Bedrock range wider than that limit is rejected with it rather than forwarded.
...
<Warning>
  ...
  op-reth logs a warning on the `rpc::historical` target when this happens.
  Split such a query into one request ending at the last pre-Bedrock block and one starting at the Bedrock block.
</Warning>
