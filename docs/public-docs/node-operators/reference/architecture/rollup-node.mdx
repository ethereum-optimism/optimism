...
Now, you are ready to build your OP Stack rollup node. You have two options for this:
  ...
  *   **Option 2:** Follow the [Building and running an OP Stack node from source](/node-operators/tutorials/run-node-from-source) tutorial, if you need to use a specific architecture or want to inspect the source code of your OP Stack rollup node.
...
You will now run your node from source for your OP Stack network. Here are your options.
  ...
  *   Follow the [Building and running an OP Stack node from source](/node-operators/tutorials/run-node-from-source) tutorial, which covers both building and running on OP Sepolia or OP Mainnet (full or archive node).
...
| [Building and running an OP Stack node from source](/node-operators/tutorials/run-node-from-source) | Compile and run op-node + op-reth (or Nethermind) from source. | 🟡 Medium        |
| [Running op-reth with Historical Proofs](/node-operators/tutorials/reth-historical-proofs) | Configure op-reth's proofs-history store for permissionless withdrawal proving. | 🟠 Advanced      |
