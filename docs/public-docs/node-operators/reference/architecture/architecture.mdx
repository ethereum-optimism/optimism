...
<Warning>
  **op-geth**, the original execution client for the OP Stack, reached end of support on May 31, 2026, and does not support the now-active Karst hardfork, so op-geth nodes can no longer follow the canonical chain. op-program has also reached end of support, replaced by kona-client for fault proofs. Migrate to op-reth; see the [End of Support for op-geth and op-program notice](/notices/op-geth-deprecation).
</Warning>
