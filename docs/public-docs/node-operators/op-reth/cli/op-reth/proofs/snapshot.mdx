...
```bash
$ op-reth proofs snapshot --help
```
```txt
Usage: op-reth proofs snapshot [OPTIONS] <COMMAND>

Commands:
  init  Build a snapshot at a target block and mark it Ready
  drop  Drop the snapshot tables and meta row
