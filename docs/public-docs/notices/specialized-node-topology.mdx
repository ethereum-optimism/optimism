...
*   **Deriver tier** — a small, redundant set of derivation-enabled `op-node` instances (the sources). These sit behind a `proxyd` configured with consensus awareness, which aggregates the sources into a single highly available endpoint and hides individual failures or reorgs from downstream consumers.
