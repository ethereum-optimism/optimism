...
The `overhead()` getter remains available and its stored value does not change during the upgrade.
Calls to `setGasConfigEcotone(uint32,uint32)` continue to emit a two-word `FEE_SCALARS` payload, but the first word for `overhead` is always zero.
