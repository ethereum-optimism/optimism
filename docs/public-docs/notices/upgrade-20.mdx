...
* For a chain in an existing Interop set, upgrade every member chain and validate both the per-chain and shared contracts.

The [Fault Proof service changes](#prepare-fault-proof-services), [`SystemConfig` integration changes](#update-systemconfig-integrations), and [prestate workflow](#locate-or-build-the-absolute-prestate) are described below.

### Node Operators

Operators who run only `op-node` and an execution client do not need to configure a hardfork activation timestamp.
Upgrade 20 has no L2 hardfork, and it does not require an `op-node` or `op-batcher` configuration change.
...
Node operators who also run `op-proposer`, `op-challenger`, or `op-dispute-mon` must follow the chain operator actions.
...
Bridge or withdrawal tooling that reads Dispute Game contracts directly must follow the app developer guidance above.
Tooling that delegates the withdrawal flow to an SDK must also use a version that supports Super Root Dispute Games.
...
| [`@eth-optimism/viem`](https://github.com/ethereum-optimism/ecosystem/tree/main/packages/viem) and [`@eth-optimism/wagmi`](https://github.com/ethereum-optimism/ecosystem/tree/main/packages/wagmi) | Their current peer ranges accept a compatible `viem` release, and neither package implements the L1 withdrawal prove and finalize path. If an application uses their underlying `viem/op-stack` prove flow, make sure its lockfile resolves `viem` `2.51.0` or later and apply the same call changes. |
| [`@eth-optimism/sdk`](https://www.npmjs.com/package/@eth-optimism/sdk) | The published `3.3.3` release does not support Super Root withdrawal discovery or proof construction. Migrate L2-to-L1 withdrawal status, prove, and finalize workflows to `viem` `2.51.0` or later, or implement the Super Root handling directly. |
...
* For an existing Interop set, include every member chain in the execution plan and prepare post-upgrade checks for the implementation and owner of each per-chain and shared proxy.
