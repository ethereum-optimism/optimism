...
The following Sepolia chains activated Karst with the buggy release and currently have an elevated block gas limit:

**Sepolia:** `OP`, `Ink`, `Soneium`, `Unichain`, `Mode`, `Metal`, `Zora`, `Lisk`

Mainnet chains have `keep_karst_upgrade_gas = true` baked into their Superchain Registry configs, but will activate Karst with the fixed releases — the gas limit elevation will not occur. **No `setGasLimit()` action is required for mainnet chains after activation.**
