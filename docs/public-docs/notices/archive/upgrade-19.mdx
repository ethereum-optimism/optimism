...
<Warning>
  **`op-geth` and `op-program` will not support Upgrade 19 / Karst.** Migrate to **`op-reth`** (and **`kona-client`** for fault proofs) before activation. End of Support is **May 31, 2026** — see [End of Support for op-geth and op-program](/notices/archive/op-geth-deprecation).
</Warning>
...
<Steps>
  ...
  * `op-node`: [`--network <network>`](/node-operators/reference/op-node-config#network-and-rollup-config) (e.g. `--network soneium-mainnet`, `--network op-sepolia`)
  * `op-reth`: [`--chain <network>`](/node-operators/op-reth/cli/op-reth/node) (e.g. `--chain soneium`, `--chain unichain`)
  ...
  * `op-reth`: set `"karstTime": <timestamp>` in the genesis file
  ...
  Make the following checks to verify that your node is properly configured:
    ...
    * In the `op-node` startup logs, check that `keep_karst_upgrade_gas` is set to true if appropriate (see [this section](#gas-limit-action-required-for-affected-chains))
  </Step>
</Steps>
