...
<Info>
  ...
  Payload types, configuration variables, and CLI flags keep their `Flashblock` and `flashblocks` spellings, so those are still the identifiers to grep for.
  The specification is published as the [Flashblocks specification](https://specs.optimism.io/protocol/flashblocks.html).
</Info>
...
The [chain operator guide](/chain-operators/guides/features/flashblocks-guide) documents that stack for the chains still running it.
...
<Expandable title="How often are subblocks produced?">
  Every 200 ms, as a target.
  The interval is configured per chain through `FLASHBLOCKS_TIME`.
