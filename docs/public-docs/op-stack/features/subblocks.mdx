...
*   **`transactions` may be empty.** A subblock can carry only an updated diff and `post_exec_tx`. Treat `"transactions": []` as ordinary, not as an error or as "nothing changed".
