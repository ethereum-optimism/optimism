...
Concatenating `diff.transactions` across a block's subblocks gives you the sealed block's transaction list minus its final `0x7D` transaction.

<Info>
  The [Lagoon post-exec specification](https://specs.optimism.io/protocol/lagoon/post-exec.html#subblocks) is normative for this field and is the reference to build against.
  ...
  The [Subblocks specification](https://specs.optimism.io/protocol/subblocks.html#post-execution-transactions) covers the same field from the stream's side.
</Info>
