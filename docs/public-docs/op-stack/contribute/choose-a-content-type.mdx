...
description: A maintainer-facing decision table for picking the right documentation or operational content type and its frontmatter.
diataxis: reference
---
...
## The taxonomy: four quadrants, four compositions, one operational type
...
prepare. It is not a fifth Diátaxis quadrant and it is not a composition of
the quadrants.

Notices omit `diataxis:` and use the metadata key `content_type: notice`
(with an underscore), which already exists in `keywords.config.yaml`. The
