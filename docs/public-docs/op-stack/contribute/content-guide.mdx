...
A page that explains protocol behavior must open with the `<NormativeSpec>`
...
page does instead of restating the spec. The
[hardfork registry pages](/op-stack/protocol/network-upgrades) carry the same
component with the spec URL from their structured frontmatter.
