...
1.  **Frontmatter**: `title`, `description`, `diataxis: how-to`,
    ...
    guides are curated artifacts and enter the review sweep on merge, per
    the [curation review policy](/op-stack/contribute/curation-policy)).
...
6.  **Verification** (final step): how the reader confirms the goal is
    reached.
7.  **Next steps** (`## Next steps`): the deliberate, curated ways out —
...
*   [Tune batcher costs](/use-cases/tune-batcher-costs): the first solution
    guide and the proof of the form, with derive-links stating explicit
    takeaways, if/choose/because tables, and badged exits.
