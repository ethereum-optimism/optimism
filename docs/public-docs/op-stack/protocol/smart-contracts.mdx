...
*   MIPS contract upgrade: Updates to support additional calls made by the new fault-proof client version (`op-program`, and from the Karst upgrade `kona-client`). The Cannon MIPS VM stays; the client program is what changes.
