...
<Info>
  **Learn the OP Stack — stop 5 of 14.**
  ...
  to [Creating your own L2 rollup testnet](/chain-operators/tutorials/create-l2-rollup).
</Info>
...
runtime, see [OP Stack architecture](/op-stack/architecture); for the hub page
of each individual component, see [Stack Components](/op-stack/components/index).
...
graph TB
  ...
  subgraph stack[OP Stack layers]
    direction TB
    gov[Governance<br/>upgrades and parameters]
    set[Settlement<br/>convinces L1 of L2 state]
    exe[Execution<br/>state and the state transition]
    der[Derivation<br/>compressed data to Engine API inputs]
    seq[Sequencing<br/>collects and publishes transactions]
    da[Data availability<br/>where the posted data lives]
  end

  seq -->|compresses and posts batches| da
  da -->|compressed batches read by| der
  der -->|block inputs via Engine API| exe
  exe -->|state committed by| set
  gov -.->|configures| set

  class stack chain
```

Layers are drawn in the order data moves through them, from where the chain's
data is posted at the bottom to the processes that govern the system at the
top. Sequencing compresses L2 blocks into batches and posts them to the data
...
*   [Specifications](https://specs.optimism.io/protocol/derivation.html?utm_source=op-docs&utm_medium=docs#batch-submission-wire-format)
...
*   [Specifications](https://specs.optimism.io/protocol/derivation.html?utm_source=op-docs&utm_medium=docs#l2-chain-derivation-pipeline)
...
*   [Source code](https://github.com/ethereum-optimism/optimism/tree/develop/rust/op-reth)
...
**This is the Settlement Layer module OP Mainnet uses today**, activated on June 10, 2024. It replaces the attestation mechanism's MultiSig challenger with a permissionless fault proving process: anyone can propose the state of the chain, and anyone can challenge a proposal they believe is wrong. A correctly constructed fault proof invalidates an incorrect proposal within the challenge period, which moves the trust assumption off the honesty of a named set of participants and onto the correctness of the fault proof construction. The Optimism Security Council, acting as the [Guardian](/op-stack/protocol/privileged-roles#guardian), is the backstop if that construction fails.

*   [Specifications](https://specs.optimism.io/fault-proof/index.html?utm_source=op-docs&utm_medium=docs)
*   [Fault proofs explainer](/op-stack/fault-proofs/explainer)
*   [Source code](https://github.com/ethereum-optimism/optimism/tree/develop/packages/contracts-bedrock/src/dispute)
