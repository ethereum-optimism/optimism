...
An OP Stack chain that publishes transaction data to a data availability layer other than Ethereum. The commitment and verification model depends on the integration; see [Alt-DA Mode](/op-stack/features/alt-da-mode).
...
Learn more at the [Light Node Topology Notice Page](/notices/specialized-node-topology).
...
by the [`SystemConfig`](https://github.com/ethereum-optimism/optimism/blob/v1.1.4/packages/contracts-bedrock/src/L1/SystemConfig.sol) (as of `v1.1.4`) contract on L1 and read by the L2 [derivation](#l2-chain-derivation) process.
