...
The default for this system component is `kona-client`, a Rust implementation combining `kona-node` and `op-reth`. Its predecessor `op-program` (a combination of `op-geth` and `op-node`, written in Go) reaches end-of-support at the Karst upgrade.
