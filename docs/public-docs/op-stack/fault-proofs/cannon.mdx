...
<Info>
  An earlier generation of Cannon was 32-bit: it ran the Go fault proof program `op-program` on a MIPS32 instruction set, verified onchain by `MIPS.sol`.
  That generation backs the legacy `CANNON` (game type 0) and `PERMISSIONED_CANNON` (game type 1) dispute games, and `op-program` has since
  ...
  `cannon-kona` (game type 8) configuration: kona-client running on the 64-bit Cannon described on this page.
  The [MIPS.sol reference](/op-stack/fault-proofs/mips) documents the legacy 32-bit onchain contract.
</Info>
...
Before `op-program` reached end-of-support, it filled this same role for the legacy 32-bit Cannon: Go code compiled to MIPS32 instructions
with the same determinism property. Only the kona variants are maintained today.
