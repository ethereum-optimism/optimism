...
*   A game type is only playable on a chain after the `DisputeGameFactory` owner registers an implementation for it (`setImplementation`) and sets its initialization bond (`setInitBond`).
...
| [`PermissionedDisputeGame`](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/PermissionedDisputeGame.sol) | `FaultDisputeGame` variant where only the `PROPOSER` and `CHALLENGER` roles can create games and make moves                                        |
...
| `minProposalSizeBytes`            | 126,000 bytes              | `PreimageOracle`      | Minimum preimage size that must use the large-preimage proposal path      |
...
*   **Distribution mode:** when a game closes it selects a `BondDistributionMode`. A proper game (registered, respected, not blacklisted, not retired) distributes bonds normally — the bond of every successfully countered claim is paid to the countering party, and uncontested claims are refunded. An improper game enters refund mode and returns every bond to its original depositor.
