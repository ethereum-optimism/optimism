...
<Note>
Before reading this section of the book, it is advised to read the [Fault Proof Program Environment](/op-stack/fault-proofs/kona/fpp-dev/env)
...
- [Fault Proof VM](https://specs.optimism.io/fault-proof/index.html#fault-proof-vm) IO and utilities
...
for programs running on top of the [FPVM targets](/op-stack/fault-proofs/kona/fpp-dev/env).
...
when developing programs that target the [FPVMs](/op-stack/fault-proofs/kona/fpp-dev/env), barring needs like printing directly to
`stdout`.
...
[Host - Client Communication](/op-stack/fault-proofs/kona/fpp-dev/env#host-and-client-communication) in the FPP Dev environment section of the
book.
...
the [Custom Backend](/op-stack/fault-proofs/kona/proof/custom-backend) section, but we'll quickly gloss over them here to build intuition.

Let's take, for example, [`OracleL1ChainProvider`](https://github.com/ethereum-optimism/optimism/blob/develop/rust/kona/crates/proof/proof/src/l1/chain_provider.rs).
...
is a composition trait of the `PreimageOracleClient + HintWriterClient` traits outlined above.
...
The hint send and the preimage fetch are both visible here: `HintType::L1BlockHeader.with_data(...).send(...)` writes the hint through the `HintWriterClient`, and `self.oracle.get(...)` reads the prepared preimage through the `PreimageOracleClient`.
