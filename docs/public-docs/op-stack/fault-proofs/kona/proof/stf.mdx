...
- The [FPVM backend](/op-stack/fault-proofs/kona/proof/fpvm-backend), which Kona ships out of the box, runs the STF on the OP Stack's Fault Proof VMs and sources its data over the PreimageOracle ABI.
- [Custom backends](/op-stack/fault-proofs/kona/proof/custom-backend) run the same STF on other verifiable environments, such as zkVMs or TEEs, by swapping out the data source traits.
