...
Before kicking off the development of your own [Fault Proof Program](https://specs.optimism.io/fault-proof/index.html#fault-proof-program),
...
to communicate with one another is synchronously through the [Preimage ABI](https://specs.optimism.io/fault-proof/index.html#pre-image-oracle) ([specification][preimage-specs]).
...
Below, we have a full architecture diagram of [`op-program`](https://github.com/ethereum-optimism/optimism/tree/op-program/v1.9.0/op-program) (as of `op-program/v1.9.0`; source: [fault proof specs][fp-specs]), the original Go implementation of the OP Stack's Fault Proof Program,
which has the objective of verifying claims about the state of an [OP Stack][op-stack] layer two. `op-program` has reached end-of-support and `kona-client` is the maintained Fault Proof Program (see [End of Support for op-geth and op-program](/notices/archive/op-geth-deprecation)), but the architecture shown here applies to both.

![op-program-architecture](/public/rust/op-program-fpp.svg)

In this program, execution and derivation of the L2 chain is performed within it, and ultimately the claimed state of the L2 chain is verified in the [prologue](/op-stack/fault-proofs/kona/fpp-dev/prologue) stage.
