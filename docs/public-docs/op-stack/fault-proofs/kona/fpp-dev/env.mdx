...
which has the objective of verifying claims about the state of an [OP Stack][op-stack] layer two. `op-program` has reached end-of-support and `kona-client` is the maintained Fault Proof Program (see [End of Support for op-geth and op-program](/notices/op-geth-deprecation)), but the architecture shown here applies to both.
