...
[execution phase](/op-stack/fault-proofs/kona/fpp-dev/execution). In most programs, this entails comparing the outputs of the execution phase
to portions of the bootstrap data made available during the [prologue phase](/op-stack/fault-proofs/kona/fpp-dev/prologue).

Generally, this phase should consist almost entirely of validation steps.

## Example

In the `kona-client` program, the epilogue phase only contains two directives:
