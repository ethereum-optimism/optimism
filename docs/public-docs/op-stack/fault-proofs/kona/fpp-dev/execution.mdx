...
This phase consumes the outputs of the [prologue phase](/op-stack/fault-proofs/kona/fpp-dev/prologue), and performs the bulk of the verifiable
computation. After execution has concluded, the outputs are passed along to the [epilogue phase](/op-stack/fault-proofs/kona/fpp-dev/epilogue) for
final verification.
...
1. Derives the inputs to the L2 derivation pipeline by unrolling the L1 head hash fetched in the prologue.
