...
source, pulled in through the [Host-Client communication](/op-stack/fault-proofs/kona/fpp-dev/env#host-and-client-communication) implementation.

As a rule of thumb, the prologue implementation should be kept minimal, and should not do much more than establish
the inputs for the [execution phase](/op-stack/fault-proofs/kona/fpp-dev/execution).
...
1. Pull in the boot information over the [Preimage Oracle ABI][preimage-specs], containing:
   ...
   - The agreed upon safe [L2 output root][l2-output-root].
   - The [L2 output root][l2-output-root] claim.
   - The block number of the [L2 output root][l2-output-root] claim.
   - The L2 chain ID.
1. Pull in the `RollupConfig` and `L1ChainConfig` corresponding to the passed L2 chain ID.
