...
The `kona-std-fpvm` crate provides IO and allocator support for both the `mips64` (cannon) and `riscv64` (asterisc) architectures. The maintained, reproducible prestate build pipeline in the monorepo (the kona `justfile`'s `build-cannon-client` recipe and the fault proof release process) targets cannon only; building for asterisc requires assembling your own build pipeline.
