...
<Info>
  The `op-challenger` docker container runs the `cannon` VM with a fault-proof program as a sub-process to generate game trace data. The respected game type going forward is `cannon-kona`, which uses `kona-host`. The legacy `cannon` game type uses `op-program host` and reaches end-of-support at the Karst upgrade.
</Info>
