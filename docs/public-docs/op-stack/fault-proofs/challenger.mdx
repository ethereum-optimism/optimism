...
<NormativeSpec
  what="The honest challenger's behavior"
  title="honest challenger specification"
  href="https://specs.optimism.io/fault-proof/stage-one/honest-challenger-fdg.html"
  note="This page explains how op-challenger implements that role; it does not restate the spec."
/>
...
*   For releases, configuration reference, and source links, see the [op-challenger hub](/op-stack/components/op-challenger).
...
The calculation for the bond amounts are hard-coded in the [`FaultDisputeGame` contract](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/FaultDisputeGame.sol#L936-L977), and there's a `getRequiredBond` method on the contract that suggests what bond to use.
