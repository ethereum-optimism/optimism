...
<NormativeSpec
  ...
  href="https://specs.optimism.io/protocol/overview.html"
...
An OP Stack chain is an *optimistic rollup*: it executes transactions on its
own network, publishes their data to a parent chain, and lets anyone challenge
its claims about the resulting state rather than proving each one up front.
That makes it a *derived* chain: its canonical history is defined by data
published to a parent chain, and every other part of the system exists to
...
graph TB
  ...
  chal[op-challenger<br/>see game type below] -->|disputes proposals| dgf
...
| [op-deployer](/op-stack/components/op-deployer) | Deploys the chain's L1 contracts | Operator | No contracts on L1, so no chain |
...
The sequencer's mempool is private, unlike L1 Ethereum's public one, which
limits the MEV opportunities a public pending-transaction pool creates.
...
Steps 1 through 5 are run by the chain operator, so they depend on the
sequencer behaving. Step 6 is where that
...
preconfirmation within roughly 200 milliseconds of submission, well before
step 4. The interval is a target rather than a guarantee.
...
*   **Custom gas token.** A [custom gas token](/op-stack/features/custom-gas-token)
    as the native fee asset instead of ETH. Configured at genesis, with no
    migration path: an existing chain would have to coordinate a hard fork.
...
**Named roles** include the L1 and L2 Proxy Admins (upgrade the system
contracts), the
