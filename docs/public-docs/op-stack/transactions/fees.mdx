...
*   The **signed** transaction, serialized according to the standard Ethereum transaction RLP encoding (op-alloy's [`OpTxEnvelope`](https://github.com/ethereum-optimism/optimism/blob/develop/rust/op-alloy/crates/consensus/src/transaction/envelope.rs)).
