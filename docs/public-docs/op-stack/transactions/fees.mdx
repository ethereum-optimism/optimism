...
<Info>
  **Learn the OP Stack — stop 7 of 13.**
  You know how a transaction moves through the stack. This page adds what
  that journey costs: the components of an OP Stack transaction fee and
  what determines each one. When you're done, continue to
  [Deposit flow](/op-stack/bridging/deposit-flow).
</Info>
...
<Info>
OP Mainnet has run a time-boxed experiment in **stake-based priority ordering**, in which stake held in the `PolicyEngineStaking` contract — not priority fee alone — influenced transaction ordering. The experiment was scheduled to begin on May 26, 2026 and to run for a maximum of four weeks, after which OP Mainnet reverts to the priority-fee ordering described above. Check the [stake-based priority ordering notice](/notices/archive/stake-based-priority-ordering) and the [Governance Proposal](https://gov.optimism.io/t/upgrade-proposal-stake-based-priority-ordering-experiment/10655) before assuming either ordering applies.
</Info>
...
*   The **signed** transaction, serialized according to [the standard Ethereum transaction RLP encoding](https://github.com/ethereum-optimism/op-geth/blob/optimism/core/types/transaction.go#L141-L151).
...
For guidance on choosing scalar values, chain operators can follow [Recover the spend with fee scalars](/use-cases/tune-batcher-costs#step-5-recover-the-spend-with-fee-scalars) in the Tune batcher costs guide.
...
The new cost function takes into account the compression ratio, so chain operators will need to adjust their `baseFeeScalar` and `blobFeeScalar`  to account for any previous compression ratios that they encountered on their chains. For guidance on choosing scalar values, chain operators can follow [Recover the spend with fee scalars](/use-cases/tune-batcher-costs#step-5-recover-the-spend-with-fee-scalars) in the Tune batcher costs guide.
