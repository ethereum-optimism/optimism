...
<Info>
  **Learn the OP Stack — stop 6 of 13.**
  You've deployed a chain and started its components. This page replays
  what you just ran as the life of a single transaction, from submission
  to finality on Ethereum. When you're done, continue to
  [Transaction fees on OP Mainnet](/op-stack/transactions/fees).
</Info>
...
*   The transaction needs to be executed to modify the state (by the execution client, `op-reth` by default).
...
[`op-batcher`](https://github.com/ethereum-optimism/optimism/tree/develop/op-batcher) has two main jobs:
...
Writing to L1 is what moves a processed L2 transaction through the protocol's finality states: **unsafe** (processed, but not written to L1 yet), **safe** (written to L1), and **finalized** (the L1 block containing it has been finalized by Ethereum).
For what these states mean and when you can rely on a transaction being irreversible, see [Transaction finality](/op-stack/transactions/transaction-finality).
...
The transactions themselves are sent in [`op-batcher`'s main loop](https://github.com/ethereum-optimism/optimism/blob/develop/op-batcher/batcher/driver.go#L489-L520), which calls [`publishStateToL1`](https://github.com/ethereum-optimism/optimism/blob/develop/op-batcher/batcher/driver.go#L784)
...
1.  Applying the transaction to the old state to produce the new state, which is performed by the execution client (`op-reth` by default).
...
The state is stored and modified by the execution client. The default is [`op-reth`](https://github.com/paradigmxyz/reth); [`op-geth`](https://op-geth.optimism.io/) — a slightly modified version of standard [geth](https://geth.ethereum.org/) — was previously supported but reached [end-of-support](/notices/op-geth-deprecation) on 2026-05-31.

### State root proposals

The state root proposals are posted by [`op-proposer`](https://github.com/ethereum-optimism/optimism/tree/develop/op-proposer) to [`L2OutputOracle`](https://github.com/ethereum-optimism/optimism/blob/233ede59d16cb01bdd8e7ff662a153a4c3178bdd/packages/contracts-bedrock/contracts/L1/L2OutputOracle.sol) (as of `233ede5`) on L1.

Output proposals are *not* immediately valid.
They can only be considered authoritative once the fault challenge period has passed; see [Withdrawal flow](/op-stack/bridging/withdrawal-flow) for how output roots are used to prove and finalize withdrawals.
