...
<Info>
  **Learn the OP Stack — stop 7 of 14.**
...
The state is stored and modified by the execution client. The default is [`op-reth`](https://github.com/paradigmxyz/reth).

<OpGethEol />

### State root proposals

The state root proposals are posted by [`op-proposer`](https://github.com/ethereum-optimism/optimism/tree/develop/op-proposer) to the [`DisputeGameFactory`](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/DisputeGameFactory.sol) on L1, which creates a dispute game holding the proposal.
On chains that predate fault proofs, proposals went to the permissioned `L2OutputOracle` instead; see [OP Stack smart contracts](/op-stack/protocol/smart-contracts) for that history.
