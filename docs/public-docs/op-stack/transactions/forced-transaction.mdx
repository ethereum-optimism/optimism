...
A forced transaction is an ordinary [deposit transaction](/op-stack/bridging/deposit-flow) submitted directly on L1; this page focuses on its censorship-resistance role and inclusion timing during sequencer downtime, while the [Deposit flow](/op-stack/bridging/deposit-flow) explainer covers the underlying mechanics.
...
2.  **Inclusion in L2:**
    ...
    *   **Extended Downtime (30 minutes to 12 hours):**
        ...
        *   Deposit Transactions: Users can submit forced transactions directly to the [`OptimismPortal`](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/L1/OptimismPortal2.sol#L684) contract on L1. However, these transactions will only be included in the L2 chain once the sequencer resumes or the 12-hour sequencing window expires.
