...
| ----------------------------------------------------------------------------------------------- | ----------------------------- | ---------------------------- | --------------------------------------------------------------------- |
| Subblock preconfirmation, on chains that stream [Subblocks](/op-stack/features/subblocks)       | pending state                 | ~200 milliseconds            | The Sequencer                                                          |
...
2.  The Sequencer includes the transaction in an L2 block and distributes that block over a public peer-to-peer network. This is **soft finality**, and the protocol marks the block **"unsafe"** because its data has not yet been posted to Ethereum. This typically takes a few seconds from transaction submission (OP Mainnet produces a new block every 2 seconds). On chains that stream [Subblocks](/op-stack/features/subblocks), applications can additionally act on a preconfirmation 200 milliseconds after submission, before the full block is published.
