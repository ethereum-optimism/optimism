...
*op-supernode* is a process that runs every chain in an interop dependency set together as virtual nodes inside one binary.
...
<img src="/public/img/op-stack/interop/supernode-architecture.png" alt="A single supernode process hosting two chains: each chain has an execution engine (EL-0, EL-1) inside the supernode and a consensus-layer node (CL-0, CL-1) attached at the boundary." width="320" />
