...
OP Stack interop adds a cross-chain message safety layer to the rollup node: before the execution client includes an executing message in a block, the node verifies that the matching initiating message really exists on the source chain, and tracks the safety of every chain whose log events the local chain depends on.
...
Every node participating in interop keeps a database of the log events of the chains in its dependency set.
Every event can potentially initiate a cross-domain message, and the node validates that the log event really happened on the source chain.
The node also reads information from L1's consensus layer to determine the transaction safety of L2 blocks.
