...
graph LR
  ...
  chain2-->|log events|node1
  chain3-->|log events|node1
  l1node-->|block status|node1

  class chain1,chain2,chain3 chain
  class X transparent
```

A node participating in interop indexes the log events of every chain in its dependency set, because any of those events can serve as an initiating message. The node also reads from L1's consensus layer to determine the safety of L2 blocks.

The cross-chain verification work is built into the rollup node itself, so there is no separate "supervisor" service to run alongside it. Verifying initiating messages on other chains does mean the node operator needs a way to follow each chain in the dependency set — typically by running rollup nodes for those chains as well.
