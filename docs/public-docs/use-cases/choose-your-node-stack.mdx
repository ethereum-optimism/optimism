...
clients, and match the pairing to the job the node does. It involves
`op-node` and `kona-node` on the consensus layer, `op-reth` and Nethermind
on the execution layer, and the special-purpose `op-supernode` and legacy
Geth. Support status changes over time; the facts on this page are stated
as of the `last-reviewed` date above, and each exit links where to
re-check them.
...
*   You are choosing software for a new node on an OP Stack chain (OP
    Mainnet, another Superchain chain, or your own chain), or re-evaluating
...
| If ...                                                       | Choose ...                | Because ...                                                                                                                                        |
| -------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| You want the default, for any node role                       | op-reth                    | It is the primary supported execution client: new OP Stack feature development, including hardfork support, happens on op-reth. As of 2026-07-21.  |
| You want client diversity or your team runs .NET infrastructure | Nethermind                 | It is the documented alternative execution client, configured and supported through Nethermind's own documentation.                                |
...
| You want a Rust stack or to contribute client diversity          | kona-node      | It ships tagged releases, superchain-registry support, and sequencer/conductor integration, but its documentation marks it experimental and in active development, so give it a role where that is acceptable. As of 2026-07-21. |
...
*   [op-node configuration reference](/node-operators/reference/op-node-config):
    the full flag catalogue. Hand-pinned to `op-node/v1.10.2` as of
    2026-07-21; confirm values against `op-node --help` for the release you
    run.
*   [op-reth configuration reference](/node-operators/reference/op-reth-config):
    the op-reth flag catalogue, including the historical-proofs options.
*   [kona-node documentation](/rust/kona/node/install/overview): the
    install-and-run book for the Rust consensus client, in the Rust section
    of these docs.
