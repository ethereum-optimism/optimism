...
Transaction finality is the point at which a transaction becomes permanent under a blockchain's security model. Once a transaction is final, users can safely act on the resulting state without expecting it to be reverted or rolled back.

On OP Mainnet, finality provides the guarantee that a transaction is irreversible and will not be lost.

Finality works differently for normal transactions that modify OP Mainnet's L2 state than it does for transactions that withdraw funds from OP Mainnet L2 to Ethereum L1.

<Note>
  **Only transactions that withdraw funds from OP Mainnet to Ethereum must wait 7 days.** Regular transactions within OP Mainnet, such as swaps or sends or receives, **do not** have to wait 7 days.
</Note>
...
**Only withdrawals to Ethereum must wait 7 days to finalize before the funds can be released to the address on Ethereum L1.** This allows OP Mainnet's Fault Proof system to provide extremely high security guarantees for funds bridged to OP Mainnet.
