...
description: Download, verify, and extract an OP Mainnet snapshot into your node's data directory to skip the initial sync.
diataxis: how-to
---

This guide walks you through bootstrapping an OP Mainnet execution client from a pre-synced snapshot: download it, verify the checksum, extract it into your data directory, and start the node from the snapshot's tip instead of replaying the chain from genesis.

For **what** snapshots are available and their download links, see the [Snapshots reference page](/op-mainnet/network-information/snapshots).

## When to use a snapshot

You don't always need one. [Snap sync](/node-operators/guides/management/snap-sync) bootstraps a full `op-geth` node without a snapshot, and [Nethermind](https://docs.nethermind.io/get-started/running-node/l2-networks#op-stack) downloads what it needs automatically.

Use a snapshot when:

*   You are running an **archive node**, or otherwise need to trace the entire chain with `op-reth` or `op-geth`.
*   You are syncing **OP Mainnet with `op-reth`** on a fresh data directory — a snapshot satisfies the pre-Bedrock state import requirement in one step.
*   You simply want to skip days of initial sync: OP Mainnet is roughly 700 GB for a full node, and syncing it from scratch takes days.
...
<Steps>
  <Step title="Pick a snapshot">
    Browse the OP Labs managed [Data Directories website](https://datadirs.optimism.io/) and pick a recent snapshot matching your network and client. The geth snapshots are configured for pebbleDB and the hash state scheme, so make sure your client configuration matches. The [Snapshots reference page](/op-mainnet/network-information/snapshots) lists the available sources, including third-party providers.
  </Step>
  ...
  <Step title="Extract into the data directory">
    ...
    `--strip-components=1` removes the top-level wrapping directory inside the tarball. If the inspection shows files already at the archive root, omit it. Use the same path your client is configured with (for example the `--datadir` flag for `op-reth` and `op-geth`).
  </Step>
...
*   See the [Snapshots reference page](/op-mainnet/network-information/snapshots) for all download links, including the legacy (pre-Bedrock) data directory for archive nodes.
