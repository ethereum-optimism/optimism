...
*   You are syncing **OP Mainnet with `op-reth`** on a fresh data directory — a snapshot satisfies the [pre-Bedrock state import requirement](/rust/op-reth/run/faq/sync-op-mainnet) in one step, even when execution-layer sync is enabled.
*   You are running an **archive node**, or otherwise need to trace the entire chain.
*   You simply want to skip the initial sync entirely: OP Mainnet is roughly 700 GB for a full node, and even execution-layer sync takes days from scratch.
...
<Steps>
  <Step title="Pick a snapshot">
    Browse the OP Labs managed [Data Directories website](https://datadirs.optimism.io/) and pick a recent snapshot matching your network and client — for a current node that means an `op-reth` snapshot, since [`op-geth` is no longer supported](/notices/op-geth-deprecation). The [Snapshots reference page](/op-mainnet/network-information/snapshots) lists the available sources, including third-party providers.
  </Step>
