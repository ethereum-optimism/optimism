...
For step-by-step instructions on downloading, verifying, and extracting a snapshot, follow the [Restore a node from a snapshot guide](/op-mainnet/network-information/restore-from-snapshot).
