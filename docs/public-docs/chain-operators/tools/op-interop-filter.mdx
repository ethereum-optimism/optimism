...
The EL points at the filter through op-reth's `--rollup.supervisor-http` flag and calls the filter's `interop_checkAccessList` method to validate each interop transaction.
...
The default `--message-expiry-window` of 7 days matches op-supervisor's default, so an operator migrating off op-supervisor for transaction validation does not need to retune that knob.
...
| `--message-expiry-window` | `168h` (7 days) | Messages older than this window are treated as expired. Matches op-supervisor's default. |
