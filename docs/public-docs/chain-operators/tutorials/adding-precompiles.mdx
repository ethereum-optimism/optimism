...
<Steps>
  <Step title="Add to `PrecompiledContractsBerlin` on line 82 (or a later fork, if the list of precompiles changes again)">
  *   add a structure named after your new precompile, and
  ...
  <Step title="Add the lines for the precompile">
  ```go
  ...
  <Step title="Stop `op-geth` and recompile:">
  ```bash
    cd ~/op-geth
    make geth
    ```
  ...
  <Step title="Restart `op-geth`">
  
  </Step>

  <Step title="Run these commands to see the result of calling the precompile successfully, and the result of an error:">
  ```bash
...
The key of the map is an address. You create those from bytes using `common.BytesToAddress([]byte{<bytes to convert to address go here>})`.  In this case you have two bytes, `0x13` and `0x37`. Together you get the address `0x0…1337`.
