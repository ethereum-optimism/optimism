...
By the end you will have one `op-proposer` proposing super-root claims to the shared factory against an `op-supernode` that derives both chains, one `op-challenger` defending super-root games on the shared factory, one `op-dispute-mon` watching it, and per-chain `op-challenger` and `op-dispute-mon` "drain" instances winding down the retired games on the old per-chain factories.

> **Critical:** `opcm.migrate` is a one-way operation that invalidates every withdrawal proof submitted but not finalized on either chain. Affected users must re-prove against a new super-root game after migration. Announce the migration window to users at least seven days in advance—long enough for a withdrawal proven just before the announcement to mature past `PROOF_MATURITY_DELAY_SECONDS` and finalize.
...
* Both chains run permissionless fault proofs with `CANNON_KONA` (game type `8`) as the respected game type today.
* A single `op-supernode` instance is in sync and derives both chains in the same process.
...
| `op-supernode` RPC URL | Your infrastructure team. The supernode must derive both `chainA` and `chainB`. |
...
* **Confirm the current respected game type on both chains is `CANNON_KONA` (`8`).**
...
* **Confirm the supernode is in sync and derives both chains.**

  ```bash
  cast rpc supernode_syncStatus --rpc-url $SUPERNODE_RPC | jq -r '.finalized_timestamp'
  ```

  If the supernode is materially behind either chain's expected finality window, postpone the migration.
...
A single `op-challenger` process can only watch one `DisputeGameFactory`, and per-chain trace types like `cannon-kona` and `permissioned` require a single `--rollup-rpc` and a single `--l2-eth-rpc`. The shared super-root factory needs both chains' EL endpoints and a depset that covers both chain IDs. Run a third `op-challenger` instance dedicated to the shared factory in addition to the existing per-chain instances.
...
Leave the per-chain `op-challenger` instances untouched. They keep `--rollup-rpc <chainX-rollup>`, `--l2-eth-rpc <chainX-EL>`, `--game-factory-address <oldDGF_chainX>`, and `--game-types cannon-kona,permissioned` for the duration of the drain window.
...
Leave both per-chain `op-dispute-mon` instances running with their existing `--rollup-rpc` and per-chain `--game-factory-address` so they continue to track in-flight pre-migration games to resolution.

### Leave op-proposer Unchanged for Now

Do not touch `op-proposer` until [Execute the Migration](#execute-the-migration). Its configuration changes at cutover; see [Switch op-proposer to Super Roots](#switch-op-proposer-to-super-roots).
...
> **Critical:** Verify that the user announcement has gone out and at least seven days have elapsed since the announcement before broadcasting. Withdrawals proven against either chain's old games will not finalize against the new shared registry; users must re-prove or finalize ahead of cutover.

1. Stop both per-chain `op-proposer` processes. Once the migration broadcasts, the per-chain factories no longer have `CANNON_KONA` registered, so `DisputeGameFactory.create` reverts. Stopping the proposers first prevents wasted gas.
...
`SUPER_CANNON_KONA` is a permissionless game type, so the moment the shared factory is live anyone can create a game against it — not just `op-proposer`. Once a game is created, its clock runs and it resolves whether or not anyone challenges. `op-challenger` must be running to defend invalid claims before their game clocks expire, and `op-dispute-mon` must be running to observe and alert. This is independent of [Switch op-proposer to Super Roots](#switch-op-proposer-to-super-roots) below — complete it first regardless of when the proposer cutover happens.
...
Start one new `op-proposer` process configured for super-root mode against the shared factory. The supernode aggregates both chains' outputs into a single super root per timestamp, so a single proposer process replaces both per-chain ones.

| Flag (env var) | Old (per-chain) | New (single super-root proposer) |
| --- | --- | --- |
| `--rollup-rpc` (`OP_PROPOSER_ROLLUP_RPC`) | per-chain rollup node | **remove** |
| `--supernode-rpcs` (`OP_PROPOSER_SUPERNODE_RPCS`) | unset | `op-supernode` RPC URL or URLs (multiple values are HA failover, not multi-chain) |
| `--game-factory-address` (`OP_PROPOSER_GAME_FACTORY_ADDRESS`) | per-chain factory | the new shared `<DGF>` |
| `--game-type` (`OP_PROPOSER_GAME_TYPE`) | `8` | `9` |
| `--l1-eth-rpc` (`OP_PROPOSER_L1_ETH_RPC`) | unchanged | unchanged |
| `--proposal-interval`, `--poll-interval`, `--mnemonic` or `--private-key` | unchanged | unchanged |

`--rollup-rpc` and `--supernode-rpcs` are mutually exclusive; the proposer rejects start-up if both are set.
...
* The two per-chain `op-dispute-mon` instances keep observing those games. The per-game-type metric `op_dispute_mon_games{game_type="cannon-kona"}` and the equivalent for `permissioned` should monotonically decrease toward zero across the drain window.
