...
* Both chains have already been upgraded to a release that exposes `OPContractsManagerMigrator.migrate` and have `Features.ETH_LOCKBOX` enabled on their `SystemConfig`. `ETH_LOCKBOX` is turned on automatically by `OPContractsManagerV2.upgrade()`; that pre-upgrade is a prerequisite for this runbook. `Features.INTEROP` must still be **disabled** going in, `migrate()` enables it.
...
| Item | Source |
| --- | --- |
| `OPCM` address for the migrate-feature container | The same OPCM the chains ran for the prerequisite `OPCMv2.upgrade()`. This run is what enables `Features.ETH_LOCKBOX` on each `SystemConfig`. `Features.INTEROP` is not set by the upgrade; `migrate()` sets it. The container must have `OPTIMISM_PORTAL_INTEROP` set in `devFeatureBitmap`, which gates `migrate()` itself; without it `migrate()` reverts with `OPContractsManagerMigrator_InteropNotEnabled`. |
...
* **Confirm `Features.INTEROP` is disabled and `Features.ETH_LOCKBOX` is enabled on both chains.** `migrate()` is the only thing that sets `INTEROP` on L1, so a chain that already has it on is already in an interop set; `_validateChainSystemConfigs` reverts with `OPContractsManagerMigrator_ChainAlreadyMigrated` when INTEROP is already on, so **do not** enable `INTEROP` yourself via `SystemConfig.setFeature`, doing so permanently blocks the migration. `ETH_LOCKBOX` flips on automatically during the prerequisite `OPCMv2.upgrade()`; if either `ETH_LOCKBOX` check below returns `false`, run that prerequisite upgrade first. `Features` constants are short-string `bytes32` values, so feed them through `cast format-bytes32-string`.
  ...
  # Expect: both INTEROP lines end in false, both ETH_LOCKBOX lines end in true.
  ```
