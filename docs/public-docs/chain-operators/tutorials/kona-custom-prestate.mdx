...
- [Generating an op-program absolute prestate (archived)](/chain-operators/tutorials/archive/op-program-prestate): the legacy `op-program` flow, kept only for resolving in-flight `CANNON` (game type `0`) disputes created before [Upgrade 19](/notices/upgrade-19). `op-program` has reached end-of-support; see [End of Support for op-geth and op-program](/notices/op-geth-deprecation).
