...
<Tabs>
  ...
  <Tab title="Build from source">
    ...
    <Info>
      This guide uses `op-proposer/v1.10.0`, compatible with the op-node and op-reth versions from the [sequencer setup](./op-reth-setup).
    ...
    <Steps>
      ...
      <Info>
        ...
        `GAME_TYPE` must match the game type your chain *respects*. `0` is permissionless
        `CANNON` (op-program); `1` is `PERMISSIONED_CANNON`; from the Karst upgrade the respected
        type is **`8` (`CANNON_KONA`)**, the permissionless kona game. Set `GAME_TYPE` to whichever
        your chain has registered and made respected — the proposer can only create games of a type
        that has an implementation registered on the `DisputeGameFactory`.
      </Info>
