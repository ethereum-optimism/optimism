...
<NormativeSpec
  ...
  note="This tutorial covers spinning up op-challenger; it does not restate the spec."
/>
...
<Tabs>
  ...
  <Tab title="Build from source">
    ...
    **Build kona-host**
    ...
    The challenger also needs `kona-host`, the pre-image oracle server for the `cannon-kona` game type. Build it from the same release tag you used for the absolute prestate (see the prestate step in the prerequisites), so the server matches the `kona-client` build committed on chain:

    ```bash
    # In the checkout you built the prestate from (kona-node/v1.6.1)
    cd rust
    ...
    ## Configuration setup
        ...
        <Info>
          The following steps require a Cannon binary and the `kona-host` pre-image server. 
          Ensure these binaries are available at the paths you configure (`CANNON_BIN` and `CANNON_KONA_SERVER`). 
        </Info>
    ...
    # Copy the prestate preimage file generated in the prerequisites
    cp /path/to/prestate-artifacts-cannon/0x<PRESTATE_HASH>.bin.gz .
    ```
    </Step>
    ...
    *   Network identifier (e.g., op-sepolia, op-mainnet, or custom)
    ...
    # Trace configuration (cannon-kona is the maintained fault-proof game type)
    ...
    CANNON_KONA_ROLLUP_CONFIG=./rollup.json
    CANNON_KONA_L2_GENESIS=./genesis-l2.json

    # Cannon VM binary (built with 'just cannon' in the op-challenger checkout)
    CANNON_BIN=../../optimism/cannon/bin/cannon
    # kona-host pre-image server - built from the same release tag as your prestate
    CANNON_KONA_SERVER=/path/to/rust/target/release/kona-host
    # Prestate file - generated with 'just reproducible-prestate-kona' in the prerequisites
    CANNON_KONA_PRESTATE=./0x<PRESTATE_HASH>.bin.gz
    ...
    **Important:** Replace ALL placeholder values (`YOUR_ACTUAL_*`) with your real configuration values.
    ...
    ../../optimism/op-challenger/bin/op-challenger \
      ...
      --cannon-kona-rollup-config=$CANNON_KONA_ROLLUP_CONFIG \
      --cannon-kona-l2-genesis=$CANNON_KONA_L2_GENESIS \
