...
<Warning>
  **This guide still shows the legacy op-program prestate flow and op-geth examples, and is being rewritten.** For fault proofs, op-program is being replaced by **kona-client** (run as `cannon-kona`) at Karst, and op-geth reached end-of-support on 2026-05-31.

  For current guidance, use the [op-challenger configuration guide](/chain-operators/guides/configuration/op-challenger-config-guide) and the [absolute prestate tutorial](/chain-operators/tutorials/absolute-prestate), and configure your execution layer with op-reth per the [execution clients guide](/node-operators/guides/configuration/execution-clients). See the [op-geth deprecation notice](/notices/op-geth-deprecation).
</Warning>
...
<Tabs>
  ...
  <Tab title="Build from source">
    ...
    *   When cannon is executed, challenger needs the roll-up config and the L2 Genesis file. Both files are automatically loaded when Cannon Network is used, but custom networks will need to specify both Cannon L2 Genesis and Cannon rollup config.
