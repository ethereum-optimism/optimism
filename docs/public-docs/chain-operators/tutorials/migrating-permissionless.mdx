...
description: "Migrate your OP Stack chain from permissioned to permissionless fault proofs: configure the dispute components, deploy the contracts with OPCM, test the off-chain agents, and switch the respected game type."
diataxis: how-to
audit-source:
...
*   `rust/kona/prestate-artifacts-cannon/<PRESTATE_HASH>.bin.gz` — the preimage file `op-challenger` loads
