...
| Item | Source |
| --- | --- |
| `OPCM` address for the target network | OPCM release notes at `https://github.com/ethereum-optimism/optimism/releases` (filter for `op-contracts/v8.x.y` tags). Use the version pinned by the `OPCMUpgradeV800` template. |
