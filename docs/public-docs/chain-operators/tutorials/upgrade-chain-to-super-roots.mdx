...
* **`op-dispute-mon` reports super-root games.** Confirm `op_dispute_mon_games` reports the enabled super game types and `op_dispute_mon_failed_games` remains zero. On a permissioned chain, alert on `op_dispute_mon_games_agreement{game_type="super-permissioned",status="disagree_defender_wins",result_correctness="incorrect"} > 0` and `Unexpected game result` logs for `SUPER_PERMISSIONED`. Existing dashboards and queries for `op_dispute_mon_games_agreement` must filter or aggregate the `game_type` label; broader dashboard updates are tracked in [optimism#21405](https://github.com/ethereum-optimism/optimism/issues/21405).
