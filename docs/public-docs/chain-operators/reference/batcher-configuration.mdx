...
definitions in the monorepo (`op-batcher/flags`), and CI keeps them in sync
with the source: a flag change that is not reflected here fails the build, so
the tables cannot silently fall behind a release. Every flag can also be set
