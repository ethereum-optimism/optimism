...
Producer policy output is advisory. To keep a faulty policy from halting block production, op-reth
caps a normal transaction's requested refund at its pre-refund EVM gas and ignores refunds requested
for deposits. Each correction increments
`reth_optimism_sdm_policy_refund_corrections{reason="exceeds_evm_gas"}` or
`reth_optimism_sdm_policy_refund_corrections{reason="ineligible_transaction"}`. Either series should
remain zero under a healthy producer policy.
