...
![OP Stack network design example (op-reth)](/public/img/chain-operators/network-design-example-op-reth.png)
...
op-reth runs as an archive node by default, so no extra flags are required — simply do **not** pass `--full` (which enables pruning; the upstream `--minimal` flag is not supported and op-reth rejects it at startup). op-reth stores state in MDBX plus static files, so the geth-specific `GETH_DB_ENGINE` and `GETH_STATE_SCHEME` options have no equivalent.
...
```sh
--full        # run a pruned full node (--minimal is not supported by op-reth)
```
