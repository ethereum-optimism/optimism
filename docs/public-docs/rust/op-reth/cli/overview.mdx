...
`op-reth node`, which starts the execution client itself. Every other
subcommand is an operational tool that works against the same data directory —
initializing it, importing history into it, inspecting it, or repairing it —
so you rarely run them while the node is running.
...
each flag namespaced by its group (for example `--rpc.*`, `--txpool.*`,
`--prune.*`, `--rollup.*`).
