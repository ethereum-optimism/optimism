...
## SuperchainTokenBridge (removed)

<Warning>
**This predeploy no longer exists.** It was removed from the OP Stack in [optimism#19999](https://github.com/ethereum-optimism/optimism/pull/19999) and nothing has replaced it, so SuperchainERC20 tokens cannot move between chains today. `0.1.0-alpha.59` is the last Supersim release that deploys it.

For L2 to L2 ETH transfers use `SuperchainETHBridge` at `0x4200000000000000000000000000000000000024`. For general message passing use `L2ToL2CrossDomainMessenger` at `0x4200000000000000000000000000000000000023`.
</Warning>

The `SuperchainTokenBridge` was an abstraction on top of the `L2ToL2CrossDomainMessenger` that facilitated token bridging using interop, as described in the [token bridging spec](https://specs.optimism.io/interop/token-bridging.html).
...
*   **Source code:** [`SuperchainTokenBridge`](https://github.com/ethereum-optimism/optimism/blob/op-node/v1.16.12/packages/contracts-bedrock/src/L2/SuperchainTokenBridge.sol) at `op-node/v1.16.12`, the last release before it was removed
