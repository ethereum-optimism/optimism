...
description: "Explains how Flashblocks affect block gas usage, transaction inclusion, and why large transactions can be rejected or stay pending."
...
If you are unfamiliar with Flashblocks you can read more in the [Flashblocks explainer](/op-stack/features/flashblocks).
...
Flashblocks are an incremental block-building mechanism: instead of constructing the full block at once, the sequencer's block builder assembles it as a series of flashblocks, each of which adds to the *cumulative gas budget* available for inclusion.
For what Flashblocks are, the components involved, and how many flashblocks a block contains, see the [Flashblocks explainer](/op-stack/features/flashblocks).
The consequence for gas usage is that earlier flashblocks can only include smaller transactions, and larger transactions fit only later in the block, *if enough gas remains*.
