...
*   **Transaction Batches:** All transactions where the transaction from address is the `batcherHash` address as defined in the chains' `SystemConfigProxy` contract, and the transaction to address is the chain's `batch_inbox_address` as defined in the Superchain Registry. (The batch inbox address was previously also readable from the `SystemConfigProxy` contract, but that getter is removed as of SystemConfig 4.0.0.)
