...
func TestEVM_SysClone_FlagHandling(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			state.GetRegistersRef()[4] = uint64(c.flags) // Set first argument
			curStep := state.Step
...
func TestEVM_SysClone_Successful(t *testing.T) {
	...
	for i, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, uint64(1), state.NextThreadId)
...
func TestEVM_SysExit(t *testing.T) {
	...
	for i, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			state.GetRegistersRef()[4] = uint64(exitCode) // The first argument (exit code)
			step := state.Step
...
func TestEVM_SysFutex_UnsupportedOp(t *testing.T) {
	...
	unsupportedFutexOps := map[string]uint64{
...
func runPreemptSyscall(t *testing.T, syscallName string, syscallNum uint64) {
	var tracer *tracing.Hooks
	cases := []struct {
		name            string
		traverseRight   bool
...
var NoopSyscalls = map[string]uint64{
	"SysGetAffinity":   4240,
	"SysMadvise":       4218,
...
func TestEVM_UnsupportedSyscall(t *testing.T) {
	...
	var SupportedSyscalls = []uint64{exec.SysMmap, exec.SysBrk, exec.SysClone, exec.SysExitGroup, exec.SysRead, exec.SysWrite, exec.SysFcntl, exec.SysExit, exec.SysSchedYield, exec.SysGetTID, exec.SysFutex, exec.SysOpen, exec.SysNanosleep}
	unsupportedSyscalls := make([]uint64, 0, 400)
	for i := 4000; i < 4400; i++ {
		candidate := uint64(i)
...
func TestEVM_WakeupTraversalStep(t *testing.T) {
	wakeupAddr := uint64(0x1234)
	wakeupVal := uint64(0x999)
	var tracer *tracing.Hooks
	cases := []struct {
		name              string
		futexAddr         uint64
	...
	for i, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			state.GetMemory().SetDoubleWord(wakeupAddr, wakeupVal)
			activeThread := state.GetCurrentThread()
			activeThread.FutexAddr = c.futexAddr
