...
func FuzzStateSyscallMmap(f *testing.F) {
	...
	f.Add(uint64(0), uint64(0x1000), uint64(program.HEAP_END), int64(1))
	f.Add(uint64(0), uint64(1<<31), uint64(program.HEAP_START), int64(2))
	// Check edge case - just within bounds
	f.Add(uint64(0), uint64(0x1000), uint64(program.HEAP_END-4096), int64(3))

	versions := GetMipsVersionTestCases(f)
	f.Fuzz(func(t *testing.T, addr uint64, siz uint64, heap uint64, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
	...
	f.Fuzz(func(t *testing.T, exitCode uint8, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state.GetRegistersRef()[4] = uint64(exitCode)
				state.GetMemory().SetMemory(state.GetPC(), syscallInsn)
				step := state.GetStep()
...
func FuzzStateSyscallFcntl(f *testing.F) {
	versions := GetMipsVersionTestCases(f)
	f.Fuzz(func(t *testing.T, fd uint64, cmd uint64, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(),
					testutil.WithRandomization(seed))
...
func FuzzStateHintRead(f *testing.F) {
	versions := GetMipsVersionTestCases(f)
	f.Fuzz(func(t *testing.T, addr uint64, count uint64, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
...
func FuzzStatePreimageRead(f *testing.F) {
	versions := GetMipsVersionTestCases(f)
	f.Fuzz(func(t *testing.T, addr uint64, count uint64, preimageOffset uint64, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				preimageValue := []byte("hello world")
				if preimageOffset >= uint64(len(preimageValue)) {
					t.SkipNow()
				}
				...
				preimageDataLen := uint64(len(preimageValue) + 8) // Data len includes a length prefix
