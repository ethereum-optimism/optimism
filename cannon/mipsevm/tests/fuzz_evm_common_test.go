	...
	f.Fuzz(func(t *testing.T, pc uint32, step uint64, preimageOffset uint32) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysBrk
	...
	f.Fuzz(func(t *testing.T, addr uint32, siz uint32, heap uint32, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				*state.GetRegisters() = testutil.RandomRegisters(seed)
				state.GetRegisters()[2] = exec.SysMmap
				state.GetRegisters()[4] = addr
				state.GetRegisters()[5] = siz
	...
	f.Fuzz(func(t *testing.T, exitCode uint8, pc uint32, step uint64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysExitGroup
				state.GetRegisters()[4] = uint32(exitCode)
				state.GetMemory().SetMemory(pc, syscallInsn)
				...
				require.Equal(t, preStateRoot, state.GetMemory().MerkleRoot())
				require.Equal(t, preStateRegisters, state.GetRegisters())
				require.Equal(t, step+1, state.GetStep())
				require.Equal(t, common.Hash{}, state.GetPreimageKey())
				require.Equal(t, uint32(0), state.GetPreimageOffset())
		...
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(),
					WithStep(step))
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysFcntl
				state.GetRegisters()[4] = fd
				state.GetRegisters()[5] = cmd
				state.GetMemory().SetMemory(0, syscallInsn)
	...
	f.Fuzz(func(t *testing.T, addr uint32, count uint32) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysRead
				state.GetRegisters()[4] = exec.FdHintRead
				state.GetRegisters()[5] = addr
				state.GetRegisters()[6] = count
				state.GetMemory().SetMemory(0, syscallInsn)
	...
	f.Fuzz(func(t *testing.T, addr uint32, count uint32, preimageOffset uint32) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysRead
				state.GetRegisters()[4] = exec.FdPreimageRead
				state.GetRegisters()[5] = addr
				state.GetRegisters()[6] = count
				state.GetMemory().SetMemory(0, syscallInsn)
		...
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(),
					WithStep(step), WithPreimageKey(preimageKey))
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysWrite
				state.GetRegisters()[4] = exec.FdHintWrite
				state.GetRegisters()[5] = addr
				state.GetRegisters()[6] = count
	...
	f.Fuzz(func(t *testing.T, addr uint32, count uint32) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				state := goVm.GetState()
				state.GetRegisters()[2] = exec.SysWrite
				state.GetRegisters()[4] = exec.FdPreimageWrite
				state.GetRegisters()[5] = addr
				state.GetRegisters()[6] = count
				state.GetMemory().SetMemory(0, syscallInsn)
