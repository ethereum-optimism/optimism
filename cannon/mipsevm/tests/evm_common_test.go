	...
	for _, c := range cases {
		...
		for _, f := range testFiles {
			...
			t.Run(testName, func(t *testing.T) {
				...
				if exitGroup {
					require.NotEqual(t, uint64(testutil.EndAddr), goVm.GetState().GetPC(), "must not reach end")
					...
					require.NotEqual(t, uint64(testutil.EndAddr), state.GetPC(), "must not reach end")
				} else {
					require.Equal(t, uint64(testutil.EndAddr), state.GetPC(), "must reach end")
					// inspect test result
...
func TestEVM_MMap(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "Increment heap by max value", heap: program.HEAP_START, address: 0, size: ^uint64(0), shouldFail: true},
		{name: "Increment heap to 0", heap: program.HEAP_START, address: 0, size: ^uint64(0) - program.HEAP_START + 1, shouldFail: true},
		{name: "Increment heap to previous page", heap: program.HEAP_START, address: 0, size: ^uint64(0) - program.HEAP_START - memory.PageSize + 1, shouldFail: true},
		{name: "Increment max page size", heap: program.HEAP_START, address: 0, size: ^uint64(0) & ^uint64(memory.PageAddrMask), shouldFail: true},
		{name: "Increment max page size from 0", heap: 0, address: 0, size: ^uint64(0) & ^uint64(memory.PageAddrMask), shouldFail: true},
	...
	for _, v := range versions {
		for i, tt := range cases {
			...
			t.Run(testName, func(t *testing.T) {
				...
				state.GetRegistersRef()[5] = uint64(tt.memOffset)
				state.GetRegistersRef()[6] = uint64(tt.bytesToWrite)

				err := state.GetMemory().SetMemoryRange(uint64(tt.memOffset), bytes.NewReader(tt.hintData))
				require.NoError(t, err)
				...
				expected.Registers[2] = uint64(tt.bytesToWrite) // Return count of bytes written
