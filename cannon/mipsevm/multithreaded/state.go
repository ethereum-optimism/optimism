...
type State struct {
	...
	PreimageOffset uint64 // note that the offset includes the 8-byte length prefix

	Heap uint64 // to handle mmap growth
...
func (s *State) GetRegistersRef() *[32]uint64 {
	activeThread := s.GetCurrentThread()
...
// NextThreadId                uint64
// len(LeftThreadStack)        uint64
// LeftThreadStack entries     as per ThreadState.Serialize
// len(RightThreadStack)       uint64
