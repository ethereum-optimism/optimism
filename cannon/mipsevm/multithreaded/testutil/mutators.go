...
func (m *StateMutatorMultiThreaded) SetHI(val uint64) {
	m.state.GetCurrentThread().Cpu.HI = val
}

func (m *StateMutatorMultiThreaded) SetLO(val uint64) {
...
func (m *StateMutatorMultiThreaded) SetPC(val uint64) {
	thread := m.state.GetCurrentThread()
	thread.Cpu.PC = val
}

func (m *StateMutatorMultiThreaded) SetHeap(val uint64) {
	m.state.Heap = val
}

func (m *StateMutatorMultiThreaded) SetNextPC(val uint64) {
	thread := m.state.GetCurrentThread()
...
func (m *StateMutatorMultiThreaded) SetPreimageOffset(val uint64) {
