...
func (m *Memory) Invalidate(addr uint64) {
	...
	gindex := u128.From64(1).Lsh(64).Or(u128.From64(addr)).Rsh(PageAddrSize).Big().Uint64()
...
func (m *Memory) MerkleizeSubtree(gindex uint64) [32]byte {
	...
	if l > PageKeySize {
		...
		if p, ok := m.pages[uint64(pageIndex)]; ok {
			...
			return zeroHashes[60-l] // page does not exist
		}
	}
	n, ok := m.nodes[gindex]
	if !ok {
		// if the node doesn't exist, the whole sub-tree is zeroed
		return zeroHashes[60-l]
	}
	if n != nil {
		return *n
	}
...
func (m *Memory) MerkleProof(addr uint64) (out [MEM_PROOF_SIZE]byte) {
...
func (m *Memory) SetMemory(addr uint64, v uint32) {
	// addr must be aligned to 4 bytes
	if addr&0x3 != 0 {
		panic(fmt.Errorf("unaligned memory access: %x", addr))
	}
...
func (m *Memory) SetDoubleWord(addr uint64, v uint64) {
	...
	binary.BigEndian.PutUint64(p.Data[pageAddr:pageAddr+8], v)
}
...
func (m *Memory) GetDoubleWord(addr uint64) uint64 {
	// addr must be aligned to 8 bytes
	if addr&0x7 != 0 {
		panic(fmt.Errorf("unaligned memory access: %x", addr))
	}
