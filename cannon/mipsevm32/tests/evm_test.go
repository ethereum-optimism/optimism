	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.valid {
				...
				require.Equal(t, uint8(vmstatus.VMStatusPanic), us.GetState().GetExitCode())
			} else {
...
func TestHelloEVM(t *testing.T) {
	...
	state := testutil.LoadELFProgram(t, "../../testdata/example32/bin/hello.elf", singlethreaded.CreateInitialState, true)
...
func TestClaimEVM(t *testing.T) {
	...
	state := testutil.LoadELFProgram(t, "../../testdata/example32/bin/claim.elf", singlethreaded.CreateInitialState, true)
