...
func ExecMipsCoreStepLogic(cpu *mipsevm32.CpuScalars, registers *[32]uint32, memory *memory.Memory, insn, opcode, fun uint32, memTracker MemTracker, stackTracker StackTracker) error {
	// j-type j/jal
...
func HandleBranch(cpu *mipsevm32.CpuScalars, registers *[32]uint32, opcode uint32, insn uint32, rtReg uint32, rs uint32) error {
...
func HandleHiLo(cpu *mipsevm32.CpuScalars, registers *[32]uint32, fun uint32, rs uint32, rt uint32, storeReg uint32) error {
	val := uint32(0)
	switch fun {
	case 0x10: // mfhi
		val = cpu.HI
...
func HandleJump(cpu *mipsevm32.CpuScalars, registers *[32]uint32, linkReg uint32, dest uint32) error {
	if cpu.NextPC != cpu.PC+4 {
...
func HandleRd(cpu *mipsevm32.CpuScalars, registers *[32]uint32, storeReg uint32, val uint32, conditional bool) error {
	if storeReg >= 32 {
