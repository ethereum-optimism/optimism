...
func ExecMipsCoreStepLogic(cpu *mipsevm64.CpuScalars, registers *[32]uint64, memory *memory.Memory, insn, opcode, fun uint64, memTracker MemTracker, stackTracker StackTracker) error {
	// j-type j/jal
...
func HandleBranch(cpu *mipsevm64.CpuScalars, registers *[32]uint64, opcode, insn, rtReg, rs uint64) error {
...
func HandleHiLo(cpu *mipsevm64.CpuScalars, registers *[32]uint64, fun, rs, rt, storeReg uint64) error {
	val := uint64(0)
	switch fun {
	case 0x10: // mfhi
		val = cpu.HI
...
func HandleJump(cpu *mipsevm64.CpuScalars, registers *[32]uint64, linkReg uint64, dest uint64) error {
	if cpu.NextPC != cpu.PC+4 {
...
func HandleRd(cpu *mipsevm64.CpuScalars, registers *[32]uint64, storeReg uint64, val uint64, conditional bool) error {
	if storeReg >= 32 {
