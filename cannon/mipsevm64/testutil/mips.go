...
type MIPSEVM struct {
	...
	addrs       *Addresses
	localOracle mipsevm64.PreimageOracle
	artifacts   *Artifacts
...
func (m *MIPSEVM) SetLocalOracle(oracle mipsevm64.PreimageOracle) {
	m.localOracle = oracle
}

// Step is a pure function that computes the poststate from the VM state encoded in the StepWitness.
func (m *MIPSEVM) Step(t *testing.T, stepWitness *run.StepWitness, step uint64, stateHashFn run.HashFn) []byte {
	m.lastStep = step
	m.lastStepInput = nil
	...
	if stepWitness.HasPreimage() {
		...
		poInput, err := EncodePreimageOracleInput(t, stepWitness, run.LocalContext{}, m.localOracle, m.artifacts.Oracle)
	...
	input := EncodeStepInput(t, stepWitness, run.LocalContext{}, m.artifacts.MIPS)
...
func EncodeStepInput(t *testing.T, wit *run.StepWitness, localContext run.LocalContext, mips *foundry.Artifact) []byte {
	input, err := mips.ABI.Pack("step", wit.State, wit.ProofData, localContext)
	require.NoError(t, err)
	return input
}

func EncodePreimageOracleInput(t *testing.T, wit *run.StepWitness, localContext run.LocalContext, localOracle mipsevm64.PreimageOracle, oracle *foundry.Artifact) ([]byte, error) {
