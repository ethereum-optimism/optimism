...
//			require.NoError(t, err)
//			state := &multithreaded.State{Cpu: mipsevm64.CpuScalars{PC: 0, NextPC: 4}, Memory: memory.NewMemory()}
...
//				require.Equal(t, uint8(mipsevm64.VMStatusPanic), us.GetState().GetExitCode())
//			} else {
...
//	for _, tt := range cases {
//		t.Run(tt.name, func(t *testing.T) {
//			state := &multithreaded.State{Cpu: mipsevm64.CpuScalars{PC: tt.pc, NextPC: tt.nextPC}, Memory: memory.NewMemory()}
...
//			curStep := state.Step
//
//			us := multithreaded.NewInstrumentedState(state, &oracle, os.Stdout, os.Stderr, nil)
//			stepWitness, err := us.Step(true)
//			require.NoError(t, err)
...
//	for _, tt := range cases {
//		t.Run(tt.name, func(t *testing.T) {
//			state := &multithreaded.State{Cpu: mipsevm64.CpuScalars{PC: 0, NextPC: tt.nextPC}, Memory: memory.NewMemory()}
//			initialState := &multithreaded.State{Cpu: mipsevm64.CpuScalars{PC: 0, NextPC: tt.nextPC}, Memory: state.Memory}
...
//			stepWitness := &run.StepWitness{
//				State:     encodedWitness,
//				ProofData: insnProof[:],
//			}
//			input := testutil.EncodeStepInput(t, stepWitness, run.LocalContext{}, contracts.MIPS)
...
//	testutil.LogStepFailureAtCleanup(t, evm)
//
//	state := testutil.LoadELFProgram(t, "../../testdata/example64/bin/hello.elf", multithreaded.CreateInitialState, true)
//	var stdOutBuf, stdErrBuf bytes.Buffer
//	goState := multithreaded.NewInstrumentedState(state, nil, io.MultiWriter(&stdOutBuf, os.Stdout), io.MultiWriter(&stdErrBuf, os.Stderr), nil)
//
//	start := time.Now()
//	for i := 0; i < 400_000; i++ {
//		curStep := goState.GetState().GetStep()
...
//	testutil.LogStepFailureAtCleanup(t, evm)
//
//	state := testutil.LoadELFProgram(t, "../../testdata/example64/bin/claim.elf", multithreaded.CreateInitialState, true)
//	oracle, expectedStdOut, expectedStdErr := testutil.ClaimTestOracle(t)
//
//	var stdOutBuf, stdErrBuf bytes.Buffer
//	goState := multithreaded.NewInstrumentedState(state, oracle, io.MultiWriter(&stdOutBuf, os.Stdout), io.MultiWriter(&stdErrBuf, os.Stderr), nil)
//
...
//		require.NoError(t, err)
//
//		evmPost := evm.Step(t, stepWitness, curStep, multithreadedmultithreaded.GetStateHashFn())
//
//		goPost, _ := goState.GetState().EncodeWitness()
//		require.Equal(t, hexutil.Bytes(goPost).String(), hexutil.Bytes(evmPost).String(),
//			"mipsevm produced different state than EVM")
