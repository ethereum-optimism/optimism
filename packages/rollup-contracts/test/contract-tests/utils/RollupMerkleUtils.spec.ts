...
/* Internal Imports */
import { makeRepeatedBytes, makeRandomBlockOfSize } from '../../test-helpers'
...
describe('RollupMerkleUtils', () => {
  ...
  describe('computeInclusionProofRoot()', async () => {
    it('should compute the root from inclusion proof for trees of various heights', async () => {
      ...
      for (
        ...
        for (let j = 0; j < block.length; j++) {
          ...
          const expectedRoot = bufToHexString(inclusionProof.rootHash)
          const calculatedRoot = await rollupMerkleUtils.computeInclusionProofRoot(
            bufToHexString(inclusionProof.value),
            path,
            siblings
          )
          // Make sure that the root calculation was successful
          calculatedRoot.should.equal(expectedRoot)
        }
      }
    })
  })
