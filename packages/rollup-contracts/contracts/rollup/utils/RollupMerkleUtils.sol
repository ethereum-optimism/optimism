...
contract RollupMerkleUtils {
    ...
    function getMerkleRoot(
        ...
        // If we only have a single node, then it must be the root.
    ...
    function verify(
        ...
        // Check that the root matches what we've been given.
    ...
    function storeLeaf(
        ...
        // Provided leaf should already be a hash.
        bytes32 computedNode = _leaf;

        // Run through each of our sibling hashes.
        for (uint i = 0; i < _siblings.length; i++) {
            bytes32 parent;
            bytes32 sibling = _siblings[i];
            uint8 isComputedRightSibling = getNthBitFromRight(_path, i);
