...
type State = {
  ...
  invalidProposalWithdrawals: Array<string>
  invalidProofWithdrawals: Array<string>
...
> {
    ...
    super({
      ...
      metricsSpec: {
        highestCheckedBlockNumber: {
          type: Gauge,
          desc: 'Highest block number checked for withdrawals',
          labels: ['type'],
        },
        highestKnownBlockNumber: {
          type: Gauge,
          desc: 'Highest block number known on L1',
          labels: ['type'],
        },
        ...
        invalidProposalWithdrawals: {
          type: Gauge,
          desc: '0 if state is ok. 1 or more if invalid proposal withdrawals are detected.',
        },
        invalidProofWithdrawals: {
          type: Gauge,
          desc: '0 if state is ok. 1 or more if invalid proof withdrawals are detected.',
        },
        forgeryDetected: {
          type: Gauge,
          desc: '0 if no forgeries detected. 1 if forgeries detected.',
        },
  ...
  async main(): Promise<void> {
    ...
    // Set metrics based on states
    this.metrics.forgeryDetected.set(this.state.forgeryDetected ? 1 : 0)
    ...
    // for (const withdrawal of this.state.invalidProposalWithdrawals) {
    //   const { gameId, gameType, status } = withdrawal
    //   if (
    //     this.state.blacklistedGames.includes(gameId) ||
    //     gameType !== this.state.portal.respectedGameType
    //   ) {
    //     // Remove from state.invalidProposalWithdrawals
    //     const index = this.state.invalidProposalWithdrawals.indexOf(withdrawal)
    //     this.state.invalidProposalWithdrawals.splice(index, 1)
    //   } else {
    //     if (status === GameStatus.CHALLENGER_WINS) {
    //       // Remove from state.invalidProposalWithdrawals
    //       const index =
    //         this.state.invalidProposalWithdrawals.indexOf(withdrawal)
    //       this.state.invalidProposalWithdrawals.splice(index, 1)
    //     } else if (status === GameStatus.DEFENDER_WINS) {
    //       // Set state.forgeryDetected
