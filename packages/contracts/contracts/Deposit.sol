...
contract Deposit {
   ...
   */
    ...
    */
    ...
    function executeTransaction(StateUpdate memory _stateUpdate,
                                bytes memory _transaction) private returns (StateUpdate memory) {
        return _stateUpdate;
    }
    ...
    */
    ...
    function startCheckpoint(StateUpdate memory _stateUpdate, bytes memory _inclusionProof,
                            Range memory _checkpointedRange) public {
    ...
    function challengeCheckpointOutdated(Checkpoint memory _olderCheckpoint,
                                        Checkpoint memory _newerCheckpoint) private {
        // Ensure checkpoint ranges intersect
        require(Math.max(_olderCheckpoint.checkpointedRange.start, _newerCheckpoint.checkpointedRange.start) <
                Math.min(_olderCheckpoint.checkpointedRange.end, _newerCheckpoint.checkpointedRange.end));
    ...
    function challengeExitDeprecated(Challenge memory _checkpoint,
                                    bytes memory _transaction,
                                    bytes memory _inclusionProof) private {
      // TODO
    }
