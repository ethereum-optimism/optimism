...
contract OVM_ExecutionManager is iOVM_ExecutionManager, Lib_AddressResolver {
  ...
  address constant GAS_METADATA_ADDRESS =
   ...
   */
  address constant CONTAINER_CONTRACT_PREFIX =
   ...
   */
  modifier netGasCost(uint256 _cost) {
   ...
   */
  modifier fixedGasDiscount(uint256 _discount) {
    uint256 gasProvided = gasleft();
    _;
    uint256 gasUsed = gasProvided - gasleft();
  ...
  function run(
    ...
    if (_isUpgrading() == true) {
      ...
      returndata = abi.encode(success, ret);
    } else {
   ...
   */
  function ovmREVERT(bytes memory _data) public override {
   ...
   */
  function ovmGETNONCE() external override returns (uint256 _nonce) {
  ...
  function ovmCREATEEOA(
    ...
    address eoa = ecrecover(_messageHash, _v + 27, _r, _s);
    ...
    if (eoa == address(0)) {
      ovmREVERT(
        bytes('Signature provided for EOA contract creation is invalid.')
      );
    }
  ...
  function ovmCALL(
    ...
    return ovmCALL(_gasLimit, _address, 0, _calldata);
  }
   ...
   */
  function ovmSLOAD(bytes32 _key)
    external
    override
    netGasCost(40000)
    returns (bytes32 _value)
  {
    ...
    return _getContractStorage(contractAddress, _key);
  }
    ...
    netGasCost(60000)
  {
    ...
    _putContractStorage(contractAddress, _key, _value);
  }
  ...
  function ovmEXTCODECOPY(
    ...
    uint256 _offset,
    uint256 _length
  ) public override returns (bytes memory _code) {
    return
      Lib_EthUtils.getCode(_getAccountEthAddress(_contract), _offset, _length);
  }
   ...
   */
  function ovmEXTCODESIZE(address _contract)
    public
    override
    returns (uint256 _EXTCODESIZE)
  {
    return Lib_EthUtils.getCodeSize(_getAccountEthAddress(_contract));
  }
   ...
   */
  function ovmEXTCODEHASH(address _contract)
    external
    override
    returns (bytes32 _EXTCODEHASH)
  {
    return Lib_EthUtils.getCodeHash(_getAccountEthAddress(_contract));
  }
   ...
   */
  function ovmBALANCE(address _contract)
    public
    override
   ...
   */
  function ovmSELFBALANCE() external override returns (uint256 _BALANCE) {
  ...
  function _handleExternalMessage(
    ...
    if (messageValue > 0 && _isValueType(_messageType)) {
    ...
    if (_isCreateType(_messageType)) {
      ...
      (success, returndata) = address(this).call{gas: _gasLimit}(
        abi.encodeWithSelector(this.safeCREATE.selector, _data, _contract)
      );
    } else {
    ...
    messageRecord.nuisanceGasLeft =
      prevNuisanceGasLeft -
      (nuisanceGasLimit - nuisanceGasLeft);
   ...
   */
  function _attemptForcedEthTransfer(address _to, uint256 _value)
    internal
   ...
   */
  function _hasEmptyAccount(address _address) internal returns (bool _exists) {
    _checkAccountLoad(_address);
   ...
   */
  function _setAccountNonce(address _address, uint256 _nonce) internal {
   ...
   */
  function _getAccountNonce(address _address)
    internal
   ...
   */
  function _getAccountEthAddress(address _address)
    internal
    returns (address _ethAddress)
  {
    _checkAccountLoad(_address);
   ...
   */
  function _initPendingAccount(address _address) internal {
   ...
   */
  function _getContractStorage(address _contract, bytes32 _key)
    internal
    returns (bytes32 _value)
  {
   ...
   */
  function _checkAccountChange(address _address) internal {
   ...
   */
  function _checkContractStorageLoad(address _contract, bytes32 _key) internal {
   ...
   */
  function _encodeRevertData(RevertFlag _flag, bytes memory _data)
    internal
    view
    returns (bytes memory _revertdata)
  {
    ...
    if (_flag == RevertFlag.INVALID_STATE_ACCESS) {
      return abi.encode(_flag, 0, 0, bytes(''));
    }
    ...
    )
  {
    ...
    if (_revertdata.length == 0) {
      return (RevertFlag.OUT_OF_GAS, 0, 0, bytes(''));
    }
   ...
   */
  function _revertWithFlag(RevertFlag _flag, bytes memory _data) internal view {
    bytes memory revertdata = _encodeRevertData(_flag, _data);
   ...
   * @param _flag Flag to revert with.
   */
  function _revertWithFlag(RevertFlag _flag) internal {
   ...
   */
  function _useNuisanceGas(uint256 _amount) internal {
   ...
   */
  function _checkNeedsNewEpoch(uint256 _timestamp) internal {
    if (
      _timestamp >=
      (_getGasMetadata(GasMetadataKey.CURRENT_EPOCH_START_TIMESTAMP) +
        gasMeterConfig.secondsPerEpoch)
    ) {
      _putGasMetadata(GasMetadataKey.CURRENT_EPOCH_START_TIMESTAMP, _timestamp);

      _putGasMetadata(
        GasMetadataKey.PREV_EPOCH_SEQUENCER_QUEUE_GAS,
        _getGasMetadata(GasMetadataKey.CUMULATIVE_SEQUENCER_QUEUE_GAS)
      );

      _putGasMetadata(
        GasMetadataKey.PREV_EPOCH_L1TOL2_QUEUE_GAS,
        _getGasMetadata(GasMetadataKey.CUMULATIVE_L1TOL2_QUEUE_GAS)
      );
    }
  }
   ...
   */
  function _isValidInput(Lib_OVMCodec.Transaction memory _transaction)
    internal
    view
    returns (bool)
  {
   ...
   */
  function _getGasMetadata(GasMetadataKey _key)
    internal
    returns (uint256 _value)
  {
    return
      uint256(
        _getContractStorage(GAS_METADATA_ADDRESS, bytes32(uint256(_key)))
      );
  }
   ...
   */
  function _putGasMetadata(GasMetadataKey _key, uint256 _value) internal {
    _putContractStorage(
  ...
  function _isUpgrading() internal returns (bool) {
    return
      uint256(
        _getContractStorage(
          0x420000000000000000000000000000000000000D,
   ...
   */
  function _initContext(Lib_OVMCodec.Transaction memory _transaction) internal {
   ...
   */
  function ovmSETCODE(address _address, bytes memory _code) external override {
    // TODO: IMPLEMENT ME
  }
   ...
   */
  function _isCreateType(MessageType _messageType)
    internal
    pure
    returns (bool)
  {
    return (_messageType == MessageType.ovmCREATE ||
   ...
   * @param _messageType the message type in question.
   */
  function _isValueType(MessageType _messageType) internal pure returns (bool) {
    // ovmSTATICCALL and ovmDELEGATECALL types do not accept or transfer value.
    return (_messageType == MessageType.ovmCALL ||
  ...
  function simulateMessage(
    ...
    _initContext(_transaction);
    messageRecord.nuisanceGasLeft = uint256(-1);
