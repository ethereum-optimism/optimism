...
library EthMerkleTrie {
    ...
    bytes32 constant private RLP_NULL_HASH = 0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421;
        ...
        returns (bytes32)
    {
        ...
        if (accountState.storageRoot == RLP_NULL_HASH) {
            ...
            accountState.storageRoot = SecureMerkleTrie.getSingleNodeRootHash(
                abi.encodePacked(_key),
                abi.encodePacked(_value)
            );
        } else {
            // Go into the trie and update the slot.
            accountState.storageRoot = SecureMerkleTrie.update(
                abi.encodePacked(_key),
                abi.encodePacked(_value),
                _storageTrieWitness,
                accountState.storageRoot
        ...
        returns (bytes32)
    {
        ...
        return SecureMerkleTrie.update(
            abi.encodePacked(_address),
            encodedAccountState,
            _stateTrieWitness,
            _stateTrieRoot
