  ...
  describe.only('Deposit with Ownership', () => {
    ...
    describe('Commitment Contract', () => {
      it('correctly calculates the parent of two state subtree nodes', async () => {
        const leftSibling = {
            hashValue: '0x1111111111111111111111111111111111111111111111111111111111111111',
            start: '0x00'
        }
        const rightSibling = {
            hashValue: '0x1111111111111111111111111111111111111111111111111111111111111111',
        ...
        const abiPacked = await commitmentContract.getAbiPacked(leftSibling, rightSibling)
        console.log('abipacked from solidity is: ', abiPacked)
        const contractParent = await commitmentContract.stateSubtreeParent(leftSibling, rightSibling)
        console.log('contract parent is: ', contractParent)

        const tsLeftSibling = new MerkleIntervalTreeNode(Buffer.from(leftSibling.hashValue.slice(2), 'hex'), new BigNum(leftSibling.start.slice(2), "hex").toBuffer('be', 16))
        console.log('typescript left sibling is ', tsLeftSibling)
        const tsRightSibling = new MerkleIntervalTreeNode(Buffer.from(rightSibling.hashValue.slice(2), 'hex'), new BigNum(rightSibling.start.slice(2), "hex").toBuffer('be', 16))
