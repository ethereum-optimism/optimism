...
import { hexStrToNumber, numberToHexString, hexStrToBuf } from '@eth-optimism/core-utils';
...
describe('GasConsumer', () => {
  ...
  const getTxCalldataGasCostForConsumeGas = (toConsume: number): number => {
    const expectedCalldata: Buffer = hexStrToBuf(GasConsumer.interface.encodeFunctionData('consumeGasExact', [toConsume]))
  ...
  describe('Precise gas Consumption', async () => {
    for (const toConsume of [1_000,10_000,20_123,100_000,200_069]) {
      it(`should properly consume ${toConsume} gas`, async () => {
        const tx = await gasConsumer.consumeGasExact(toConsume)
        const receipt = await gasConsumer.provider.getTransactionReceipt(tx.hash)
