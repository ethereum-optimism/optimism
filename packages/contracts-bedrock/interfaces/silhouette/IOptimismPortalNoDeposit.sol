...
// The ABI of `src/silhouette/OptimismPortalNoDeposit.sol`: `IOptimismPortal2` plus one error, because
// the gate is a BEHAVIOUR-only override -- `depositTransaction` keeps its selector and its signature
// and reverts. Nothing is added to the surface and nothing is taken away, which is the property the
// rest of P's L1 stack depends on and which this file makes checkable.
...
interface IOptimismPortalNoDeposit is IProxyAdminOwnedBase {
