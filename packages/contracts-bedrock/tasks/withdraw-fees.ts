...
task('withdraw-fees')
  ...
  .setAction(async (args) => {
    ...
    if (!args.useLedger) {
      if (!args.privateKey) {
        throw new Error('Must pass --private-key when not using Ledger')
      }
      signer = new ethers.Wallet(args.privateKey).connect(provider)
    } else {
