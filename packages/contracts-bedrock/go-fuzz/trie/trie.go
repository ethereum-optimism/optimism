...
func FuzzTrie(variant string) {
	...
	case prefixedValidKey:
		...
		// Ensure that the random prefix always contains at least one non-zero byte
		randNonZeroBytes(&bytes)
...
// Randomizes an array of bytes and ensures that it never contains all zero values.
func randNonZeroBytes(bytes *[]byte) {
	zero := true
	for _, v := range *bytes {
		if v != 0x00 {
			zero = false
	...
	// If the random bytes arr contains all zero bytes, re-randomize it.
	if zero {
		rand.Read(*bytes)
		randNonZeroBytes(bytes)
	}
