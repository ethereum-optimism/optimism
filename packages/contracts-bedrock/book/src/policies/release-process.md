...
Since `op-node` and `kona` are typically released from the trunk (`develop`) branch, and `op-deployer` is typically released from a `proposal/op-contracts/vX.0.0` branch,
special effort is required to keep the L2 contract versions in sync between the two branches. In the event that a change is made which modifies the
