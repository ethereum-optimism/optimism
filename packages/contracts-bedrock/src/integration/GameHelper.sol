...
contract GameHelper {
    ...
    function performMoves(
        IFaultDisputeGame _game,
        Move[] calldata _moves
    ) public payable {
        uint256 movesLen = _moves.length;
        for (uint256 i = 0; i < movesLen; i++) {
            Move memory move = _moves[i];
            (, , , , Claim pClaim, Position pPosition, ) = _game.claimData(
                move.parentIdx
            );
            uint256 requiredBond = _game.getRequiredBond(
                pPosition.move(move.attack)
            );
            _game.move{value: requiredBond}(
                pClaim,
                move.parentIdx,
                move.claim,
                move.attack
    ...
    function createGameWithClaims(
        ...
        IFaultDisputeGame game = IFaultDisputeGame(
            address(
                _dgf.create{value: initBond}(_gameType, _rootClaim, _extraData)
            )
        );
