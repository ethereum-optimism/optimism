...
abstract contract OPContractsManagerBase {
    ...
    function getImplementations()
        internal
        view
        returns (IOPContractsManagerContractsContainer.Implementations memory)
    {
        return thisOPCM.implementations();
    }

    /// @notice Retrieves the blueprint addresses stored in this OPCM contract
    function getBlueprints() internal view returns (IOPContractsManagerContractsContainer.Blueprints memory) {
    ...
    function _registerPermissionedGameV2(
        ...
        IOPContractsManagerContractsContainer.Implementations memory _implementation,
        ...
        for (uint256 i = 0; i < _gameConfigs.length; i++) {
            ...
            if (
                ...
                // Separate context to avoid stack too deep.
                {
                    ...
                    IOPContractsManagerContractsContainer.Blueprints memory bps = getBlueprints();
...
contract OPContractsManagerUpgrader is OPContractsManagerBase {
    ...
    function _doChainUpgrade(
        IOPContractsManagerContractsContainer.Implementations memory _impls,
...
contract OPContractsManagerDeployer is OPContractsManagerBase {
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        ...
        IOPContractsManagerContractsContainer.Blueprints memory blueprint = getBlueprints();
        IOPContractsManagerContractsContainer.Implementations memory implementation = getImplementations();
...
contract OPContractsManager is ISemver {
    ...
    IOPContractsManagerV2 public immutable opcmV2;
    ...
    function deploy(DeployInput calldata _input) external virtual returns (DeployOutput memory) {
        ...
        if (isDevFeatureEnabled(DevFeatures.OPCM_V2)) {
            ...
            IOPContractsManagerV2.FullConfig memory cfg = _toFullConfig(_input, superchainConfig);
    ...
    function upgrade(OpChainConfig[] memory _opChainConfigs) external virtual {
        ...
        if (isDevFeatureEnabled(DevFeatures.OPCM_V2)) {
            for (uint256 i = 0; i < _opChainConfigs.length; i++) {
                IOPContractsManagerV2.UpgradeInput memory upgradeInput = _toUpgradeInput(_opChainConfigs[i]);
                bytes memory data = abi.encodeCall(IOPContractsManagerV2.upgrade, (upgradeInput));
        ...
        returns (IOPContractsManagerV2.FullConfig memory)
    {
        ...
        cfg.disputeGameConfigs = new IOPContractsManagerV2.DisputeGameConfig[](3);
        cfg.disputeGameConfigs[0] = IOPContractsManagerV2.DisputeGameConfig({
        ...
        cfg.disputeGameConfigs[2] = IOPContractsManagerV2.DisputeGameConfig({
        ...
        returns (IOPContractsManagerV2.UpgradeInput memory)
    {
        ...
        disputeGameConfigs[2] = IOPContractsManagerV2.DisputeGameConfig({
            ...
            enabled: cannonKonaPrestate.raw() != bytes32(0),
            initBond: ckgBond,
            gameType: GameTypes.CANNON_KONA,
    ...
    function _toDeployOutput(IOPContractsManagerV2.ChainContracts memory _cts)
        internal
        view
