...
library MIPSMemory {
    ...
    function readMem(bytes32 _memRoot, uint64 _addr, uint256 _proofOffset) internal pure returns (uint32 out_) {
        unchecked {
    ...
    /// @notice Reads a 64-bit value from memory.
    /// @param _memRoot The current memory root
    /// @param _addr The address to read from.
    /// @param _proofOffset The offset of the memory proof in calldata.
    /// @return out_ The hashed MIPS state.
    function readMemDoubleword(
        bytes32 _memRoot,
        uint64 _addr,
        ...
        returns (uint64 out_)
    {
        unchecked {
            ...
            assembly {
                ...
                out_ := and(shr(shamt, leaf), 0xFFffFFffFFffFFff)
            }
        }
    }

    /// @notice Writes a 64-bit value to memory.
        ...
        returns (bytes32 newMemRoot_)
    {
        unchecked {
            ...
            assembly {
                ...
                leaf := or(and(leaf, not(shl(shamt, 0xFFffFFffFFffFFff))), shl(shamt, _val))
                _proofOffset := add(_proofOffset, 32)
    ...
    function memoryProofOffset(uint256 _proofDataOffset, uint8 _proofIndex) internal pure returns (uint256 offset_) {
        unchecked {
            ...
            offset_ = _proofDataOffset + (uint256(_proofIndex) * (60 * 32));
            return offset_;
        }
    }
    ...
    function validateMemoryProofAvailability(uint256 _proofStartOffset) internal pure {
        unchecked {
            ...
            // A memory proof consists of 60 bytes32 values - verify we have enough calldata
            require(s >= (_proofStartOffset + 60 * 32), "check that there is enough calldata");
        }
    }
}
