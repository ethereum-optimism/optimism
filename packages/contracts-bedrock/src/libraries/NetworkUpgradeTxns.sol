...
library NetworkUpgradeTxns {
    ...
    /// @notice The bundle schema version this library writes.
    string internal constant BUNDLE_VERSION = "1.1.0";

    /// @notice The bundle schema version that predates the `extraGas` field. Still accepted when
    ///         reading, because a bundle locked at this version is regenerated by the generator of
    ///         its own locked commit, which cannot emit anything newer.
    string internal constant BUNDLE_VERSION_NO_EXTRA_GAS = "1.0.0";

    /// @notice Metadata for the Network Upgrade Transaction bundle.
    /// @param extraGas Gas added to the activation block's gas limit on top of the sum of this
    ///                 bundle's own transactions, covering activation deposits the consensus layer
    ///                 emits outside the bundle. Zero for a bundle that needs none.
        ...
        internal
    {
        ...
        vm.serializeUint(metadataKey, "extraGas", uint256(_metadata.extraGas));
    ...
    /// @notice Reverts unless the bundle declares a schema version this library understands, and
    ///         unless its `extraGas` usage matches that version.
    /// @param _json The raw bundle JSON.
    function requireSupportedVersion(string memory _json) internal view {
        string memory version = abi.decode(vm.parseJson(_json, ".metadata.version"), (string));

        if (LibString.eq(version, BUNDLE_VERSION_NO_EXTRA_GAS)) {
            // Absent reads as zero, matching how op-node and kona decode the field.
            uint256 extraGas = vm.keyExistsJson(_json, ".metadata.extraGas") ? _json.readUint(".metadata.extraGas") : 0;
            require(extraGas == 0, "NetworkUpgradeTxns: extraGas requires bundle version 1.1.0");
            return;
        }
