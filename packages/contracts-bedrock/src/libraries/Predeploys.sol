...
library Predeploys {
    ...
    /// @notice Selects which implementation variant of a proxy applies to a chain configuration.
    ///         Doubles as the index into `PredeployRecord.variants`: index DEFAULT is always
    ///         populated; CGT is populated only for proxies that swap implementation on custom
    ///         gas token chains. New kinds must be appended to keep existing arrays gap-free.
    enum VariantKind {
        DEFAULT,
        CGT
    }

    /// @notice A single implementation that can sit behind a predeploy proxy.
    ...
    /// @notice Configuration record for a single predeploy proxy. One record per 0x4200... address.
    /// @param proxy             Canonical proxy address (0x4200...).
    /// @param variants          Implementations behind this proxy, indexed by VariantKind. Index
    ///                          DEFAULT is always present; CGT is present only when the proxy swaps
    ///                          implementation on custom gas token chains (e.g. L1Block/L1BlockCGT).
    ...
    /// @notice Builds a single-variant array (DEFAULT only).
    function _variants(
        string memory _name,
    ...
    /// @notice Builds a DEFAULT + CGT variant array.
    function _variants(
        string memory _name,
        string memory _artifactPath,
        uint64 _deployGasLimit,
    ...
    /// @notice Returns the implementation variant of a record for the given gas token config.
    ///         Falls back to DEFAULT when the proxy has no variant for the requested config.
    ...
    ///         Always returns the DEFAULT implementation name for a given proxy address.
