...
const deployFn: DeployFunction = async (hre) => {
  ...
  if (
    ...
    if (isLiveDeployer) {
      ...
      const tx = await AddressManager.populateTransaction.transferOwnership(SystemDictator.address)
  ...
  if (
    ...
    if (isLiveDeployer) {
      ...
      const tx = await L1CrossDomainMessenger.populateTransaction.transferOwnership(SystemDictator.address)
  ...
  if (
    ...
    if (isLiveDeployer) {
      ...
      const tx = await L1StandardBridgeProxy.populateTransaction.setOwner(SystemDictator.address)
      console.log(`Please transfer L1StandardBridge (proxy) owner to MSD`)
      console.log(`L1StandardBridgeProxy address: ${L1StandardBridgeProxy.address}`)
      console.log(`MSD address: ${SystemDictator.address}`)
      console.log(`Calldata:`)
      console.log(tx.data)
    }
  ...
  if (
    ...
    if (isLiveDeployer) {
      ...
      const tx = await L1ERC721BridgeProxy.populateTransaction.changeAdmin(SystemDictator.address)
