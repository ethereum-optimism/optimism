...
contract ProxyAdmin_Test is Test {
    ...
    function test_chugsplashGetProxyImplementation() external {
        getProxyImplementation(address(chugsplash));
    }

    function test_delegateResolvedGetProxyImplementation() external {
        getProxyImplementation(address(resolved));
    }

    function getProxyImplementation(address _proxy) internal {
        {
    ...
    function test_erc1967Upgrade() external {
        upgrade(address(proxy));
    }

    function test_chugsplashUpgrade() external {
        upgrade(address(chugsplash));
    }

    function test_delegateResolvedUpgrade() external {
        upgrade(address(resolved));
    }

    function upgrade(address _proxy) internal {
        vm.prank(alice);
        admin.upgrade(_proxy, address(implementation));
    ...
    function test_erc1967UpgradeAndCall() external {
        upgradeAndCall(address(proxy));
    }

    function test_chugsplashUpgradeAndCall() external {
        upgradeAndCall(address(chugsplash));
    }

    function test_delegateResolvedUpgradeAndCall() external {
        upgradeAndCall(address(resolved));
    }

    function upgradeAndCall(address _proxy) internal {
        vm.prank(alice);
    ...
    function test_onlyOwner() external {
        vm.expectRevert("UNAUTHORIZED");
        admin.changeProxyAdmin(address(proxy), address(0));

        vm.expectRevert("UNAUTHORIZED");
        admin.upgrade(address(proxy), address(implementation));

        vm.expectRevert("UNAUTHORIZED");
        admin.upgradeAndCall(address(proxy), address(implementation), hex"");
    }
