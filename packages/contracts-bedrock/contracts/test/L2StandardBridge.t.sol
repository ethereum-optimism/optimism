...
contract PreBridgeERC20 is Bridge_Initializer {
    ...
    function _preBridgeERC20(bool isLegacy) internal {
        ...
        bytes memory message = abi.encodeWithSelector(
            StandardBridge.finalizeBridgeERC20.selector, address(L1Token), address(L2Token), alice, alice, 100, hex""
        );
        ...
        if (isLegacy) {
            vm.expectCall(
                address(L2Bridge),
                abi.encodeWithSelector(L2Bridge.withdraw.selector, address(L2Token), 100, 1000, hex"")
            );
        } else {
            vm.expectCall(
                address(L2Bridge),
                abi.encodeWithSelector(
                    L2Bridge.bridgeERC20.selector, address(L2Token), address(L1Token), 100, 1000, hex""
                )
            );
        }
...
contract PreBridgeERC20To is Bridge_Initializer {
    ...
    function _preBridgeERC20To(bool isLegacy) internal {
        ...
        uint256 nonce = L2Messenger.messageNonce();
        bytes memory message = abi.encodeWithSelector(
            StandardBridge.finalizeBridgeERC20.selector, address(L1Token), address(L2Token), alice, bob, 100, hex""
        );
        uint64 baseGas = L2Messenger.baseGas(message, 1000);
        bytes memory withdrawalData = abi.encodeWithSelector(
            CrossDomainMessenger.relayMessage.selector, nonce, address(L2Bridge), address(L1Bridge), 0, 1000, message
        ...
        if (isLegacy) {
            vm.expectCall(
                address(L2Bridge),
                abi.encodeWithSelector(L2Bridge.withdrawTo.selector, address(L2Token), bob, 100, 1000, hex"")
            );
        } else {
            vm.expectCall(
                address(L2Bridge),
                abi.encodeWithSelector(
                    L2Bridge.bridgeERC20To.selector, address(L2Token), address(L1Token), bob, 100, 1000, hex""
                )
            );
        }
