...
contract MerkleTrie_Test is CommonTest {
    ...
    function test_get_validProofSucceeds1() external {
        ...
        (bool exists, bytes memory ret) = MerkleTrie.get(key, proof, root);
        assertTrue(exists);
        assertEq(ret, val);
    }
    ...
    function test_get_reverts_nonexistentKey1() external {
        ...
        vm.expectRevert("MerkleTrie: invalid key length for leaf or extension node");
    ...
    /// @dev The `MerkleTrie` verifier allows for malleable proofs. This is a known
    ///      behavior of our MPT implementation, and this test exists to assert that.
    function test_get_extraProofElements_success() external {
        bytes32 root = 0x278c88eb59beba4f8b94f940c41614bb0dd80c305859ebffcd6ce07c93ca3749;
        bytes memory key = hex"aa";
    ...
    function testFuzz_get_validProofs_success(bytes4) external {
        ...
        (bool b, bytes memory ret) = MerkleTrie.get(key, proof, root);
        assertTrue(b);
        assertEq(val, ret);
    }
    ...
    /// @dev The `MerkleTrie` verifier allows for malleable proofs. This is a known
    ///      behavior of our MPT implementation, and this test exists to assert that.
    function testFuzz_get_extraProofElements_success(bytes4) external {
        ...
        (bytes32 root, bytes memory key, bytes memory val, bytes[] memory proof) = ffi
            .getMerkleTrieFuzzCase("extra_proof_elems");
    ...
    function testFuzz_get_partialProof_reverts(bytes4) external {
        ...
        vm.expectRevert("MerkleTrie: provided proof is invalid");
        MerkleTrie.get(key, proof, root);
    }
}
