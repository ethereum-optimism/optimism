...
abstract contract StandardBridge is Initializable {
    ...
    modifier onlyOtherBridge() {
        require(
            ...
            "StandardBridge: could not authenticate bridge message"
        );
        _;
    }
    ...
    function finalizeBridgeETH(
        ...
        require(
            msg.value == _amount,
            "StandardBridge: amount sent does not match amount required in the bridge transaction"
        );

        require(
            _to != address(this),
            "StandardBridge: cannot send ETH to self for security reasons"
        );
    ...
    function completeOutboundTransfer(
        ...
        require(
            _localToken != address(this),
            "StandardBridge: local token cannot be self for security reasons"
        );

        if (_isOptimismMintableERC20(_localToken)) {
            require(
                _isCorrectTokenPair(_localToken, _remoteToken),
                "StandardBridge: wrong remote token for OptimismMintableERC20 local token"
            );
