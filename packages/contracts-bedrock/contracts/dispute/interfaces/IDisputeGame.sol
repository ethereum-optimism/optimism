...
interface IDisputeGame is IInitializable {
    ...
    /// @notice Getter for the length of the `extraData`.
    /// @dev `clones-with-immutable-args` argument #2
    /// @return extraDataLen_ The length of the `extraData`.
    function extraDataLen() external pure returns (uint64 extraDataLen_);

    /// @notice Getter for the extra data.
    /// @dev `clones-with-immutable-args` argument #3 (dynamically sized [`extraDataLen`])
