...
- `CrossL2Inbox` exports a locally proven event through the standard L2 cross-domain messenger, imports an L1 certificate, and validates
...
3. `CrossL2Inbox` sends a zero-value cross-domain message targeting `L1EventRegistry.registerEvent`;
   the L2 messenger creates the withdrawal through the standard message passer.
4. Anyone proves and finalizes that withdrawal using the ordinary portal mechanism.
5. `L1EventRegistry` authenticates the calling L1 messenger and its `xDomainMessageSender()` as the source
   `CrossL2Inbox`, then checks the associated portal, ETHLockbox authorization and source chain ID.
   Failed registration can be retried through the standard L1 messenger.
...
- Calldata cannot forge source identity. L1 authenticates the calling messenger, its cross-domain sender
  and associated portal configuration.
