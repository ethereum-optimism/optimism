...
pattern = re.compile(
    rb"\x5b\x83\x81\x10\x15\x61(..)\x57"
    rb"\x81\x81\x01\x51\x83\x82\x01\x52\x60\x20\x01\x61(..)\x56",
    re.DOTALL,
)
matches = [
    match for match in pattern.finditer(code)
...
print(json.dumps({"artifact": str(args.artifact), "claim": str(args.output),
                  "head": head, "exit": end, "runtimeSha256": hashlib.sha256(code).hexdigest()}))
