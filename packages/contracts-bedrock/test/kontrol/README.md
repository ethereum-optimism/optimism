...
└── <a href="./scripts">scripts</a>: Where the scripts of the projects live
    ├── <a href="./scripts/json">json</a>: Data cleaning scripts for the output of KontrolDeployment.sol
    ├── <a href="./scripts/make-summary-deployment.sh">make-summary-deployment.sh</a>: Executes KontrolDeployment.sol, curates the result and writes the summary deployment contract
...
`KontrolDeployment.sol` contains the deployment sequence required by the proofs.
...
The summary contract can be found in [`DeploymentSummary.sol`](./proofs/utils/DeploymentSummary.sol), which summarizes the state changes of the `KontrolDeployment.sol` contract.

### Execute Proofs

Use the [`run-kontrol.sh`](./scripts/run-kontrol.sh) script to run the proofs in all `*.k.sol` files.
...
The `runKontrolDeployment` function of `KontrolDeployment` reproduces the deployment process laid out in the `_run` function of [`Deploy.s.sol`](../../scripts/deploy/Deploy.s.sol). `runKontrolDeployment` has the `stateDiff` modifier to make use of [Foundry's state diff cheatcodes](https://book.getfoundry.sh/cheatcodes/start-state-diff-recording). Kontrol utilizes the JSON resulting from this modifier for two purposes:
...
- `kdeploy`: Used by [`make-summary-deployment.sh`](./scripts/make-summary-deployment.sh) to generate the [`DeploymentSummary.sol`](./proofs/utils/DeploymentSummary.sol) contract based on execution of the `KontrolDeployment.sol` contract using Foundry's state diff recording cheatcodes.
...
The `make-summary-deployment.sh` script writes the generated JSON state diff to `snapshots/state-diff/Kontrol-Deploy.json`.
Run `just kontrol-summary-full` to regenerate both default and fault proof summary contracts.
The `kontrol-summaries` check in `checks.yaml` verifies that the committed summary contracts remain unchanged unless their expected hashes are updated.
We commit the `DeploymentSummary.sol` and `DeploymentSummaryCode.sol` contracts because forge fails to build if those contracts are not present; it is simpler to commit these autogenerated files than to work around their absence in forge builds.

### Kontrol Proof Execution in CircleCI
...
```bash
mise exec -- just build-go-ffi kontrol-summary-full test-kontrol-no-build
```

Set `KONTROL_WORKERS` to a positive integer to limit concurrency; the default is 16.
The CircleCI job retains available proof archives, container logs, and `kontrol_prove_report.xml` from `test/kontrol/logs` as artifacts, and publishes the JUnit report as test results.
Open the job's Artifacts tab to download proof output for debugging.
