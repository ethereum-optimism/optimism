...
On each execution a status is returned to the commit checks the execution was run from. The checks for a commit provide a "Details" link to the execution KCFG artifacts and Kontrol proof summary results. See below for further information on how to fetch these artifacts from the summary using the GitHub API.
...
To fetch the artifacts of the execution from KaaS, you can either manually navigate to the summary page using the "Details" link or use the GitHub API to pull the artifacts.
