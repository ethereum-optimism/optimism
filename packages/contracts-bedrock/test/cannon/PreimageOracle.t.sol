...
contract PreimageOracle_LargePreimageProposals_Test is Test {
    ...
    /// @notice Sets up the testing suite.
    function setUp() public {
        oracle = new PreimageOracle({ _minProposalSize: MIN_SIZE_BYTES, _challengePeriod: CHALLENGE_PERIOD, _cancunActivation: 0 });
        vm.label(address(oracle), "PreimageOracle");
    ...
    function test_initLPP_sizeTooSmall_reverts() public {
        oracle = new PreimageOracle({ _minProposalSize: 1000, _challengePeriod: CHALLENGE_PERIOD, _cancunActivation: 0 });
