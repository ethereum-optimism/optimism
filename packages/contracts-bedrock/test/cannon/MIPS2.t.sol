...
//         bytes32 newRoot = keccak256(abi.encodePacked(stack[stack.length - 1].root,
// keccak256(encodeThread(_thread))));
...
//         expect.rightThreadStack = keccak256(abi.encodePacked(EMPTY_THREAD_ROOT,
// keccak256(encodeThread(expectThread))));
...
//         expect.leftThreadStack = keccak256(abi.encodePacked(EMPTY_THREAD_ROOT,
// keccak256(encodeThread(expectThread))));
//
//         bytes32 postState = mips.step(encodeState(state), bytes.concat(threadWitness, memProof), 0);
//         assertEq(postState, outputState(expect), "unexpected post state");
...
//         expect.stepsSinceLastContextSwitch = 0;
//         bytes32 innerThreadRoot = keccak256(abi.encodePacked(EMPTY_THREAD_ROOT,
// keccak256(encodeThread(expectThread))));
...
//     }
//
//     /// @dev Static unit test asserting VM behavior when the current thread has exited and the current thread stack
// is
//     /// almost empty
...
//             _state.rightThreadStack = keccak256(abi.encodePacked(EMPTY_THREAD_ROOT,
// keccak256(encodeThread(_thread))));
//         } else {
//             _state.leftThreadStack = keccak256(abi.encodePacked(EMPTY_THREAD_ROOT,
// keccak256(encodeThread(_thread))));
//         }
//     }
//
