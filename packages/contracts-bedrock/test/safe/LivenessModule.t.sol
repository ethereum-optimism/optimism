...
contract LivenessModule_TestInit is Test, SafeTestTools {
    ...
    /// @notice Set the current time to after the liveness interval
    function _warpPastLivenessInterval() internal {
        vm.warp(INIT_TIME + LIVENESS_INTERVAL + 1);
    }

    /// @notice Helper function to calculate the required threshold for a given number of owners
    ///         and threshold percentage. This does a lot of extra work to set up a dummy
    ///         LivenessModule and Safe, but helps to keep the tests clean.
    function _getRequiredThreshold(uint256 _numOwners, uint256 _thresholdPercentage) internal returns (uint256) {
        ...
        // This module is only used to calculate the required threshold for the Safe. Its state
        // will not be modified after it is created so the constructor arguments are not important.
...
/// @title LivenessModule_Constructor_Test
/// @notice Tests the constructor of the `LivenessModule` contract.
contract LivenessModule_Constructor_Test is LivenessModule_TestInit {
    /// @notice Tests that the constructor fails if the minOwners is greater than the number of
    ///         owners.
...
/// @title LivenessModule_GetRequiredThreshold_Test
/// @notice Tests the `getRequiredThreshold` function of the `LivenessModule` contract.
contract LivenessModule_GetRequiredThreshold_Test is LivenessModule_TestInit {
        ...
        returns (uint256)
    {
        ...
        // If the total multiplied by the percentage is not divisible by 100, we need to add 1 to
        // the result to compensate for the rounding down by integer division.
    ...
    /// @notice Differentially tests the getRequiredThreshold function against
    ///         _getLeastIntegerValueAbovePercentage.
    ...
    /// @notice Check the return values of the getRequiredThreshold function against the boundary
    ///         conditions of 1 and 100 percent.
    ...
    /// @notice Check the return values of the getRequiredThreshold function against manually
    ///         calculated values.
...
/// @title LivenessModule_CanRemove_Test
/// @notice Tests the `canRemove` function of the `LivenessModule` contract.
contract LivenessModule_CanRemove_Test is LivenessModule_TestInit {
    /// @notice Tests if canRemove work correctly
...
/// @title LivenessModule_RemoveOwners_Test
/// @notice Tests the `removeOwners` function of the `LivenessModule` contract.
contract LivenessModule_RemoveOwners_Test is LivenessModule_TestInit {
    using SafeTestLib for SafeInstance;

    /// @notice We put this array in storage so that we can more easily populate it using push in
    ///         the fuzz tests.
    address[] ownersToRemove_Fuzz;

    /// @notice Options for handling the event that the number of owners remaining is less than
    ///         minOwners.
    ...
    /// @notice Extracts the setup of the test environment into a separate function for fuzz test.
    function _prepare(
        uint256 _numOwners,
        uint256 _minOwners,
    ...
    {
        // First we modify the test parameters to ensure that they describe a plausible starting
        // point.
        //
        // _numOwners must be at least 4, so that _minOwners can be set to at least 3 by the
        // following bound() call.
        // Limiting the owner set to 20 helps to keep the runtime of the test reasonable.
        numOwners_ = bound(_numOwners, 4, 20);
        // _minOwners must be at least 3, otherwise we don't have any range below _minOwners in
        // which to test all of the ShutDownBehavior options.
        minOwners_ = bound(_minOwners, 3, numOwners_ - 1);

        // Ensure that _numLiveOwners is less than _numOwners so that we can remove at least one
        // owner.
        ...
        // The above bounds are a bit tricky, so we assert that the resulting parameters enable us
        // to test all possible success and revert cases in the removeOwners function.
        ...
        // Create a Safe with _numOwners owners for fuzz test
    ...
    /// @notice Tests if removing one owner works correctly
    ...
    /// @notice Tests if removing all owners works correctly
    ...
    /// @notice Tests with different length owner arrays
    ...
    /// @notice Test removing an owner which has recently signed a transaction
    ...
    /// @notice Test removing an owner which has recently called showLiveness
    ...
    /// @notice Test removing an owner with an incorrect previous owner
    function test_removeOwners_wrongPreviousOwner_reverts() external {
        address[] memory prevOwners = new address[](1);
        address[] memory ownersToRemove = new address[](1);
        ownersToRemove[0] = safeInstance.owners[0];
        // Incorrectly set the previous owner as equal to the owner to remove, which will cause the
        // Safe to revert.
    ...
    /// @notice Tests if remove owners reverts if it removes too many owners without removing all
    ///         of them.
    ...
    /// @notice Tests if remove owners reverts if it removes too many owners transferring to the
    ///         shutDown owner.
    ...
    /// @notice Tests if remove owners reverts if the current Safe.guard does not match the
    ///         expected livenessGuard address.
    ...
    /// @notice Tests if removing owners works correctly for various safe configurations and
    ///         numbers of live owners.
        ...
        external
    {
        ...
        if (numLiveOwners >= minOwners) {
            // The safe will remain above the minimum number of owners, so we can remove only those
            // owners which are not live.
            ...
            // We need to test all of the possible ShutDownBehavior options, so we'll create a
            // ShutDownBehavior enum from the _shutDownBehavior input.
            ...
            if (shutDownBehavior == ShutDownBehavior.Correct) {
                ...
                // But we need to remove the non-live owners first, so we reverse the owners array,
                // since the first owners in the array were the ones to call showLiveness.
                for (uint256 i; i < numOwners; i++) {
                    ownersToRemove_Fuzz.push(safeInstance.owners[numOwners - i - 1]);
                }
                prevOwners = safeInstance.getPrevOwners(ownersToRemove_Fuzz);
                livenessModule.removeOwners(prevOwners, ownersToRemove_Fuzz);
                ...
                // For both of the incorrect behaviors, we need to calculate the number of owners
                // to remove to trigger that behavior. We initialize that value here then set it in
                // the if statements below.
                uint256 numOwnersToRemoveinShutDown;
                if (shutDownBehavior == ShutDownBehavior.DoesNotRemoveAllOwners) {
                    // In the DoesNotRemoveAllOwners case, we should have more than 1 of the
                    // pre-existing owners remaining
                    ...
                    for (uint256 i; i < numOwnersToRemoveinShutDown; i++) {
                        ...
                        if (i < nonLiveOwners.length) {
                            ownersToRemove_Fuzz.push(nonLiveOwners[i]);
                        } else {
                            // Then add live owners to remove
                            ownersToRemove_Fuzz.push(liveOwners[i - nonLiveOwners.length]);
                        }
                    }
                    prevOwners = safeInstance.getPrevOwners(ownersToRemove_Fuzz);
                    vm.expectRevert(
                    ...
                    // In the DoesNotRemoveAllOwners case, we should have exactly 1 pre-existing
                    // owners remaining
