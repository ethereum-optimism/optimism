...
contract DelayedWETH_Unlock_Test is DelayedWETH_TestInit {
    ...
    function testFuzz_unlock_once_succeeds(address _guy, uint256 _wad) public {
        delayedWeth.unlock(_guy, _wad);
        (uint256 amount, uint256 timestamp) = delayedWeth.withdrawals(address(this), _guy);
        assertEq(amount, _wad);
        assertEq(timestamp, block.timestamp);
    }

    /// @notice Tests that unlocking twice is successful and
    ///         timestamp/amount is updated.
    /// @param _wad1 First unlock amount.
    /// @param _wad2 Second unlock amount.
    /// @param _timeDelta Seconds between the two unlocks.
    function testFuzz_unlock_twice_succeeds(uint256 _wad1, uint256 _wad2, uint256 _timeDelta) public {
        ...
        _timeDelta = bound(_timeDelta, 1, type(uint64).max);
...
contract DelayedWETH_Withdraw_Test is DelayedWETH_TestInit {
    /// @notice Tests that withdrawing while unlocked and delay
    ///         has passed is successful.
    /// @param _wad Amount of WETH to deposit and withdraw.
    ...
    /// @notice Tests that withdrawing while locked and delay
    ///         has not passed fails.
    /// @param _wad Amount to deposit and unlock.
    /// @param _timeBefore Seconds before the delay is met.
    function testFuzz_withdraw_whileLockedNotLongEnough_fails(uint256 _wad, uint256 _timeBefore) public {
        _wad = bound(_wad, 1, type(uint192).max);
        _timeBefore = bound(_timeBefore, 1, delayedWeth.delay());
        ...
        vm.warp(block.timestamp + delayedWeth.delay() - _timeBefore);
    ...
    /// @notice Tests that withdrawing more than unlocked amount
    ///         fails.
    /// @param _deposit Amount to deposit and unlock.
    /// @param _extra Extra amount beyond the deposit.
    function testFuzz_withdraw_tooMuch_fails(uint256 _deposit, uint256 _extra) public {
        _deposit = bound(_deposit, 0, type(uint192).max);
        _extra = bound(_extra, 1, type(uint192).max);

        // Deposit some WETH.
        vm.deal(alice, _deposit);
        vm.prank(alice);
        delayedWeth.deposit{ value: _deposit }();
        uint256 balance = address(alice).balance;

        // Unlock the withdrawal.
        vm.prank(alice);
        delayedWeth.unlock(alice, _deposit);
        ...
        vm.prank(alice);
        delayedWeth.withdraw(_deposit + _extra);
        assertEq(address(alice).balance, balance);
    }
    ...
    /// @notice Tests that withdrawing via two-arg withdraw while
    ///         unlocked and delay has passed is successful.
    /// @param _wad Amount of WETH to deposit and withdraw.
    ...
    /// @notice Tests that withdrawing via two-arg withdraw while
    ///         locked and delay has not passed fails.
    /// @param _wad Amount to deposit and unlock.
    /// @param _timeBefore Seconds before the delay is met.
    function testFuzz_withdraw_withdrawFromWhileLockedNotLongEnough_fails(uint256 _wad, uint256 _timeBefore) public {
        _wad = bound(_wad, 1, type(uint192).max);
        _timeBefore = bound(_timeBefore, 1, delayedWeth.delay());
    ...
    /// @notice Tests that withdrawing more than unlocked amount
    ///         via two-arg withdraw fails.
    /// @param _deposit Amount to deposit and unlock.
    /// @param _extra Extra amount beyond the deposit.
    function testFuzz_withdraw_withdrawFromTooMuch_fails(uint256 _deposit, uint256 _extra) public {
        _deposit = bound(_deposit, 0, type(uint192).max);
        _extra = bound(_extra, 1, type(uint192).max);
        ...
        vm.expectRevert("DelayedWETH: insufficient unlocked withdrawal");
        vm.prank(alice);
        delayedWeth.withdraw(alice, _deposit + _extra);
        assertEq(address(alice).balance, balance);
    }
...
contract DelayedWETH_Recover_Test is DelayedWETH_TestInit {
    ...
    /// @param _sender Random non-owner address.
    ...
    /// @notice Tests that recovering a partial amount leaves the
    ///         remaining balance in the contract.
    /// @param _balance Contract ETH balance.
    ...
    function testFuzz_recover_partialAmount_succeeds(uint256 _balance, uint256 _wad) public {
        _balance = bound(_balance, 1, type(uint192).max);
        ...
        // Verify only partial amount was recovered.
...
contract DelayedWETH_Hold_Test is DelayedWETH_TestInit {
    ...
    function testFuzz_hold_byOwner_succeeds(uint256 _amount) public {
    ...
    /// @notice Tests that holding all WETH without specifying
    ///         amount succeeds.
    /// @param _amount Amount of WETH to deposit.
    function testFuzz_hold_withoutAmount_succeeds(uint256 _amount) public {
        _amount = bound(_amount, 0, type(uint192).max);
...
/// @notice Tests the `version` function of the `DelayedWETH`
///         contract.
contract DelayedWETH_Version_Test is DelayedWETH_TestInit {
    /// @notice Tests that the version string is a valid semver.
