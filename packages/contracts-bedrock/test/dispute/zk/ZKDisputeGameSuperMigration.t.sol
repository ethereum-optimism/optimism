...
contract ZKDisputeGameSuperMigration_Test is DisputeGameFactory_TestInit {
    ...
    ///         `anchorGame`, `retirementTimestamp` and the blacklist are preserved, and an
    ...
    function test_flip_inFlightStateCrossesBoundary_succeeds() public {
        ...
        // The in-place ASR re-init preserves the advanced anchor.
        assertEq(address(anchorStateRegistry.anchorGame()), address(advancer), "anchorGame must be preserved");
        (Hash preservedRoot, uint256 preservedSeq) = anchorStateRegistry.getAnchorRoot();
        assertEq(preservedRoot.raw(), advancer.rootClaim().raw(), "advanced anchor root must be preserved");
        assertEq(preservedSeq, anchorSeqNum + 1, "advanced anchor sequence must be preserved");
    ...
    function _runFlipAndAssert(GameType _sourceSuperType) internal {
        ...
        (Hash anchorRootBefore, uint256 anchorSeqNum) = anchorStateRegistry.getAnchorRoot();
        ...
        //       flips the respected type to ZK, preserves the anchor.
        _buildZKFlipUpgradeInput();
        _runUpgradeAsPAO();
        ...
        // Assert that the flip preserves the existing anchor.
        (Hash preservedRoot, uint256 preservedSeq) = anchorStateRegistry.getAnchorRoot();
        assertEq(preservedRoot.raw(), anchorRootBefore.raw(), "anchor root must be preserved");
        assertEq(preservedSeq, anchorSeqNum, "anchor sequence must be preserved");
    ...
    ///         The existing anchor root is preserved.
