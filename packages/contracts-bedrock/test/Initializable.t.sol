...
contract Initializer_Test is ERC721Bridge_Initializer {
    function test_cannotReinitializeL1_succeeds() public {
        ...
        L1Messenger.initialize(OptimismPortal(payable(address(0))), SuperchainConfig(address(0)));

        vm.expectRevert("Initializable: contract is already initialized");
        L1Bridge.initialize(CrossDomainMessenger(address(0)));

        vm.expectRevert("Initializable: contract is already initialized");
        oracle.initialize(0, 0, SystemConfig(address(0)));

        vm.expectRevert("Initializable: contract is already initialized");
        op.initialize(L2OutputOracle(address(0)), SystemConfig(address(0)), SuperchainConfig(address(0)));
        ...
        sysConf.initialize({
            ...
            _oracleRoles: SystemConfig.OracleRoles({ proposer: address(0), challenger: address(0) }),
