...
contract OptimismMintableERC721Factory_CreateOptimismMintableERC721_Test is OptimismMintableERC721Factory_TestInit {
    /// @notice Tests that creating a token succeeds with
    ///         various remote addresses.
    function testFuzz_createOptimismMintableERC721_succeeds(address _remote) external {
        vm.assume(_remote != address(0));

        address local = calculateTokenAddress(_remote, "L2Token", "L2T");

        // Expect a token creation event.
        vm.expectEmit(address(l2OptimismMintableERC721Factory));
        emit OptimismMintableERC721Created(local, _remote, alice);

        // Create the token.
        vm.prank(alice);
        OptimismMintableERC721 created = OptimismMintableERC721(
            l2OptimismMintableERC721Factory.createOptimismMintableERC721(_remote, "L2Token", "L2T")
        );
        ...
        assertEq(created.REMOTE_TOKEN(), _remote);
        assertEq(created.BRIDGE(), address(l2ERC721Bridge));
        assertEq(created.REMOTE_CHAIN_ID(), deploy.cfg().l1ChainID());
    }

    /// @notice Tests that creating the same token twice reverts
    ///         for various remote addresses.
    function testFuzz_createOptimismMintableERC721_sameTwice_reverts(address _remote) external {
        vm.assume(_remote != address(0));
    ...
    /// @notice Tests that creating a token with zero remote
    ///         address reverts.
...
/// @title OptimismMintableERC721Factory_Version_Test
