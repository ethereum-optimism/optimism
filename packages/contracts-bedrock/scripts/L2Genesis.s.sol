...
contract L2Genesis is Script {
    ...
    function setPredeployImplementations(Input memory _input) internal {
        ...
        if (DevFeatures.isDevFeatureEnabled(_input.devFeatureBitmap, DevFeatures.L2CM)) {
            ...
            // deploy() upgrades ConditionalDeployer and L2DevFeatureFlags, run after both are set.
            _deployPredeploysViaL2CM(_input);
        }
    }

    /// @notice Builds the implementation records for the temporary L2ContractsManager.
    /// @dev StorageSetter has no predeploy address, so it is passed as a zero record that deploy
    ///      mode ignores. Variants are kept because the constructor resolves every record by name.
    function _buildL2CMImplRecords() internal pure returns (L2ContractsManagerTypes.ImplRecord[] memory records_) {
        ...
        records_ = new L2ContractsManagerTypes.ImplRecord[](upgradeable.length + 1);
        records_[0] = L2ContractsManagerTypes.ImplRecord({ name: "StorageSetter", impl: address(0) });
        for (uint256 i = 0; i < upgradeable.length; i++) {
            records_[i + 1] = L2ContractsManagerTypes.ImplRecord({
                name: upgradeable[i].name,
                impl: Predeploys.predeployToCodeNamespace(upgradeable[i].proxy)
            });
        }
    }
    ...
    ///         interop the same way as _apply. Variants are skipped since they share a proxy.
    ...
    /// @dev Swaps each touched proxy's admin to the L2ContractsManager so upgrades run as admin,
    ///      calls deploy() once, restores the admins, then erases it to leave no genesis residue.
    ///      Must run with no active prank so the script creates the throwaway L2ContractsManager.
