...
library ChainAssertions {
    ...
    /// @notice Asserts that the SystemConfig is setup correctly
    function checkSystemConfigProxies(Types.ContractSet memory _contracts, Types.DeployOPChainInput memory _doi) internal view {
        ISystemConfig config = ISystemConfig(_contracts.SystemConfig);
        // Check that the contract is initialized
        ...
        require(config.batchInbox() == IOPContractsManager(_doi.opcm).chainIdToBatchInboxAddress(_doi.l2ChainId), "CHECK-SCFG-150");
        // Check _addresses
        ...
        view
    {
        ...
        DeployUtils.assertValidContractAddress(address(_factory));
        if (_isProxy) {
            DeployUtils.assertERC1967ImplementationSet(address(_factory));
        }
