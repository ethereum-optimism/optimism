...
import { IOPContractsManagerLegacyUpgrade } from "../../interfaces/L1/IOPContractsManagerLegacy.sol";
import { BaseDeployIO } from "scripts/deploy/BaseDeployIO.sol";

contract LegacyUpgradeOPChainV2Input is BaseDeployIO {
    address internal _prank;
    IOPContractsManagerLegacyUpgrade internal _opcm;
    ...
    function set(bytes4 _sel, address _value) public {
        ...
        else if (_sel == this.opcm.selector) _opcm = IOPContractsManagerLegacyUpgrade(_value);
        else revert("UpgradeOPCMInput: unknown selector");
    }

    function set(bytes4 _sel, IOPContractsManagerLegacyUpgrade.OpChainConfig[] memory _value) public {
    ...
    function opcm() public view returns (IOPContractsManagerLegacyUpgrade) {
...
contract LegacyUpgradeOPChainV2 is Script {
    function run(LegacyUpgradeOPChainV2Input _uoci) external {
        IOPContractsManagerLegacyUpgrade opcm = _uoci.opcm();
        IOPContractsManagerLegacyUpgrade.OpChainConfig[] memory opChainConfigs =
            abi.decode(_uoci.opChainConfigs(), (IOPContractsManagerLegacyUpgrade.OpChainConfig[]));
        ...
        bytes memory code = vm.getDeployedCode("LegacyUpgradeOPChainV2.s.sol:DummyCaller");
        vm.etch(prank, code);
...
contract DummyCaller {
    ...
    function upgrade(IOPContractsManagerLegacyUpgrade.OpChainConfig[] memory _opChainConfigs)
        external
