...
export class OptimisticCanonicalChainBatchSubmitter extends ScheduledTask {
  ...
  public async runTask(): Promise<void> {
    let l2Batch: OccBatchSubmission
    try {
      l2Batch = await this.dataService.getNextOccTransactionBatchToSubmit()
    } catch (e) {
    ...
    switch (l2Batch.txBatchStatus) {
      case OptimisticCanonicalChainStatus.QUEUED:
        txBatchTxHash = await this.buildAndSendRollupBatchTransaction(l2Batch)
        if (!txBatchTxHash) {
          return
        }
      // Fallthrough on purpose -- this is a workflow
      case OptimisticCanonicalChainStatus.SENT:
        await this.waitForTxBatchConfirms(txBatchTxHash, l2Batch.occBatchNumber)
      // Fallthrough on purpose -- this is a workflow
      case OptimisticCanonicalChainStatus.FINALIZED:
        // Try to submit State roots (below)
        break
      default:
        log.error(
          `Received L1 Batch submission in unexpected tx batch state: ${l2Batch.txBatchStatus}!`
        )
        return
    }

    switch (l2Batch.rootBatchStatus) {
      case OptimisticCanonicalChainStatus.QUEUED:
      ...
      case OptimisticCanonicalChainStatus.SENT:
        await this.waitForStateRootBatchConfirms(
          rootBatchTxHash,
          l2Batch.occBatchNumber
        )
        break
      default:
        log.error(
          `Received L1 Batch submission in unexpected root batch state: ${l2Batch.rootBatchStatus}!`
        )
        return
    }
  }
  ...
  private async buildAndSendRollupBatchTransaction(
    ...
    try {
      ...
      log.debug(
        `Tx batch ${l2Batch.occBatchNumber} appended with at least one confirmation! Tx Hash: ${txRes.hash}`
      )
      txHash = txRes.hash
    } catch (e) {
      logError(
        log,
        `Error submitting tx batch ${l2Batch.occBatchNumber} to canonical chain!`,
        e
      )
      return undefined
    }
    ...
    try {
      log.debug(`Marking tx batch ${l2Batch.occBatchNumber} submitted`)
      ...
      logError(
        log,
        `Error marking tx batch ${l2Batch.occBatchNumber} as submitted!`,
        e
      )
  ...
  private async buildAndSendStateRootBatchTransaction(
    ...
    try {
      ...
      log.debug(
        `State batch ${l2Batch.occBatchNumber} appended with at least one confirmation! Tx Hash: ${stateRootRes.hash}`
      )
      txHash = stateRootRes.hash
    } catch (e) {
      logError(
        log,
        `Error submitting state root batch ${l2Batch.occBatchNumber} to state commitment chain!`,
        e
      )
      return undefined
    }
    ...
    try {
      log.debug(`Marking state root batch ${l2Batch.occBatchNumber} submitted`)
      ...
      logError(
        log,
        `Error marking state root batch ${l2Batch.occBatchNumber} as submitted!`,
        e
      )
      // TODO: Should we return here? Don't want to resubmit, so I think we should update the DB
    }
   ...
   */
  private getTransactionBatchCalldata(batch: OccBatchSubmission): string[] {
    const txs: string[] = []
