...
export class CrossChainMessenger {
   ...
   * @param options Options for the storage proof.
      ...
      if (opts.blockTag === 'finalized') {
        ...
        while (lo + 1 !== hi) {
          ...
          if (await isBatchFinalized(mid)) {
            ...
            // Break early if the distance between the found batch and the upper bound is
            // sufficiently small. Useful for decreasing the cost of finding a finalized batch
            // because binary searching can take a while.
            if (
              hasSomeFinalized &&
              opts?.finalizedDistance !== undefined &&
              hi - lo <= toNumber(opts.finalizedDistance)
            ) {
              break
            }
          } else {
