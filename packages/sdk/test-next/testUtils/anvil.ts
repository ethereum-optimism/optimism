...
type ChainOptions = CreateAnvilOptions &
  Required<Pick<CreateAnvilOptions, 'forkBlockNumber'>> & { chain: Chain }
...
export type AnvilTestUtil = Awaited<ReturnType<typeof anvilTestUtilFactory>>
...
export const anvilTestUtilFactory = async ({ l1, l2 }: TestUtilOptions) => {
  ...
  const accounts = anvilAccounts.map(({ privateKey }) =>
