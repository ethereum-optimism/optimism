...
describe('Message Passing Integration Tests', () => {
  ...
  describe('L1 Message Passing Tests', () => {
    it('Should not queue any messages if ', async () => {
      ...
      messageSubmitter.getReceivedMessages().length.should.equal(0, 'Should not receive any L2ToL1Messages!')
    })

    it('Should not queue any messages if event is not from message passer', async () => {
      const messagePasserFraud = await deployContract(wallet, MessagePasserUtil, [], [])
    ...
    it.only('Should queue messages from message passer', async () => {
      const messagePasserFraud = await deployContract(wallet, MessagePasserUtil, [], [])
      const messagePasserAddress: Address = handler.getL2ToL1MessagePasserAddress()

      const message: string = add0x(Buffer.from(`Roads? Where we're going, we don't need roads.`).toString('hex'))
      ...
      receivedMessages.length.should.equal(1, 'Should receive any L2ToL1Message!')
      ...
      msg.ovmSender.should.equal(messagePasserFraud.address, 'Address mismatch!')
    })
  })
})