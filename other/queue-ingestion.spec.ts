...
describe('Queue Ingestion', () => {
  ...
  before(async () => {
    ...
    l2Provider = injectL2Context(env.alicel2Wallet.provider as any)
    ...
    for (let i = 0; i < numTxs; i++) {
      ...
      const txResponse = await env.bobl1Wallet.sendTransaction({
        data: calldata,
        to: env.ctc.address
      })
  ...
  it('should order transactions correctly', async () => {
    ...
    const from = await env.bobl1Wallet.getAddress()
