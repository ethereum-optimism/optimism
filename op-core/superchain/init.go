...
// submodule at packages/contracts-bedrock/lib/superchain-registry; that SHA is
// resolved at build time by `just sync-superchain` and written into the
...
//
// A missing zip is caught at compile time by //go:embed (in chain.go), not here.
