...
| `<fork>_nut_bundle.json` | Embedded bundle consumed by op-node and kona-node at fork activation |
...
This copies `current-upgrade-bundle.json` to `op-core/nuts/<fork>_nut_bundle.json` and updates `fork_lock.toml` with the sha256 hash and the merge-base commit with `origin/develop`.
...
```toml
[<fork-name>]
bundle = "op-core/nuts/<fork>_nut_bundle.json"  # repo-relative path
