...
func TestVerifyInteropMessages(t *testing.T) {
	...
	tests := []verifyInteropTestCase{
		...
		// Cycle verification routing tests
		{
			name: "CycleVerify/SameTimestampInvokesCycleVerifyFn",
			setup: func() (*Interop, uint64, map[eth.ChainID]eth.BlockID) {
				// Same-timestamp messages should invoke cycleVerifyFn when set
				sourceChainID := eth.ChainIDFromUInt64(10)
				destChainID := eth.ChainIDFromUInt64(8453)
				...
				// Executing message with SAME timestamp as executing block
				execMsg := &suptypes.ExecutingMessage{
					ChainID:   sourceChainID,
					BlockNum:  50,
					LogIdx:    0,
					Timestamp: sharedTimestamp, // Same timestamp
					Checksum:  suptypes.MessageChecksum{0x01},
				}
				...
				interop := &Interop{
					...
					cycleVerifyFn: func(ts uint64, blocks map[eth.ChainID]eth.BlockID) (Result, error) {
						// cycleVerifyFn is called for same-timestamp messages
			...
			validate: func(t *testing.T, result Result) {
				// Result should be valid (cycleVerifyFn returned success)
				require.True(t, result.IsValid(), "result should be valid when cycleVerifyFn succeeds")
			},
		},
		{
			name: "CycleVerify/NonSameTimestampDoesNotInvokeCycleVerifyFn",
			setup: func() (*Interop, uint64, map[eth.ChainID]eth.BlockID) {
				// Non-same-timestamp messages should NOT invoke cycleVerifyFn
				sourceChainID := eth.ChainIDFromUInt64(10)
				destChainID := eth.ChainIDFromUInt64(8453)
				...
				// Executing message with PRIOR timestamp (not same-timestamp)
				execMsg := &suptypes.ExecutingMessage{
					ChainID:   sourceChainID,
					BlockNum:  50,
					LogIdx:    0,
					Timestamp: 500, // Prior timestamp
					Checksum:  suptypes.MessageChecksum{0x01},
				}
				...
				interop := &Interop{
					...
					cycleVerifyFn: func(ts uint64, blocks map[eth.ChainID]eth.BlockID) (Result, error) {
						// This should NOT be called for non-same-timestamp messages
						t.Error("cycleVerifyFn should not be called for non-same-timestamp messages")
						return Result{}, errors.New("unexpected call")
					},
				}
		...
		{
			name: "CycleVerify/CycleVerifyFnInvalidHeadsPropagated",
			setup: func() (*Interop, uint64, map[eth.ChainID]eth.BlockID) {
				// Invalid heads from cycleVerifyFn should be propagated to result
				sourceChainID := eth.ChainIDFromUInt64(10)
				destChainID := eth.ChainIDFromUInt64(8453)
				...
				interop := &Interop{
					...
					cycleVerifyFn: func(ts uint64, blocks map[eth.ChainID]eth.BlockID) (Result, error) {
						// Return invalid head for destChainID
						return Result{
							Timestamp: ts,
							L2Heads:   blocks,
							InvalidHeads: map[eth.ChainID]eth.BlockID{
								destChainID: blocks[destChainID],
							},
						}, nil
					},
				}
			...
			validate: func(t *testing.T, result Result) {
				...
				require.Contains(t, result.InvalidHeads, destChainID, "destChainID should be in InvalidHeads")
				require.NotContains(t, result.InvalidHeads, sourceChainID, "sourceChainID should not be in InvalidHeads")
			},
		},
		{
			name: "CycleVerify/CycleVerifyFnErrorPropagated",
			setup: func() (*Interop, uint64, map[eth.ChainID]eth.BlockID) {
				// Errors from cycleVerifyFn should be propagated
				chainID := eth.ChainIDFromUInt64(10)
				blockHash := common.HexToHash("0x123")
				sharedTimestamp := uint64(1000)
				block := eth.BlockID{Number: 100, Hash: blockHash}
				...
				mockDB := &algoMockLogsDB{
					openBlockRef: eth.BlockRef{Hash: blockHash, Number: 100, Time: sharedTimestamp},
					openBlockExecMsg: map[uint32]*suptypes.ExecutingMessage{
						0: execMsg,
					},
				}
				...
				interop := &Interop{
					...
					logsDBs: map[eth.ChainID]LogsDB{chainID: mockDB},
					cycleVerifyFn: func(ts uint64, blocks map[eth.ChainID]eth.BlockID) (Result, error) {
						return Result{}, errors.New("cycle verification error")
					},
				}

				return interop, sharedTimestamp, map[eth.ChainID]eth.BlockID{chainID: block}
			},
			expectError: true,
			errorMsg:    "cycle verification",
			validate: func(t *testing.T, result Result) {
				require.True(t, result.IsEmpty())
			},
		},
