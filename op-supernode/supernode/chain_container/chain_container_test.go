...
func TestChainContainer_Constructor(t *testing.T) {
	...
	t.Run("P2P disabled for virtual nodes", func(t *testing.T) {
		...
		container := NewChainContainer(chainID, vncfgCopy, log, cfg, initOverload, nil, nil)
	...
	t.Run("SafeDBPath uses subPath", func(t *testing.T) {
		...
		container := NewChainContainer(types.ChainID(420), vncfg, log, cfg, initOverload, nil, nil)
...
func TestChainContainer_Lifecycle(t *testing.T) {
	...
	t.Run("signals stopped channel on exit", func(t *testing.T) {
		...
		impl.virtualNodeFactory = func(cfg *opnodecfg.Config, log gethlog.Logger, initOverload *rollupNode.InitOverload, appVersion string) virtual_node.VirtualNode {
			return mockVN
		}
