...
func NewL2OutputSubmitterWithSigner(cfg Config, from common.Address, signer SignerFactory, l log.Logger) (*L2OutputSubmitter, error) {
	ctx, cancel := context.WithCancel(context.Background())
	...
	for {
		...
		case <-ticker.C:
			...
			updateGasPrice := func(ctx context.Context) (*types.Transaction, error) {
				l.log.Info("proposer updating batch tx gas price", "block", nextCheckpointBlock, "nonce", nonce)
