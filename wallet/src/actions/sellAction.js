...
const configureItemToPlasmaBegin = () => ({
  type: "CONFIGURE_ITEM_TO_PLASMA",
})

const configureItemToPlasmaSuccess = () => ({
  type: "CONFIGURE_ITEM_TO_PLASMA_SUCCESS",
})

const configureItemToPlasmaFailure = (data) => ({
...
//this starts the listing, which is always paid for in OMG
const configureItemToPlasma = (itemID, itemToSend, itemToReceive, address) => async (dispatch) => {
  dispatch(configureItemToPlasmaBegin());
  try {
    ...
    if (tx) {
      dispatch(configureItemToPlasmaSuccess());
      ...
      dispatch(configureItemToPlasmaFailure(404));
    }
  } catch(error) {
    ...
    if (message.data.status === "success" && 
      ...
      dispatch(uploadItemFiles(message, itemID, itemToSend, itemToReceive, address)).then(statusCode => {
        if (statusCode.status === 201) {
          dispatch(configureItemToPlasma(itemID, itemToSend, itemToReceive, address));
        } else {
...
export const acceptBid = ( cMD ) => async (dispatch) => {
  ...
  */
  ...
  const sellerItemToSendDecimals = tokenList[cMD.sellerItemToSend.currency].decimals;
  const sellerItemToSendAgreeAmount = powAmount(cMD.agreeAmount, sellerItemToSendDecimals);
  const sellerItemToReceiveDecimals = tokenList[cMD.sellerItemToReceive.currency].decimals;
      ...
      if (message.data.status === "success" && 
        ...
        if (uploadFilesStatus.status === 201) {
          ...
          try {
            ...
            const swapStatus = await networkService.VarnaSwapContract.open(
              swapID,
              openValue,
              openContractAddress,
              closeValue,
