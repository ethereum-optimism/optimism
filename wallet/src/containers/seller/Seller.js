...
class Seller extends React.Component {
  ...
  constructor(props) {
    ...
    const {
      generateKeyLoad, 
      uploadKeyLoad,
      configureItemToPlasmaLoad, 
      configureItemToPlasmaError,
      itemOpenList, 
      itemOpenOrClosed, 
  ...
  componentDidUpdate(prevState) {
    ...
    if (prevState.sell.configureItemToPlasmaLoad !== configureItemToPlasmaLoad) {
      this.setState({ configureItemToPlasmaLoad });
    }

    if (prevState.sell.configureItemToPlasmaError !== configureItemToPlasmaError) {
      this.setState({ configureItemToPlasmaError });

      if (configureItemToPlasmaError === false) {
...
****/
  ...
  async handleListItem() {
    ...
    const networkStatus = await networkService.checkNetwork('L2');
    if (networkStatus) {
  ...
  render() {
    ...
    ***/
    ...
    if (configureItemToPlasmaLoad) buttonText = "TRANSFERRING"
    ...
    return (
      ...
      <div className={styles.VarnaCube}>
        ...
        <Button
          ...
          size='small'
          type='primary'
          loading={generateKeyLoad || uploadKeyLoad || configureItemToPlasmaLoad}
          disabled={
            itemToSend.symbol === "" || 
            itemToSendAmount === "" || 
