...
type GameTypeRegistry struct {
	types        map[gameTypes.GameType]scheduler.PlayerCreator
...
func NewGameTypeRegistry() *GameTypeRegistry {
	return &GameTypeRegistry{
		types:        make(map[gameTypes.GameType]scheduler.PlayerCreator),
...
func (r *GameTypeRegistry) RegisterGameType(gameType gameTypes.GameType, creator scheduler.PlayerCreator) {
...
func (r *GameTypeRegistry) CreatePlayer(game gameTypes.GameMetadata, dir string) (scheduler.GamePlayer, error) {
	creator, ok := r.types[gameTypes.GameType(game.GameType)]
	if !ok {
...
func (r *GameTypeRegistry) CreateBondContract(game gameTypes.GameMetadata) (claims.BondContract, error) {
	creator, ok := r.bondCreators[gameTypes.GameType(game.GameType)]
	if !ok {
		return nil, fmt.Errorf("%w: %v", ErrUnsupportedGameType, game.GameType)
	}
	return creator(game)
