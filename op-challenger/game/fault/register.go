...
func registerAsterisc(
	...
	playerCreator := func(game types.GameMetadata, dir string) (scheduler.GamePlayer, error) {
		...
		creator := func(ctx context.Context, logger log.Logger, gameDepth faultTypes.Depth, dir string) (faultTypes.TraceAccessor, error) {
			...
			accessor, err := outputs.NewOutputAsteriscTraceAccessor(logger, m, cfg.AsteriscConfig, l2Client, prestateProvider, asteriscPrestate, rollupClient, dir, l1HeadID, splitDepth, prestateBlock, poststateBlock)
			if err != nil {
				return nil, err
			}
			return accessor, nil
...
func registerCannon(
	...
	playerCreator := func(game types.GameMetadata, dir string) (scheduler.GamePlayer, error) {
		...
		creator := func(ctx context.Context, logger log.Logger, gameDepth faultTypes.Depth, dir string) (faultTypes.TraceAccessor, error) {
			...
			accessor, err := outputs.NewOutputCannonTraceAccessor(logger, m, cfg.CannonConfig, l2Client, prestateProvider, cannonPrestate, rollupClient, dir, l1HeadID, splitDepth, prestateBlock, poststateBlock)
			if err != nil {
				return nil, err
			}
			return accessor, nil
