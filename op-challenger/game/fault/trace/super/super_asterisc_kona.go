...
import (
	...
	"github.com/HashKeyChain/verse/op-challenger/game/fault/trace/asterisc"
...
func NewSuperAsteriscKonaTraceAccessor(
	logger log.Logger,
	m metrics.Metricer,
	cfg vm.Config,
	...
	asteriscCreator := func(ctx context.Context, localContext common.Hash, depth types.Depth, claimInfo ClaimInfo) (types.TraceProvider, error) {
		...
		provider := asterisc.NewTraceProvider(logger, m.ToTypedVmMetrics(cfg.VmType.String()), cfg, serverExecutor, prestateProvider, asteriscPrestate, localInputs, subdir, depth)
		return provider, nil
	}

	cache := NewProviderCache(m, "super_asterisc_kona_provider", asteriscCreator)
