	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if errors.Is(err, os.ErrNotExist) {
			...
			if exited && stateStep <= i {
				...
				// Last step is the final state generated by cannon - that is the first state with Exited=0
				// This state is then repeated via trace extension for all remaining leaves in the game.
				p.lastStep = stateStep
