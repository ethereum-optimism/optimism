...
func TestGenerateProof(t *testing.T) {
	...
	t.Run("Network", func(t *testing.T) {
		...
		require.DirExists(t, filepath.Join(dir, vm.PreimagesDir))
		require.DirExists(t, filepath.Join(dir, utils.ProofsDir))
		require.DirExists(t, filepath.Join(dir, vm.SnapsDir))
		...
		require.Equal(t, filepath.Join(dir, vm.FinalState), args["--output"])
		...
		require.Equal(t, filepath.Join(dir, vm.PreimagesDir), args["--datadir"])
		require.Equal(t, filepath.Join(dir, utils.ProofsDir, "%d.json.gz"), args["--proof-fmt"])
		require.Equal(t, filepath.Join(dir, vm.SnapsDir, "%d.json.gz"), args["--snapshot-fmt"])
...
func TestRunCmdLogsOutput(t *testing.T) {
	...
	err := vm.RunCmd(ctx, logger, bin, "Hello World")
	require.NoError(t, err)
...
func TestFindStartingSnapshot(t *testing.T) {
	...
	t.Run("UsePrestateWhenSnapshotsDirDoesNotExist", func(t *testing.T) {
		dir := t.TempDir()
		snapshot, err := vm.FindStartingSnapshot(logger, filepath.Join(dir, "doesNotExist"), execTestCannonPrestate, 1200)
		require.NoError(t, err)
	...
	t.Run("UsePrestateWhenSnapshotsDirEmpty", func(t *testing.T) {
		dir := withSnapshots(t)
		snapshot, err := vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 1200)
		require.NoError(t, err)
		require.Equal(t, execTestCannonPrestate, snapshot)
	})
	...
	t.Run("UsePrestateWhenNoSnapshotBeforeTraceIndex", func(t *testing.T) {
		...
		snapshot, err := vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 99)
		require.NoError(t, err)
		require.Equal(t, execTestCannonPrestate, snapshot)

		snapshot, err = vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 100)
		require.NoError(t, err)
		require.Equal(t, execTestCannonPrestate, snapshot)
	})
	...
	t.Run("UseClosestAvailableSnapshot", func(t *testing.T) {
		...
		snapshot, err := vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 101)
		require.NoError(t, err)
		require.Equal(t, filepath.Join(dir, "100.json.gz"), snapshot)

		snapshot, err = vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 123)
		require.NoError(t, err)
		require.Equal(t, filepath.Join(dir, "100.json.gz"), snapshot)

		snapshot, err = vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 124)
		require.NoError(t, err)
		require.Equal(t, filepath.Join(dir, "123.json.gz"), snapshot)

		snapshot, err = vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 256)
		require.NoError(t, err)
	...
	t.Run("IgnoreDirectories", func(t *testing.T) {
		...
		snapshot, err := vm.FindStartingSnapshot(logger, dir, execTestCannonPrestate, 150)
		require.NoError(t, err)
		require.Equal(t, filepath.Join(dir, "100.json.gz"), snapshot)
	})
