...
type gameSource interface {
	FetchAllGamesAtBlock(ctx context.Context, earliestTimestamp uint64, blockNumber *big.Int) ([]types.PlayerCreator, error)
}

type gameScheduler interface {
	Schedule([]types.PlayerCreator) error
}
...
func (m *gameMonitor) progressGames(ctx context.Context, blockNum uint64) error {
	...
	var gamesToPlay []types.PlayerCreator
	for _, game := range games {
		// TODO: Move filtering to GameSource
		if !m.allowedGame(game.Addr()) {
			m.logger.Debug("Skipping game not on allow list", "game", game.Addr())
			continue
		}
