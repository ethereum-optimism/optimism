...
func TestSchedulerProcessesGames(t *testing.T) {
	...
	require.NoError(t, s.Schedule(games, blockID(0)))
...
func TestReturnBusyWhenScheduleQueueFull(t *testing.T) {
	...
	require.NoError(t, s.Schedule(asGames(common.Address{0xaa}), blockID(0)))

	// Second call should return busy
	err := s.Schedule(asGames(common.Address{0xaa}), blockID(0))
