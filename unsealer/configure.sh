...
function secrets {
    ...
    vault write kv/vault_cacert value=$(cat $HOME/etc/vault.unsealer/ca.pem | base64)
    vault write kv/vault_key value=$(cat $HOME/etc/vault.unsealer/services-key.pem | base64)
    vault write kv/vault_crt value=$(cat $HOME/etc/vault.unsealer/services.pem | base64)
    if [ -f "$OVPN_FILE" ]; then
