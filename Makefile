...
mise.sha256sum: mise.toml Containerfile.mise ## Update the sumfile used by `make shell`
	docker build -f Containerfile.mise --output type=local,dest=/tmp/mise-build --target mise-lock --secret id=GITHUB_TOKEN,env=$(GITHUB_TOKEN) .
...
shell: ## Spawn a shell with all necessary development tools accessible
	docker build -f Containerfile.mise --tag optimism-shell --target mise-shell --secret id=GITHUB_TOKEN,env=$(GITHUB_TOKEN) .
