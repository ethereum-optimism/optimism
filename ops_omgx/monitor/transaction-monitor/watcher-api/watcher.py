...
def watcher_getTransaction(event, context):
  ...
  fromRange = body.get("fromRange")
  toRange = body.get("toRange")
...
def watcher_getDeployments(event, context):
  ...
  with con.cursor() as cur:
    try:
      ...
      cur.execute("""SELECT hash, deployedContract, `from`, blocknumber, timestamp FROM transaction WHERE `from`=%s and deployedContract IS NOT NULL ORDER BY CAST(Timestamp as unsigned)""", (address))
      transactionsDataRaw = cur.fetchall()
      for transactionDataRaw in transactionsDataRaw:
        transactionData.append({
          "hash": transactionDataRaw[0],
          "contract address": int(transactionDataRaw[1]),
          "from": transactionDataRaw[2],
          "blocknumber": transactionDataRaw[3],
          "timeStamp": transactionDataRaw[4]
        })
        contractAddresses.append(int(transactionDataRaw[1]))
    except:
      transactionData = []
      contractAddresses = []
  ...
  response = {
    ...
    "body": json.dumps(transactionData),
    "data": json.dumps(contractAddresses)
  }
  return response
