...
func TestProofSizeCap(t *testing.T) {
	...
	require.Len(t, data, EncodedSizeEmptyProof+MaxProofSize, "empty writes add no payload bytes")
	got, err := DecodeMode(data, ModeProven)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Greater(t, len(overBytes), EncodedSizeEmptyProof+MaxProofSize)
