...
impl OpExecutionData {
    ...
    /// # Panics
    ///
    /// This function will not panic; the `unwrap` is guarded by the preceding emptiness check.
    pub fn from_flashblocks(
        ...
        for (i, fb) in flashblocks.iter().enumerate() {
            // `fb.index` is a `u64`; on 64-bit targets this cannot truncate, and on 32-bit
            // targets sequential fixture payloads never exceed `usize::MAX`.
    ...
    pub fn from_flashblocks_unchecked(flashblocks: &[OpFlashblockPayload]) -> Self {
        ...
        Self::v4(v4, Vec::new(), base.parent_beacon_block_root, Requests::default())
    }
...
impl OpNetworkPayloadEnvelope {
    ...
    /// # Errors
    ///
    /// Returns an error if the data cannot be snappy-decompressed, the length is below the
    /// signature + block prefix, or the signature or SSZ payload cannot be parsed.
    #[cfg(feature = "std")]
    ...
    /// # Errors
    ///
    /// Returns [`PayloadEnvelopeEncodeError::WrongVersion`] if `self` does not contain a V1
    /// payload, or any error returned by the snappy encoder.
    #[cfg(feature = "std")]
    pub fn encode_v1(&self) -> Result<Vec<u8>, PayloadEnvelopeEncodeError> {
        use ssz::Encode;
        let OpExecutionPayload::V1(execution_payload_v1) = &self.payload else {
            return Err(PayloadEnvelopeEncodeError::WrongVersion);
        };

        let mut data = Vec::new();
        let mut sig = self.signature.as_bytes();
        sig[64] = u8::from(self.signature.v());
    ...
    /// # Errors
    ///
    /// Returns [`PayloadEnvelopeEncodeError::WrongVersion`] if `self` does not contain a V2
    /// payload, or any error returned by the snappy encoder.
    #[cfg(feature = "std")]
    pub fn encode_v2(&self) -> Result<Vec<u8>, PayloadEnvelopeEncodeError> {
        use ssz::Encode;
        let OpExecutionPayload::V2(execution_payload_v2) = &self.payload else {
            return Err(PayloadEnvelopeEncodeError::WrongVersion);
        };
    ...
    ///
    /// Returns an error if the data cannot be snappy-decompressed, the length is below the
    /// signature + beacon root + block prefix, or the signature or SSZ payload cannot be parsed.
    #[cfg(feature = "std")]
    ...
    /// # Errors
    ///
    /// Returns [`PayloadEnvelopeEncodeError::WrongVersion`] if `self` does not contain a V3
    /// payload, or any error returned by the snappy encoder.
    ///
    /// # Panics
    ///
    /// Panics if `parent_beacon_block_root` is `None`, which is required for V3 payloads.
    #[cfg(feature = "std")]
    pub fn encode_v3(&self) -> Result<Vec<u8>, PayloadEnvelopeEncodeError> {
        use ssz::Encode;
        let OpExecutionPayload::V3(execution_payload_v3) = &self.payload else {
            return Err(PayloadEnvelopeEncodeError::WrongVersion);
        };
    ...
    /// # Errors
    ///
    /// Returns [`PayloadEnvelopeEncodeError::WrongVersion`] if `self` does not contain a V4
    /// payload, or any error returned by the snappy encoder.
    ///
    /// # Panics
    ///
    /// Panics if `parent_beacon_block_root` is `None`, which is required for V4 payloads.
    #[cfg(feature = "std")]
    pub fn encode_v4(&self) -> Result<Vec<u8>, PayloadEnvelopeEncodeError> {
        use ssz::Encode;
        let OpExecutionPayload::V4(execution_payload_v4) = &self.payload else {
            return Err(PayloadEnvelopeEncodeError::WrongVersion);
        };
...
mod tests {
    ...
    fn test_roundtrip_encode_envelope_v1() {
        ...
        assert_eq!(1_725_271_882, payload_envelop.payload.timestamp());
    ...
    fn test_roundtrip_encode_envelope_v2() {
        ...
        assert_eq!(1_708_427_627, payload_envelop.payload.timestamp());
    ...
    fn test_roundtrip_encode_envelope_v3() {
        ...
        assert_eq!(1_708_427_461, payload_envelop.payload.timestamp());
    ...
    fn test_roundtrip_encode_envelope_v4() {
        ...
        assert_eq!(1_741_842_007, payload_envelop.payload.timestamp());
    ...
    fn test_from_flashblocks_unichain_sepolia_block() {
        ...
        assert_eq!(execution_data.payload.block_number(), 0x21E_3B52);
        assert_eq!(execution_data.payload.timestamp(), 0x690b_e89e);
        assert_eq!(
            execution_data.payload.fee_recipient(),
            address!("4200000000000000000000000000000000000011")
        );
        assert_eq!(execution_data.payload.gas_limit(), 0x393_8700);
    ...
    fn test_from_flashblocks_base_sepolia_block() {
        ...
        assert_eq!(execution_data.payload.block_number(), 0x01fe_4052);
        assert_eq!(execution_data.payload.timestamp(), 0x690f_df84);
        assert_eq!(
            execution_data.payload.fee_recipient(),
            address!("4200000000000000000000000000000000000011")
        );
        assert_eq!(execution_data.payload.gas_limit(), 0x393_8700);
        assert_eq!(execution_data.payload.as_v1().gas_used, 0x49_f43c);
