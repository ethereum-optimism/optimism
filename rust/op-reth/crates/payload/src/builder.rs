...
mod tests {
    ...
    fn build_post_exec_recovered_tx_wraps_entries_in_post_exec_tx() {
        ...
        let block_number = 42;
        let recovered =
            build_post_exec_recovered_tx::<OpTransactionSigned>(block_number, entries.clone());
    ...
    /// Consensus-critical: if the post-exec tx fails to execute, the payload build
    ...
    fn try_include_post_exec_tx_aborts_when_execution_fails() {
        ...
        let result = try_include_post_exec_tx::<OpTransactionSigned, _>(1, entries, |_tx| {
            called.set(true);
