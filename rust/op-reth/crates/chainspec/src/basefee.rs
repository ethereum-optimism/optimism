...
mod tests {
    ...
    use crate::{OP_SEPOLIA, OpChainSpec};
    ...
    fn get_chainspec() -> Arc<OpChainSpec> {
        let mut op_sepolia_spec = OP_SEPOLIA.inner.clone();
        op_sepolia_spec
            .hardforks
            .insert(OpHardfork::Jovian.boxed(), ForkCondition::Timestamp(JOVIAN_TIMESTAMP));
        Arc::new(OpChainSpec {
            inner: ChainSpec {
                chain: op_sepolia_spec.chain,
                genesis: op_sepolia_spec.genesis,
                genesis_header: op_sepolia_spec.genesis_header,
                ..Default::default()
            },
        })
    }
    ...
    fn test_next_base_fee_jovian_blob_gas_used_greater_than_gas_used() {
        ...
        parent.extra_data = encode_jovian_extra_data(
            [0; 8].into(),
