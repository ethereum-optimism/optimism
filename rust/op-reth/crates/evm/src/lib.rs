...
#[cfg(test)]
// Test-only pedantic warnings: avoid importing revm internals (`BundleState`,
// `CfgEnv`) into test-only code, keep long literal block numbers readable as-is,
// and permit `items_after_statements` for the closure-style inner tests used here.
#[allow(
    clippy::default_trait_access,
    clippy::unreadable_literal,
    clippy::items_after_statements,
    clippy::similar_names,
    reason = "test-only code; avoids pulling revm internals into test imports"
)]
mod tests {
    use super::*;
