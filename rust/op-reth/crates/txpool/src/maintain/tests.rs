...
async fn reorg_evicts_pooled_interop_txs() {
    ...
    maintain_transaction_pool_interop(pool.clone(), events, MockInteropFilter::transient_failure())
        .await;
...
async fn commit_with_failsafe_evicts_all_interop_txs() {
    ...
    maintain_transaction_pool_interop(pool.clone(), events, MockInteropFilter::failsafe_enabled())
        .await;
