...
/// Post-exec replay mode.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum PostExecReplayMode {
    /// Run block execution without post-exec.
    Disabled,
    /// Re-execute a historical block as if post-exec had been enabled.
    #[default]
    CounterfactualEnabled,
    /// Re-execute while also validating against an already-existing post-exec payload.
    Verifier,
}
...
pub struct PostExecReplaySummary {
    ...
    pub replay_mode: PostExecReplayMode,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PostExecReplayRunConfig {
    #[serde(rename = "type")]
