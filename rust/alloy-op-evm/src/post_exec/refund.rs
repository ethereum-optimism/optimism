...
/// refunds for ineligible transaction kinds. It never observes how the refund was computed.
/// Verifiers run the default inspector and discard its refund, so a proprietary producer policy
/// cannot make a verifier accept an *invalid* block. The seam still requires the implementor to be
/// side-effect-free w.r.t. EVM state (see the [`Inspector`](revm::Inspector) rule below): mutating
/// execution rather than merely returning malformed refund data can produce a self-rejecting block.
