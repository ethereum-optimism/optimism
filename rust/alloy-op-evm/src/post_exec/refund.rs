...
/// [`finish_tx`](Self::finish_tx), caps normal-transaction refunds at `evm_gas_used`, and discards
/// deposit refunds. It never observes how the refund was computed. Verifiers run the default
/// inspector and discard its refund, so a proprietary producer policy cannot make a verifier
/// accept an *invalid* block. The seam still requires the implementor to be side-effect-free
/// w.r.t. EVM state (see the [`Inspector`](revm::Inspector) rule below): mutating execution rather
/// than merely returning malformed refund data can produce a self-rejecting block.
...
pub trait PostExecRefundInspector {
    ...
    /// Finish the current transaction. The result's aggregate refund is the final net gas discount;
    /// its attribution events are optional diagnostics.
    ///
    /// `gas` is the original EVM gas accounting, before any post-exec adjustment. It includes gross
    /// spend, the EIP-3529-capped native refund, and the EIP-7623 calldata floor. Policies that
    /// rebate gross execution costs can use it to recompute the net charge rather than
    /// double-counting native refunds. The executor does not prescribe a policy's economics.
    ///
    /// Success, revert, and halt outcomes all supply `Some(gas)`. An EVM error supplies `None`:
    /// implementations must still finish per-transaction cleanup, but should return no refund.
