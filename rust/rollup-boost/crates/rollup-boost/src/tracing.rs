...
impl SpanProcessor for MetricsSpanProcessor {
    ...
    fn on_end(&self, span: opentelemetry_sdk::trace::SpanData) {
        ...
        let labels = span
            ...
            .map(|attr| (attr.key.as_str().to_string(), attr.value.as_str().to_string()))
            .chain([
...
pub fn init_tracing(args: &RollupBoostServiceArgs) -> eyre::Result<()> {
    ...
    let log_filter =
        Targets::new().with_default(LevelFilter::INFO).with_target(&filter_name, args.log_level);
    ...
    if args.tracing {
        ...
        let resource =
            Resource::builder().with_attribute(KeyValue::new("service.name", service_name)).build();
