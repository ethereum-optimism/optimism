...
use crate::{
    ClientResult, EngineApiExt, RpcClient, flashblocks::metrics::FlashblocksServiceMetrics,
};
...
use op_alloy_rpc_types_engine::{
    ...
    OpFlashblockPayload, OpFlashblockPayloadBase, OpFlashblockPayloadDelta, OpPayloadAttributes,
};
...
impl FlashblocksService {
    ...
    pub async fn run(&mut self, mut stream: mpsc::Receiver<OpFlashblockPayload>) {
        while let Some(event) = stream.recv().await {
            self.on_event(FlashblocksEngineMessage::OpFlashblockPayload(event)).await;
        }
    }
}
...
mod tests {
    ...
    async fn test_flashblocks_builder() -> eyre::Result<()> {
        ...
        let result = builder.extend(OpFlashblockPayload {
            payload_id: PayloadId::default(),
            index: 0,
            base: Some(OpFlashblockPayloadBase { ..Default::default() }),
            ..Default::default()
        });
        assert!(result.is_ok());
