    ...
    async fn test_redis_instance_app_tracking_and_cleanup() {
...

        {
...

            {
                ...
                let exists: bool = redis::cmd("EXISTS")
                    .arg("test:instance:instance1:heartbeat")
                    .query(&mut conn)
                    .unwrap();
                assert!(exists, "Instance1 heartbeat should exist initially");

                let app1_instance1_count: usize = redis::cmd("GET")
                    .arg(format!("test:app:{app_1}:instance:instance1:connections"))
                    .query(&mut conn)
                    .unwrap();
                let app2_instance1_count: usize = redis::cmd("GET")
                    .arg(format!("test:app:{app_2}:instance:instance1:connections"))
                    .query(&mut conn)
                    .unwrap();
...

        {
            ...
            let app1_instance2_count: usize = redis::cmd("GET")
                .arg(format!("test:app:{app_1}:instance:instance2:connections"))
                .query(&mut conn)
                .unwrap();
