...
const ELF_NAMES: [&str; 2] = ["aggregation-elf", "range-elf"];
...
fn write_elf_input(source: &Path, dest: &Path) {
    if source.is_file() {
        ...
        // run the Dockerized `cargo-prove` ELF build. The empty files written
        // here keep normal host-toolchain compile, clippy, rustdoc, and test
        // jobs working without making SP1 a global CI prerequisite. Runtime
        // bench/proof entrypoints still reject these empty placeholders and
        // require `cd rust/kona/sp1 && just build-elfs` first.
        fs::write(dest, b"")
