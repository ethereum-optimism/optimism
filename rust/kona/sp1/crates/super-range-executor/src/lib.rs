...
pub async fn run(config: RunConfig) -> Result<Verdict> {
    if kona_sp1_elfs::SUPER_RANGE_ELF.is_empty() {
        bail!(
            "SUPER_RANGE_ELF is an empty placeholder; build the guest ELFs first with \
    ...
    let sp1_range_outputs = match execute_sp1_range(&synthesized.range_inputs, range_witness).await
    {
        Ok(outputs) => outputs,
        Err(err) => {
            tracing::info!("super-range guest execution failed (claim rejected): {err}");
            return Ok(Verdict::Invalid);
        }
    };

    if sp1_range_outputs != native_range_outputs {
        tracing::error!(
            expected = ?native_range_outputs,
            actual = ?sp1_range_outputs,
            "super-range guest committed output that differs from native witness-discovery pass",
        );
        return Ok(Verdict::Invalid);
    }

    ensure_range_outputs_match_inputs(&sp1_range_outputs, &synthesized.range_inputs)?;
    ...
    let sp1_consolidation_outputs =
        match execute_sp1_consolidation(&synthesized.consolidation_inputs, consolidation_witness)
            .await
        {
            Ok(outputs) => outputs,
            Err(err) => {
                tracing::info!(
                    "super-range consolidation guest execution failed (claim rejected): {err}"
                );
                return Ok(Verdict::Invalid);
            }
        };

    if sp1_consolidation_outputs != native_consolidation_outputs {
        tracing::error!(
            expected = ?native_consolidation_outputs,
            actual = ?sp1_consolidation_outputs,
            "super-range guest committed consolidation output that differs from native witness-discovery pass",
        );
        return Ok(Verdict::Invalid);
    }

    ensure_consolidation_outputs_match_inputs(
        &sp1_consolidation_outputs,
        &synthesized.consolidation_inputs,
    )?;
    tracing::info!(
        timestamp = config.end_timestamp,
        chain_count = synthesized.range_inputs.chain_ids.len(),
        "super-range guest validated the optimistic transitions and consolidation",
    );
    Ok(Verdict::Valid)
}
...
async fn execute_sp1_range(
    ...
    let elf = Elf::Static(kona_sp1_elfs::SUPER_RANGE_ELF);
