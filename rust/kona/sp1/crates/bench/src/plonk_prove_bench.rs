...
async fn main() -> anyhow::Result<()> {
    ...
    for path in &args.proofs {
        let proof_with_public_values = SP1ProofWithPublicValues::load(path).with_context(|| {
            format!("failed to load compressed range proof from {}", path.display())
        })?;
        let boot_info =
            BootInfoStruct::abi_decode(proof_with_public_values.public_values.as_slice())
                .with_context(|| {
                    format!(
                        "failed to ABI-decode BootInfoStruct from proof public values: {}",
                        path.display()
                    )
                })?;
