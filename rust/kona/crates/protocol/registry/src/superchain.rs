...
mod tests {
    ...
    fn test_read_chain_configs() {
        ...
        let op_config = superchains.op_chains.get(&10).unwrap();
        assert_eq!(op_config.name, "OP Mainnet");
        assert_eq!(op_config.chain_id, 10);
        assert_eq!(op_config.l1_chain_id, 1);
        assert_eq!(op_config.hardfork_config, crate::test_utils::OP_MAINNET_CONFIG.hardforks);
    }
