...
//!  1. A pre-bundle `L1Block.setFeature(INTEROP)` deposit. Required so the `L2ContractsManager`'s
//!     runtime `_loadFullConfig()` reads the feature flag as enabled and applies the Interop-gated
//!     proxy upgrades.
//!  2. The JSON NUT bundle generated by
//!     `packages/contracts-bedrock/scripts/upgrade/GenerateNUTBundle.s.sol` with `UPGRADE_NAME =
//!     "interop"`.
//!  3. A post-bundle `ETHLiquidity` funding deposit with mint and value set to `u128::MAX` — the
//!     only Interop deposit with nonzero mint/value, hence not expressible in the JSON schema.
