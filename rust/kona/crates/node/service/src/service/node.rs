...
impl RollupNode {
    ...
    pub async fn start(&self) -> Result<(), String> {
        ...
        let (network, network_inbound) = NetworkActor::init(NetworkActorBuilder {
        ...
        let (l1_watcher, l1_inbound) = L1WatcherActor::init(L1WatcherActorBuilder {
        ...
        let (engine, _) = EngineActor::init(engine_builder)
            .await
        ...
        let (derivation, _) = ConfiguredDerivationActor::init(derivation_builder)
            .await
        ...
        let (sequencer, sequencer_admin_client) = if self.mode().is_sequencer() {
            ...
            let (seq_actor, seq_inbound) = SequencerActor::init(SequencerActorBuilder {
        ...
        let rpc = if let Some(b) = self.rpc_builder() {
            let (rpc_actor, _) = RpcActor::init(RpcActorBuilder {
                config: b,
