...
impl<P: L1OriginSelectorProvider> L1OriginSelector<P> {
    ...
    async fn try_fetch_next_origin(&mut self) -> Result<(), L1OriginSelectorError> {
        ...
        // Re-query the canonical number even when a candidate is cached. A reorg may replace the
        // next block while leaving the current origin unchanged.
        let next = self.l1.get_block_by_number(current.number.saturating_add(1)).await?;
        self.next = next.filter(|next| next.parent_hash == current.hash);
        Ok(())
    }
}
...
pub enum L1OriginSelectorError {
    /// Shared L1 query failed.
    #[error("shared L1 query failed: {0}")]
    Provider(String),
...
/// A wrapper around shared L1 access that delays the visible canonical chain by a configurable
/// amount of blocks.
#[derive(Debug)]
...
mod test {
    ...
    struct MutableOriginSelectorProvider {
        by_hash: Arc<Mutex<HashMap<B256, BlockInfo>>>,
        by_number: Arc<Mutex<HashMap<u64, BlockInfo>>>,
    }

    impl MutableOriginSelectorProvider {
        fn insert(&self, block: BlockInfo) {
            self.by_hash.lock().unwrap().insert(block.hash, block);
            self.by_number.lock().unwrap().insert(block.number, block);
        }
    }

    #[async_trait]
    impl L1OriginSelectorProvider for MutableOriginSelectorProvider {
        async fn get_block_by_hash(
            &self,
            hash: B256,
        ) -> Result<Option<BlockInfo>, L1OriginSelectorError> {
        ...
        async fn get_block_by_number(
            ...
            Ok(self.by_number.lock().unwrap().get(&number).copied())
        }
    }

    #[tokio::test]
    async fn replaces_a_cached_next_origin_after_an_l1_reorg() {
        ...
        let old_next = BlockInfo {
            parent_hash: current.hash,
            hash: B256::with_last_byte(2),
            number: 2,
            timestamp: 2,
        };
        let new_next = BlockInfo { hash: B256::with_last_byte(3), ..old_next };
        let provider = MutableOriginSelectorProvider::default();
        provider.insert(current);
        provider.insert(old_next);
        let mut selector = L1OriginSelector::new(
            Arc::new(RollupConfig { block_time: 2, ..Default::default() }),
            provider.clone(),
        );
        let unsafe_head = L2BlockInfo {
            l1_origin: alloy_eips::NumHash { number: current.number, hash: current.hash },
            ..Default::default()
        };

        assert_eq!(selector.next_l1_origin(unsafe_head, false).await.unwrap(), old_next);
        provider.insert(new_next);
        assert_eq!(selector.next_l1_origin(unsafe_head, false).await.unwrap(), new_next);
    }
