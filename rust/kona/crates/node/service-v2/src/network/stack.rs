...
/// An error from the [`UnstartedNetwork`].
...
impl UnstartedNetwork {
    /// Starts the network.
    pub async fn start(mut self) -> Result<NetworkHandler, NetworkStartError> {
        ...
        if self.enr_update {
            ...
            let ip_address = gossip_listen_addr
                ...
                .ok_or_else(|| {
                    NetworkStartError::InvalidGossipListenAddr(gossip_listen_addr.clone())
                })?;
