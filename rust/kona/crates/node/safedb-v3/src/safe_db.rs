//! A [`rocksdb`]-backed [`SafeDbV3`] implementation.
...
///
/// The store is held behind an [`RwLock`] so that [`SafeDatabaseV3::close`] can drop it while
/// ensuring no read iterator is still borrowing it, mirroring the read/write exclusion of the Go
/// implementation.
#[derive(Debug)]
pub struct SafeDatabaseV3 {
...
impl SafeDbV3 for SafeDatabaseV3 {
