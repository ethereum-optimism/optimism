...
#[cfg(feature = "rocksdb")]
mod safe_db {
    use crate::{SafeDatabaseV2, SafeDbError, SafeDbV2, SafeHeadRecord};
    use alloy_eips::BlockNumHash;
    ...
    fn open() -> (TempDir, SafeDatabaseV2) {
        let dir = TempDir::new().unwrap();
        let db = SafeDatabaseV2::new(dir.path()).unwrap();
        (dir, db)
    }
    ...
    fn store_safe_heads() {
        ...
        db.close().unwrap();
        let reopened = SafeDatabaseV2::new(dir.path()).unwrap();
        verify(&reopened);
    }
    ...
    fn populated_l1_at_safe_head_db() -> (TempDir, SafeDatabaseV2) {
        let (dir, db) = open();
    ...
    fn open_on_locked_path_returns_backend_error() {
        ...
        let (dir, _held) = open();
        assert!(matches!(SafeDatabaseV2::new(dir.path()), Err(SafeDbError::Backend(_))));
    }
}

mod disabled {
    use crate::{DisabledDatabaseV2, SafeDbError, SafeDbV2};

    #[test]
    fn l1_at_safe_head_disabled() {
        assert!(matches!(DisabledDatabaseV2.l1_at_safe_head(500), Err(SafeDbError::NotEnabled)));
    }

    #[test]
    fn reports_disabled() {
        assert!(!DisabledDatabaseV2.enabled());
    }
}
