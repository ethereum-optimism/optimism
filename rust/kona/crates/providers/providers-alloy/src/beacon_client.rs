...
mod tests {
    ...
    #[test]
    fn test_boxed_blob_response_deserializes_on_small_stack() {
        ...
        let bundle = std::thread::Builder::new()
            .stack_size(1024 * 1024)
            .spawn(move || serde_json::from_slice::<BoxedGetBlobsResponse>(&response).unwrap())
            .unwrap()
            .join()
            .unwrap();

        assert_eq!(bundle.data.len(), 1);
        assert_eq!(*bundle.data[0].0, TEST_BLOB_DATA);
    }
