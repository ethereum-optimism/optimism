...
pub struct Broadcast {
    /// Pranked/broadcast sender the sub-call executes as.
    pub from: String,
    /// Call target, or the address a CREATE/CREATE2 produced.
    pub to: String,
    /// Calldata (CALL) or init code (CREATE/CREATE2).
    pub input: String,
    /// Wei value sent with the sub-call.
    pub value: String,
    /// CREATE2 salt; zero for CALL and CREATE.
    pub salt: String,
    /// Gas the sub-call consumed, used to size the on-chain broadcast's gas limit.
    #[serde(rename = "gasUsed")]
    pub gas_used: u64,
    /// Sub-call kind: `call`, `create`, or `create2`.
    #[serde(rename = "type")]
    pub kind: String,
    /// Sender nonce at broadcast time.
    pub nonce: u64,
}
...
/// revm `Inspector` that reproduces the `op-chain-ops/script` host: it intercepts calls to the
/// cheatcode/console precompiles, applies prank/broadcast caller overrides, and captures
/// broadcasts.
#[derive(Debug, Default)]
pub struct CheatInspector {
    /// Addresses permitted to invoke cheatcodes, mirroring the Go host's `AllowCheatcodes`.
    pub allowed: alloy_primitives::map::HashSet<Address>,
    /// Values returned by `vm.envOr`/`vm.env*`, keyed by environment variable name.
    pub env_vars: std::collections::HashMap<String, String>,
    /// Whether the deterministic CREATE2 deployer is preloaded (mirrors `WithCreate2Deployer`).
    ...
    /// Broadcasts captured during the run, in call order, drained via `take_broadcasts`.
...
impl CheatInspector {
    /// Clears per-call state (frame stack and pending nonce undo) between top-level invocations.
    ...
    /// Standard EVM precompiles (`0x01..=0x0a` at Cancun). The Go host frames only bytecode-bearing
    /// targets — its call stack is driven by `onOpcode`, which never fires for precompiles — so
    /// calls to these must not consume a one-shot broadcast/prank, emit a broadcast entry, or push
    /// a call frame here.
    fn is_standard_precompile(addr: &Address) -> bool {
        let b = addr.as_slice();
        b[..19].iter().all(|&x| x == 0) && (1..=0x0a).contains(&b[19])
    }
}
...
/// tx.
///
/// The `transaction_id` bump also covers geth's `Finalise(true)` (the SSTORE-original re-baseline),
/// not just the access-list reset: revm's `mark_warm_with_transaction_id` resets a cold slot's
/// `original_value` to its `present_value` on the next access, so a broadcast that writes a slot
/// the enclosing script already wrote sees `original == present` (a clean-slot SSTORE) with
/// geth-identical gas and refund accounting — matching `state.Finalise(true)` before the broadcast
/// prelude.
...
impl CheatInspector {
    ...
    fn dispatch_cheatcode(
        ...
        // The 2-arg forms (prank_1/startPrank_1) also carry a tx.origin override; it is not
        // applied here, matching the deploy-script paths that never depend on a pranked origin.
        if sel == Vm::prank_0Call::SELECTOR {
            let c = decode::<Vm::prank_0Call>(args)?;
            self.set_prank(Some(c.msgSender), false, false);
            return Ok(Bytes::new());
        }
        ...
        if sel == Vm::startPrank_0Call::SELECTOR {
            ...
            self.set_prank(Some(c.msgSender), true, false);
            return Ok(Bytes::new());
        }
        ...
        if sel == Vm::broadcast_1Call::SELECTOR {
            ...
            self.set_prank(Some(c.signer), false, true);
            return Ok(Bytes::new());
        }
        ...
        if sel == Vm::startBroadcast_1Call::SELECTOR {
            ...
            self.set_prank(Some(c.signer), true, true);
            return Ok(Bytes::new());
        }
...
impl Inspector<ScriptContext> for CheatInspector {
    fn call(&mut self, ctx: &mut ScriptContext, inputs: &mut CallInputs) -> Option<CallOutcome> {
        ...
        // cheatcode precompile. NOTE: access control (Go's AccessControlledPrecompile) is
        // intentionally NOT enforced here — the deploy scripts deploy helper contracts that call
        // cheatcodes and rely on Go's transitive auto-grant-on-deploy (script.go onOpcode), so a
        // naive `allowed`-set check denies legitimate callers and breaks byte parity. Every cheat
        // caller on the routed paths is a would-be-allowed address, so enforcing vs not is a no-op
        // for committed state. Enforcing correctly requires replicating the auto-grant.
        if target == VM_ADDR {
        ...
        // Standard precompiles run without a call frame in the Go host, so they must not consume a
        // one-shot broadcast/prank, be recorded as a broadcast, or push a frame here.
        if Self::is_standard_precompile(&target) {
            return None;
        }
        ...
        if let Some(parent) = self.frames.last_mut() &&
            let Some(prank) = parent.prank.clone()
        {
    ...
    fn call_end(
        ...
        if Self::is_cheat_target(&inputs.target_address) ||
            ...
            Self::is_standard_precompile(&inputs.target_address)
        {
            return;
        }
        if let Some(frame) = self.frames.pop() &&
            let Some(cap) = frame.captured
        {
            #[allow(deprecated)]
