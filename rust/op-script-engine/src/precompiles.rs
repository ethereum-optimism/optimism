...
impl HostPrecompile {
    ...
    pub fn run(&mut self, input: &[u8]) -> PrecompileOutcome {
        ...
        match self {
            HostPrecompile::InputSnapshot(map) => match map.get(&sel) {
                Some(bytes) => PrecompileOutcome::Return(bytes.clone()),
                None => PrecompileOutcome::Revert(format!(
                    "unrecognized 4 byte signature: {}",
                    crate::cheatcodes::hex(&sel)
                )),
            },
            HostPrecompile::OutputCapture(out) => {
