...
pub enum ForkUnderlay {
    /// Unforked: every uninserted account is non-existent (the non-forked genesis default).
    Empty(EmptyDB),
    /// Forked: reads miss through to an L1 archive pinned to an immutable block hash.
    Fork(Box<ForkDb>),
}

impl Default for ForkUnderlay {
    fn default() -> Self {
        Self::Empty(EmptyDB::default())
    }
}
...
impl DatabaseRef for ForkUnderlay {
    ...
    fn basic_ref(&self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        match self {
            Self::Empty(db) => match db.basic_ref(address) {
                Ok(v) => Ok(v),
                Err(e) => match e {},
            },
            Self::Fork(db) => db.basic_ref(address),
        }
    }

    fn code_by_hash_ref(&self, code_hash: B256) -> Result<Bytecode, Self::Error> {
        match self {
            Self::Empty(db) => match db.code_by_hash_ref(code_hash) {
                Ok(v) => Ok(v),
                Err(e) => match e {},
            },
            Self::Fork(db) => db.code_by_hash_ref(code_hash),
        }
    }

    fn storage_ref(&self, address: Address, index: StorageKey) -> Result<U256, Self::Error> {
        match self {
            Self::Empty(db) => match db.storage_ref(address, index) {
                Ok(v) => Ok(v),
                Err(e) => match e {},
            },
            Self::Fork(db) => db.storage_ref(address, index),
        }
    }

    fn block_hash_ref(&self, number: u64) -> Result<B256, Self::Error> {
        match self {
            Self::Empty(db) => match db.block_hash_ref(number) {
                Ok(v) => Ok(v),
                Err(e) => match e {},
            },
            Self::Fork(db) => db.block_hash_ref(number),
        }
    }
}
...
impl std::fmt::Debug for ForkUnderlay {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Empty(_) => write!(f, "ForkUnderlay::Empty"),
            Self::Fork(_) => write!(f, "ForkUnderlay::Fork"),
        }
    }
}
...
pub struct ForkMeta {
    /// The resolved fork block number.
    #[serde(rename = "blockNumber")]
    pub block_number: u64,
    /// The fork block hash; all fork reads are pinned to it (reorg-safe, matching Go `RPCSource`).
    #[serde(rename = "blockHash")]
    pub block_hash: B256,
    /// The fork block state root.
...
impl ForkDiff {
    ...
    /// Record a direct host-side account write (`set_balance` / `set_nonce` / `set_code)`: these
    /// bypass EVM execution, so they are not in any finalized state, but in Go the equivalent
    /// cheatcode writes land in the fork diff. `code` is `Some` only when the code changed.
