...
over to [the binary guide](/kona/node/run/binary).
...
See the [configuration guide](/kona/node/configuration#rpc-trust-configuration) for more details on RPC trust settings.
