...
[Installation](/kona/node/install/overview) guide.
...
[Docker Guide](/kona/node/run/docker) which uses a `docker-compose`
...
[Docker Guide](/kona/node/run/docker) to avoid misconfigurations.
</Callout>
...
out the [docker guide](/kona/node/run/docker).
