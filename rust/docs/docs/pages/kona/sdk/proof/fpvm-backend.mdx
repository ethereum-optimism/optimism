...
> 📖 Before reading this section of the book, it is advised to read the [Fault Proof Program Environment](/kona/sdk/fpp-dev/env)
...
- [Fault Proof VM](/kona/glossary#fault-proof-vm) IO and utilities
...
for programs running on top of the [FPVM targets](/kona/sdk/fpp-dev/env).
...
when developing programs that target the [FPVMs](/kona/sdk/fpp-dev/env), barring needs like printing directly to
`stdout`.
...
[Host - Client Communication](/kona/sdk/fpp-dev/env#host---client-communication) in the FPP Dev environment section of the
book.
...
the [Custom Backend](/kona/sdk/proof/custom-backend) section, but we'll quickly gloss over them here to build intuition.
