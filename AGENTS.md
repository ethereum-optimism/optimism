...
| [`interop-reviewer`](.claude/agents/interop-reviewer.md) | touches interop code (op-supernode, op-interop-filter/-mon, op-core/interop, kona-interop/proof-interop, lokahi, op-alloy interop types, op-node's interop-adjacent engine paths) | [docs/ai/interop-review.md](docs/ai/interop-review.md) |
...
- [docs/ai/interop-review.md](docs/ai/interop-review.md) - Reviewing interop protocol changes (Go or Rust): the invariants registry with spec citations, the Go/Rust/proof-side parity map, accept-set comparison discipline, and the intentional-divergence registry (tx-pool filter strictness, admission-only concepts). Pairs with the `interop-reviewer` agent
