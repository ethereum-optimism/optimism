...
This is the Optimism monorepo containing core components of the OP Stack - the decentralized software stack that powers Optimism and forms the backbone of OP Mainnet and the Superchain. The codebase is primarily Go with Solidity smart contracts and a Rust subproject (Kona).
...
# Run tests for a specific package (use the package's justfile)
# Each package has its own test target that handles dependencies
...
# Run all contract tests with the faster 'lite' build mode enabled
just test-dev

# Run a specific test
just test-dev --match-test testFunctionName
...
# Prerequisites: E2E tests require contract artifacts and cannon prestates.
# Run this first if artifacts are missing:
make pre-test
...
For the full Solidity style guide, see `packages/contracts-bedrock/book/src/contributing/style-guide.md`.

Key points when working with Solidity files:
...
- `immutable` variables are discouraged in upgradeable contracts
...
1. PRs should target the `develop` branch
2. Use `just pr` (in `packages/contracts-bedrock`) to run all contracts checks prior to submitting contract changes
