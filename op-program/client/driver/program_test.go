...
func (fakeEngineController) StartBuildAsync(ctx context.Context, attrs *derive.AttributesWithParent) event.Promise0[error] {
	return nil
}

func TestProgramDeriver(t *testing.T) {
	newProgram := func(t *testing.T, target uint64) (*ProgramDeriver, *testutils.MockEmitter, *attributes.MockEngineController) {
	...
	t.Run("engine reset confirmed", func(t *testing.T) {
		p, m, controller := newProgram(t, 1000)
		m.ExpectOnce(derive.ConfirmPipelineResetEvent{})
		controller.On("RequestPendingSafeUpdate", mock.Anything).Once()
	...
	t.Run("derived attributes", func(t *testing.T) {
		...
		m.ExpectOnceRun(func(ev event.Event) {
			be, ok := ev.(engine.BuildStartEvent)
			require.True(t, ok)
			require.Same(t, attrib, be.Attributes)
		})
	...
	t.Run("forkchoice update", func(t *testing.T) {
		t.Run("surpassed", func(t *testing.T) {
			p, m, controller := newProgram(t, 42)
	...
	t.Run("engine temp error event", func(t *testing.T) {
		p, _, _ := newProgram(t, 1000)
		// Temporary errors should request another pending-safe update
		controller := p.engineController.(*attributes.MockEngineController)
		controller.On("RequestPendingSafeUpdate", mock.Anything).Once()
