...
func NewDriver(logger log.Logger, cfg *rollup.Config, l1Source derive.L1Fetcher, l1BlobsSource derive.L1BlobsFetcher, l2Source L2Source, targetBlockNum uint64) *Driver {
	...
	ec := engine.NewEngineController(l2Source, logger, metrics.NoopMetrics, cfg, syncCfg, engine.Geth)
	attributesHandler := attributes.NewAttributesHandler(logger, cfg, ec, l2Source)
