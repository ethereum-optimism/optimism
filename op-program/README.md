...
The program includes various configurations for different networks and hardforks:

### Sepolia Isthmus

The Sepolia Isthmus hardfork configuration is available in the `chainconfig/configs/sepolia-isthmus.json` file.
It contains the Unix timestamp (`1744905600`) when the Isthmus hardfork activates on the Sepolia network.

You can retrieve this timestamp using the utility function:

```go
import "github.com/ethereum-optimism/optimism/op-program/chainconfig"

time, err := chainconfig.GetSepoliaIsthmusTime()
```

Or run the provided script:

```shell
go run ./scripts/isthmus-info.go
```
