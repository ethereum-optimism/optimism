...
transaction are determined by the corresponding `TransactionDepositedV0` event emitted by the [deposit
...
### Deposit Event Versioning

The deposit event is versioned in order to allow for future modifications to the event format. The
version is modified simply by incrementing the integer suffix. Thus the initial event name will
be `TransactionDepositedV0`.
...
the `TransactionDepositedV0` event(s) emitted by the deposit contract.
