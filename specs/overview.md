...
1. Call `engine_forkchoiceUpdatedV3` with the payload attributes object. We'll skip over the details of the fork choice
...
2. Call `engine_getPayloadV3` with the payload ID returned in step 1. The engine API returns a payload object that
includes a block hash as one of its fields.
3. Call `engine_newPayloadV3` with the payload returned in step 2. (Pre-Ecotone blocks MUST use the V2 version)
4. Call `engine_forkchoiceUpdatedV3` with the fork choice parameter's `headBlockHash` set to the block hash returned in
