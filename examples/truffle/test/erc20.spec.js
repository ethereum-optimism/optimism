...
contract('ERC20', (accounts) => {
  ...
  beforeEach(async () => {
    token = await ERC20.new(10000, tokenName, tokenDecimals, tokenSymbol, { gasPrice: 0, from: accounts[ 0 ] })
  })
  ...
  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const token2 = await ERC20.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'Simon Bucks', 1, 'SBX', { gasPrice: 0, from: accounts[ 0 ] })
  ...
  it('transfers: ether transfer should be reversed.', async () => {
    ...
    let threw = false
    try {
      await web3.eth.sendTransaction({ gasPrice: 0, from: accounts[ 0 ], to: token.address, value: web3.utils.toWei('10', 'Ether') })
    } catch (e) {
      threw = true
    }
  ...
  it('transfers: should transfer 10000 to accounts[1] with accounts[0] having 10000', async () => {
    await token.transfer(accounts[ 1 ], 10000, { gasPrice: 0, from: accounts[ 0 ] })
  ...
  it('transfers: should fail when trying to transfer 10001 to accounts[1] with accounts[0] having 10000', async () => {
    let threw = false
    try {
      await token.transfer.call(accounts[ 1 ], 10001, { gasPrice: 0, from: accounts[ 0 ] })
    } catch (e) {
      threw = true
    }
    assert.equal(threw, true)
  ...
  it('transfers: should handle zero-transfers normally', async () => {
    assert(await token.transfer.call(accounts[ 1 ], 0, { gasPrice: 0, from: accounts[ 0 ] }), 'zero-transfer has failed')
  })
  ...
  it('approvals: msg.sender should approve 100 to accounts[1]', async () => {
    await token.approve(accounts[ 1 ], 100, { gasPrice: 0, from: accounts[ 0 ] })
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 20 once.', async () => {
    ...
    await token.approve(accounts[ 1 ], 100, { gasPrice: 0, from: accounts[ 0 ] }) // 100
    const balance2 = await token.balanceOf.call(accounts[ 2 ])
    assert.strictEqual(balance2.toNumber(), 0, 'balance2 not correct')

    await token.transferFrom.call(accounts[ 0 ], accounts[ 2 ], 20, { gasPrice: 0, from: accounts[ 1 ] })
    await token.allowance.call(accounts[ 0 ], accounts[ 1 ])
    await token.transferFrom(accounts[ 0 ], accounts[ 2 ], 20, { gasPrice: 0, from: accounts[ 1 ] }) // -20
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 20 twice.', async () => {
    ...
    await token.transferFrom(accounts[ 0 ], accounts[ 2 ], 20, { gasPrice: 0, from: accounts[ 1 ] })
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)', async () => {
    ...
    await token.transferFrom(accounts[ 0 ], accounts[ 2 ], 50, { gasPrice: 0, from: accounts[ 1 ] })
    const allowance012 = await token.allowance.call(accounts[ 0 ], accounts[ 1 ])
    ...
    let threw = false
    try {
      await token.transferFrom.call(accounts[ 0 ], accounts[ 2 ], 60, { gasPrice: 0, from: accounts[ 1 ] })
    } catch (e) {
      threw = true
    }
    assert.equal(threw, true)
  ...
  it('approvals: allow accounts[1] 100 to withdraw from accounts[0]. Withdraw 60 and then approve 0 & attempt transfer.', async () => {
    await token.approve(accounts[ 1 ], 100, { gasPrice: 0, from: accounts[ 0 ] })
    await token.transferFrom(accounts[ 0 ], accounts[ 2 ], 60, { gasPrice: 0, from: accounts[ 1 ] })
    await token.approve(accounts[ 1 ], 0, { gasPrice: 0, from: accounts[ 0 ] })
    let threw = false
    try {
      await token.transferFrom.call(accounts[ 0 ], accounts[ 2 ], 10, { gasPrice: 0, from: accounts[ 1 ] })
    } catch (e) {
      threw = true
    }
    assert.equal(threw, true)
  ...
  it('approvals: approve max (2^256 - 1)', async () => {
    await token.approve(accounts[ 1 ], '115792089237316195423570985008687907853269984665640564039457584007913129639935', { gasPrice: 0, from: accounts[ 0 ] })
  ...
  it('approvals: msg.sender approves accounts[1] of max (2^256 - 1) & withdraws 20', async () => {
    ...
    await token.approve(accounts[ 1 ], max, { gasPrice: 0, from: accounts[ 0 ] })
    const balance2 = await token.balanceOf.call(accounts[ 2 ])
    assert.strictEqual(balance2.toNumber(), 0, 'balance2 not correct')
  ...
  it('events: should fire Transfer event properly', async () => {
    const res = await token.transfer(accounts[ 1 ], '2666', { gasPrice: 0, from: accounts[ 0 ] })
  ...
  it('events: should fire Transfer event normally on a zero transfer', async () => {
    const res = await token.transfer(accounts[ 1 ], '0', { gasPrice: 0, from: accounts[ 0 ] })
    const transferLog = res.logs.find(
      element => element.event.match('Transfer') &&
        element.address.match(token.address)
    )
  ...
  it('events: should fire Approval event properly', async () => {
    const res = await token.approve(accounts[ 1 ], '2666', { gasPrice: 0, from: accounts[ 0 ] })
