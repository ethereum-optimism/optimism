...
func SetStorage(name string, address common.Address, values StorageValues, g *types.Genesis) error {
	...
	for _, slot := range slots {
		storage := g.Alloc[address].Storage
		...
		log.Info("setting storage", "address", address.Hex(), "key", slot.Key.Hex(), "value", slot.Value.Hex())
	}
	return nil
}
