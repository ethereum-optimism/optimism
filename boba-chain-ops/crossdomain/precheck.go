...
import (
	...
	"github.com/bobanetwork/boba/boba-chain-ops/util"
...
func PreCheckWithdrawals(g *types.Genesis, withdrawals DangerousUnfilteredWithdrawals, invalidMessages []InvalidMessage) (SafeFilteredWithdrawals, error) {
	...
	err := ForEachStorage(g, predeploys.LegacyMessagePasserAddr, func(key, value common.Hash) bool {
		progress()
...
func ForEachStorage(g *types.Genesis, addr common.Address, cb func(key, value common.Hash) bool) error {
	if g.Alloc[addr].Storage != nil {
		for key, value := range g.Alloc[addr].Storage {
			if !cb(key, value) {
				return nil
			}
		}
