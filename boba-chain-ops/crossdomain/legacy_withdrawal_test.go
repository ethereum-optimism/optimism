...
func findCrossDomainMessage(t *testing.T, receipt *types.Receipt) (*crossdomain.CrossDomainMessage, error) {
	backend := backends.NewSimulatedBackend(t, nil, 15000000)
	l2xdm, err := bindings.NewL2CrossDomainMessenger(predeploys.L2CrossDomainMessengerAddr, backend)
	if err != nil {
		return nil, err
	}
	abi, _ := abi.JSON(strings.NewReader(bindings.L2CrossDomainMessengerABI))
