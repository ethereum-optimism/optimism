...
func SetL2Proxies(db vm.StateDB) error {
	return setProxies(db, predeploys.HardforkOnlyProxyOwnerAddr, bigL2PredeployNamespace, 2048)
}
...
func setProxies(db vm.StateDB, proxyAdminAddr common.Address, namespace *big.Int, count uint64) error {
	depBytecode, err := bindings.GetDeployedBytecode("HardforkOnlyProxy")
	if err != nil {
		return err
	}
...
func setupPredeploy(db vm.StateDB, deployResults immutables.DeploymentResults, storage state.StorageConfig, name string, proxyAddr common.Address, implAddr common.Address) error {
	// Create the proxy address if it doesn't exist.
	if !db.Exist(proxyAddr) {
		db.CreateAccount(proxyAddr)
	}

	// Create the impl address if it doesn't exist.
	if !db.Exist(implAddr) {
		db.CreateAccount(implAddr)
	}

	// Special handling for untouchable addresses, if code already exists then
	// we just copy the code and DO NOT change the storage.
	if _, untouchable := UntouchablePredeploys[proxyAddr]; untouchable {
		// If there's code in the proxy address, copy it to implementation address.
		code := db.GetCode(proxyAddr)
		if len(code) > 0 {
			db.SetCode(implAddr, code)
			return nil
		}
	}
