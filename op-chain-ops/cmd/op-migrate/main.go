...
func main() {
	...
	app := &cli.App{
		...
		Action: func(ctx *cli.Context) error {
			...
			// tag := config.L1StartingBlockTag // TODO(pl): What is this used for?
			// if tag == nil {
			// 	return errors.New("l1StartingBlockTag cannot be nil")
			// }
			// log.Info("Using L1 Starting Block Tag", "tag", tag.String())
			log.Info("Using L1 Starting Block Tag", "tag", 1)
			block, err = l1Client.BlockByNumber(context.Background(), big.NewInt(1))
			...
			_, err = genesis.MigrateDB(ldb, block, !dryRun, noCheck)
			if err != nil {
				return err
			}
			...
			// TODO(pl): Implement post migration checks
			...
			// TODO(pl): What needs to be written here?
