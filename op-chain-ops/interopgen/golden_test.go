...
// outputs (addresses, rollup configs) are committed raw; multi-MB genesis dumps are pinned by the
// SHA-256 of their canonical json.Marshal bytes (encoding/json sorts map keys, so the serialization
// is deterministic). Once the Go host is deleted these are the only record of its behavior — a
// mismatch means the Rust engine diverged. See goldens/README.md for provenance.
