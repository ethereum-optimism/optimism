...
func TestInjectState(t *testing.T) {
	...
	require.Equal(t, big.NewInt(1_000_000_000_000_000_000), bal)
	...
	statedb, err = state.New(transitionBlock.Header().Root, state.NewDatabaseWithConfig(db, &trie.Config{Preimages: true}), nil)
	require.NoError(t, err)
	bal = statedb.GetBalance(common.Address{1})
