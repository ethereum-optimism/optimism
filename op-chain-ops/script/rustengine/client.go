...
func Spawn(binPath string, opts SpawnOpts, logw io.Writer) (*Engine, error) {
	...
	setDeathSignal(cmd) // on Linux, SIGKILL the engine if this test process dies (no leaked engines)
	...
	go func() {
		// bufio.Reader (not Scanner) so a long engine log line can't hit Scanner's 64KiB token cap
		// — which would stop draining stderr, fill the pipe, and hang the engine.
