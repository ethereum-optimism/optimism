...
func (el *L1ELNode) ReorgTriggeredFn(target eth.L1BlockRef, attempts int) CheckFunc {
	return func() error {
		el.log.Info("expecting chain to reorg on block ref", "id", el.inner.ID(), "target", target)
			...
			func() error {
				...
				if err != nil {
					if strings.Contains(err.Error(), "not found") { // reorg is happening wait a bit longer
						el.log.Info("chain still hasn't been reorged", "error", err)
						return err
					}
					return err
				}
				...
				if target.Hash == reorged.Hash { // want not equal
					el.log.Info("chain still hasn't been reorged", "ref", reorged)
				...
				el.log.Info("reorg on divergence block", "pre_blockref", target)
				el.log.Info("reorg on divergence block", "post_blockref", reorged)
