...
// betaUsage appends a beta disclaimer to a flag usage string.
func betaUsage(usage string) string {
	return usage + " Part of the Alt-DA beta feature."
}
...
func CLIFlags(envPrefix string, category string) []cli.Flag {
	return []cli.Flag{
		&cli.BoolFlag{
			...
			Usage:    betaUsage("Enable Alt-DA mode"),
			Value:    false,
		...
		&cli.StringFlag{
			...
			Usage:    betaUsage("HTTP address of a DA Server"),
			EnvVars:  altDAEnvs(envPrefix, "DA_SERVER"),
			Category: category,
		},
		&cli.BoolFlag{
			Name:     VerifyOnReadFlagName,
			Usage:    betaUsage("Verify input data matches the commitments from the DA storage service"),
			Value:    true,
		...
		&cli.BoolFlag{
			...
			Usage:    betaUsage("Use DA service type where commitments are generated by Alt-DA server"),
			Value:    false,
		...
		&cli.DurationFlag{
			...
			Usage:    betaUsage("Timeout for put requests. 0 means no timeout."),
		...
		&cli.DurationFlag{
			...
			Usage:    betaUsage("Timeout for get requests. 0 means no timeout."),
			Value:    time.Duration(0),
		...
		&cli.Uint64Flag{
			...
			Usage:    betaUsage("Maximum number of concurrent requests to the DA server"),
			Value:    1,
