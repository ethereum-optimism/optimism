...
func checkEIP1559ParamsMatch(opCfg *params.OptimismConfig, attrParams *eth.Bytes9, blockExtraData []byte) error {
	...
	if attrParams != nil {
		...
		if err := eip1559.ValidateJovian1559Params(params); err != nil {
		...
		} else if err := eip1559.ValidateJovianExtraData(blockExtraData); err != nil {
			// This can happen if the unsafe chain contains invalid (in particular, empty) extraData while Jovian
		...
		var translated bool
		// Translate 0,0,0 to the pre-Jovian protocol constants, like the EL does too.
		if ad == 0 {
			// If attrParams are non-nil, Jovian, and so Canyon, must be active.
			ad = *opCfg.EIP1559DenominatorCanyon
			ae = opCfg.EIP1559Elasticity
			am = 0 // Default MinBaseFeeLog2 to 0 when not configured
			translated = true
		}

		bd, be, bm := eip1559.DecodeJovianExtraData(blockExtraData)
		if ad != bd || ae != be || am != bm {
			extraErr := ""
			if translated {
