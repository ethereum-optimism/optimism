...
func (c *syncStartTestCase) Run(t *testing.T) {
	...
	syncStart := NewFindSyncStart(cfg, chain, chain)
	ctx := context.Background()
	i := 0
	for {
		err := syncStart.Step(ctx)
		if err == io.EOF {
			break
		}
		if err != nil {
			if c.ExpectedErr != nil {
				...
				require.ErrorIs(t, err, c.ExpectedErr, "Unexpected error")
				return
			} else {
		...
		i++
		if i > 1000 {
			t.Fatal("too many steps to find sync start, infinite loop?")
		}
	}
	finalized, safe, unsafe, startL1 := syncStart.Result()

	if c.ExpectedErr != nil {
		t.Fatal("expected error, but got sync result", finalized, safe, unsafe, startL1)
	}

	gotUnsafeHead := refToRune(unsafe.ID())
	require.Equal(t, string(c.UnsafeL2Head), string(gotUnsafeHead), "Unsafe L2 Head not equal")

	gotSafeHead := refToRune(safe.ID())
