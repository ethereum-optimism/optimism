...
func TestSuperAuthority_DeniedPayload_EmitsInvalidEvent(t *testing.T) {
	cfg, _, _, payloadA1 := buildSimpleCfgAndPayload(t)

	emitter := &testutils.MockEmitter{}
	sa := newMockSuperAuthority()
	// Deny the payload
	sa.DenyBlock(uint64(payloadA1.ExecutionPayload.BlockNumber), payloadA1.ExecutionPayload.BlockHash)

	ec := NewEngineController(context.Background(), nil, testlog.Logger(t, 0), metrics.NoopMetrics, cfg, &sync.Config{}, false, &testutils.MockL1Source{}, emitter, sa)

	// Expect PayloadInvalidEvent to be emitted (use type match since event has dynamic fields)
	emitter.ExpectOnceType("PayloadInvalidEvent")

	// Trigger payload processing
	blockRef := eth.L2BlockRef{
		Hash:       payloadA1.ExecutionPayload.BlockHash,
		Number:     uint64(payloadA1.ExecutionPayload.BlockNumber),
		ParentHash: payloadA1.ExecutionPayload.ParentHash,
		Time:       uint64(payloadA1.ExecutionPayload.Timestamp),
	}
...
func TestSuperAuthority_AllowedPayload_Proceeds(t *testing.T) {
	cfg, _, _, payloadA1 := buildSimpleCfgAndPayload(t)
	...
	ec := NewEngineController(context.Background(), mockEngine, testlog.Logger(t, 0), metrics.NoopMetrics, cfg, &sync.Config{}, false, &testutils.MockL1Source{}, emitter, sa)

	// Expect NewPayload to be called (payload is allowed)
	mockEngine.ExpectNewPayload(payloadA1.ExecutionPayload, nil, &eth.PayloadStatusV1{Status: eth.ExecutionValid}, nil)
...
func TestSuperAuthority_Error_ProceedsWithPayload(t *testing.T) {
	cfg, _, _, payloadA1 := buildSimpleCfgAndPayload(t)

	emitter := &testutils.MockEmitter{}
	mockEngine := &testutils.MockEngine{}
	sa := newMockSuperAuthority()
	sa.shouldError = true // Simulate check failure

	ec := NewEngineController(context.Background(), mockEngine, testlog.Logger(t, 0), metrics.NoopMetrics, cfg, &sync.Config{}, false, &testutils.MockL1Source{}, emitter, sa)

	// Despite error, expect NewPayload to be called (graceful degradation)
	mockEngine.ExpectNewPayload(payloadA1.ExecutionPayload, nil, &eth.PayloadStatusV1{Status: eth.ExecutionValid}, nil)
	emitter.ExpectOnceType("PayloadSuccessEvent")
...
func TestSuperAuthority_NilAuthority_Proceeds(t *testing.T) {
	cfg, _, _, payloadA1 := buildSimpleCfgAndPayload(t)
	...
	// nil SuperAuthority
	ec := NewEngineController(context.Background(), mockEngine, testlog.Logger(t, 0), metrics.NoopMetrics, cfg, &sync.Config{}, false, &testutils.MockL1Source{}, emitter, nil)

	// Expect NewPayload to be called (no authority check)
	mockEngine.ExpectNewPayload(payloadA1.ExecutionPayload, nil, &eth.PayloadStatusV1{Status: eth.ExecutionValid}, nil)
	emitter.ExpectOnceType("PayloadSuccessEvent")
