...
func TestParseL1InfoDepositTxData(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			depTx, err := L1InfoDeposit(&rollupCfg, l1Cfg, seqNr, info, 0, params.MergedTestChainConfig)
	...
	t.Run("invalid selector", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, 0, params.MergedTestChainConfig)
		require.NoError(t, err)
	...
	t.Run("ecotone", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, timestamp, params.MergedTestChainConfig)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
	...
	t.Run("activation-block ecotone", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, ecotoneTime, params.MergedTestChainConfig)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
	...
	t.Run("genesis-block ecotone", func(t *testing.T) {
		...
		rollupCfg := rollup.Config{BlockTime: 2, Genesis: rollup.Genesis{L2Time: 1000}}
		rollupCfg.ActivateAtGenesis(rollup.Ecotone)
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, rollupCfg.Genesis.L2Time, params.MergedTestChainConfig)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
		require.Equal(t, L1InfoEcotoneLen, len(depTx.Data))
	...
	t.Run("activation-block isthmus", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, isthmusTime, params.MergedTestChainConfig)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
	...
	t.Run("activation-block interop", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, interopTime, params.MergedTestChainConfig)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
