...
const (
	...
	metricsDisabledInstruction = "ZK proposer metrics are disabled; pass sysgo.WithZKMetrics() when creating the preset"
)
...
func New(t devtest.T, runtime Runtime) *ZKProposer {
	...
	if httpClient := runtime.MetricsClient(); httpClient != nil {
		metricsClient = devtestmetrics.NewMetricsClient(httpClient)
	}
