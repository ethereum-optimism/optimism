...
func TestBatchConsensusMockVerifierCommonwareProofCode(t *testing.T) {
	...
	cfg := &runtime.Config{ChainConfig: params.MergedTestChainConfig}
	...
	require.NoError(t, err)
	out, _, err := runtime.Execute(code, valid, cfg)
	require.NoError(t, err)
	ok, err := batchconsensus.DecodeVerifyResult(out)
	require.NoError(t, err)
	...
	out, _, err = runtime.Execute(code, withoutCertificate, cfg)
	require.NoError(t, err)
	ok, err = batchconsensus.DecodeVerifyResult(out)
	require.NoError(t, err)
	require.False(t, ok)
	...
	out, _, err = runtime.Execute(code, wrongCertificate, cfg)
	require.NoError(t, err)
	ok, err = batchconsensus.DecodeVerifyResult(out)
	require.NoError(t, err)
	require.False(t, ok)
	...
	require.NoError(t, err)
	out, _, err = runtime.Execute(code, invalid, cfg)
	require.NoError(t, err)
	ok, err = batchconsensus.DecodeVerifyResult(out)
	require.NoError(t, err)
	require.False(t, ok)
