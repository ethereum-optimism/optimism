...
type SingleChainInterop struct {
	...
	SuperRoots    proofs.SuperRootsSource
...
func NewSingleChainInterop(t devtest.T) *SingleChainInterop {
	...
	var superRoots proofs.SuperRootsSource
	switch {
	case len(system.Supervisors()) > 0:
		...
		superRoots = proofs.NewSuperRootsFromSupervisor(supervisor)
	case len(system.Supernodes()) > 0:
		supernode := system.Supernode(match.Assume(t, match.FirstSupernode))
		superRoots = proofs.NewSuperRootsFromSupernode(t, supernode)
	default:
...
func (s *SimpleInterop) DisputeGameFactory() *proofs.DisputeGameFactory {
	return proofs.NewDisputeGameFactory(s.T, s.L1Network, s.L1EL.EthClient(), s.L2ChainA.DisputeGameFactoryProxyAddr(), nil, nil, s.SuperRoots, nil)
}

func (s *SingleChainInterop) StandardBridge(l2Chain *dsl.L2Network) *dsl.StandardBridge {
	s.T.Require().NotNil(s.Supervisor, "supervisor is required for StandardBridge")
