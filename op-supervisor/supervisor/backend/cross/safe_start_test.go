...
func TestCrossSafeHazards(t *testing.T) {
	...
	t.Run("executing msg does bad link", func(t *testing.T) {
		...
		ssd.openBlockFn = newOpenBlockFn(&messages.ExecutingMessage{
	...
	t.Run("timestamp is equal, Check returns error", func(t *testing.T) {
		ssd := &mockSafeStartDeps{}
		ssd.checkFn = func() (includedIn messages.BlockSeal, err error) {
	...
	t.Run("timestamp is equal, same hazard twice", func(t *testing.T) {
		ssd := &mockSafeStartDeps{}
		sampleBlockSeal := messages.BlockSeal{Number: 3, Hash: common.BytesToHash([]byte{0x02}), Timestamp: 2}
		ssd.checkFn = func() (includedIn messages.BlockSeal, err error) {
			return sampleBlockSeal, nil
		}
		chainID := eth.ChainIDFromUInt64(123)
		inL1Source := eth.BlockID{}
		candidate := messages.BlockSeal{Number: 0, Hash: common.Hash{}, Timestamp: 2}
		ssd.candidate = candidate
		em1 := &messages.ExecutingMessage{ChainID: chainID, Timestamp: 2}
		em2 := &messages.ExecutingMessage{ChainID: chainID, Timestamp: 2}
		ssd.openBlockFn = func(chainID eth.ChainID, blockNum uint64) (ref eth.BlockRef, logCount uint32, execMsgs map[uint32]*messages.ExecutingMessage, err error) {
	...
	t.Run("timestamp is equal, different hazards", func(t *testing.T) {
		...
		sampleBlockSeal := messages.BlockSeal{Number: 3, Hash: common.BytesToHash([]byte{0x02}), Timestamp: 2}
		sampleBlockSeal2 := messages.BlockSeal{Number: 333, Hash: common.BytesToHash([]byte{0x22}), Timestamp: 2}
		calls := 0
		ssd.checkFn = func() (includedIn messages.BlockSeal, err error) {
			defer func() { calls++ }()
			if calls == 0 {
	...
	t.Run("timestamp is less, DerivedToSource returns error", func(t *testing.T) {
		...
		ssd.derivedToSrcFn = func() (source messages.BlockSeal, err error) {
			return messages.BlockSeal{}, errors.New("some error")
		}
		chainID := eth.ChainIDFromUInt64(123)
		inL1Source := eth.BlockID{}
	...
	t.Run("timestamp is less, DerivedToSource Number is greater", func(t *testing.T) {
		...
		sampleSource := messages.BlockSeal{Number: 4, Hash: common.BytesToHash([]byte{0x03})}
		ssd.derivedToSrcFn = func() (source messages.BlockSeal, err error) {
	...
	t.Run("timestamp is less, DerivedToSource Number less", func(t *testing.T) {
		...
		sampleSource := messages.BlockSeal{Number: 1, Hash: common.BytesToHash([]byte{0x03}), Timestamp: 3}
		ssd.derivedToSrcFn = func() (source messages.BlockSeal, err error) {
			return sampleSource, nil
		}
		chainID := eth.ChainIDFromUInt64(123)
	...
	t.Run("timestamp is less, DerivedToSource Number equal", func(t *testing.T) {
		...
		// Don't need checkFn since we want no hazards
		ssd.derivedToSrcFn = func() (source messages.BlockSeal, err error) {
			return messages.BlockSeal{Number: 1, Hash: common.BytesToHash([]byte{0x03}), Timestamp: 1}, nil
		}
		chainID := eth.ChainIDFromUInt64(123)
...
type mockSafeStartDeps struct {
	...
	derivedToSrcFn func() (source messages.BlockSeal, err error)
...
func (m *mockSafeStartDeps) Contains(chain eth.ChainID, query messages.ContainsQuery) (includedIn messages.BlockSeal, err error) {
	if m.checkFn != nil {
		return m.checkFn()
	}
...
func (m *mockSafeStartDeps) CrossDerivedToSource(chainID eth.ChainID, derived eth.BlockID) (source messages.BlockSeal, err error) {
...
func (m *mockSafeStartDeps) OpenBlock(chainID eth.ChainID, blockNum uint64) (ref eth.BlockRef, logCount uint32, execMsgs map[uint32]*messages.ExecutingMessage, err error) {
