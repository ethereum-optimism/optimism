...
func TestCrossSafeUpdate(t *testing.T) {
	t.Run("scopedCrossSafeUpdate passes", func(t *testing.T) {
		...
		execs := map[uint32]*messages.ExecutingMessage{1: {}}
		...
		csd.checkFn = func(chainID eth.ChainID, blockNum uint64, logIdx uint32, checksum messages.MessageChecksum) (messages.BlockSeal, error) {
	...
	t.Run("scopedCrossSafeUpdate returns ErrOutOfScope", func(t *testing.T) {
		...
		currentCrossSafe := messages.BlockSeal{Number: 5}
		...
		parent := messages.BlockSeal{Number: 4}
...
func TestScopedCrossSafeUpdate(t *testing.T) {
	...
	t.Run("HazardSafeFrontierChecks returns error", func(t *testing.T) {
		...
		csd.crossDerivedToSourceFn = func(chainID eth.ChainID, derived eth.BlockID) (source messages.BlockSeal, err error) {
			return messages.BlockSeal{}, errors.New("some error")
		}
		// when CrossSafeHazards returns an error,
		// the error is returned
	...
	t.Run("HazardCycleChecks returns error", func(t *testing.T) {
		...
		csd.openBlockFn = func(chainID eth.ChainID, blockNum uint64) (ref eth.BlockRef, logCount uint32, execMsgs map[uint32]*messages.ExecutingMessage, err error) {
			return opened, 3, map[uint32]*messages.ExecutingMessage{1: em1, 2: em2}, nil
		}
		csd.checkFn = func(chainID eth.ChainID, blockNum uint64, logIdx uint32, checksum messages.MessageChecksum) (messages.BlockSeal, error) {
			return messages.BlockSeal{Number: 1, Timestamp: 1}, nil
		}
	...
	t.Run("UpdateCrossSafe hits invalid dependency", func(t *testing.T) {
		...
		csd.checkFn = func(chainID eth.ChainID, blockNum uint64, logIdx uint32, checksum messages.MessageChecksum) (messages.BlockSeal, error) {
			...
			return messages.BlockSeal{}, errors.New("unexpected checkFn call. expected short-circuit")
		}
		// when OpenBlock and CandidateCrossSafe return different blocks,
		// an ErrConflict is returned
...
type mockCrossSafeDeps struct {
	...
	crossDerivedToSourceFn func(chainID eth.ChainID, derived eth.BlockID) (source messages.BlockSeal, err error)
	...
	previousDerivedFn      func(chain eth.ChainID, derived eth.BlockID) (prevDerived messages.BlockSeal, err error)
	checkFn                func(chainID eth.ChainID, blockNum uint64, logIdx uint32, checksum messages.MessageChecksum) (messages.BlockSeal, error)
...
func (m *mockCrossSafeDeps) CrossDerivedToSource(chainID eth.ChainID, derived eth.BlockID) (source messages.BlockSeal, err error) {
...
func (m *mockCrossSafeDeps) Contains(chainID eth.ChainID, q messages.ContainsQuery) (messages.BlockSeal, error) {
	if m.checkFn != nil {
...
func (m *mockCrossSafeDeps) PreviousCrossDerived(chain eth.ChainID, derived eth.BlockID) (prevDerived messages.BlockSeal, err error) {
...
func (m *mockCrossSafeDeps) OpenBlock(chainID eth.ChainID, blockNum uint64) (ref eth.BlockRef, logCount uint32, execMsgs map[uint32]*messages.ExecutingMessage, err error) {
