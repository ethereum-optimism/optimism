...
func FetchChainInfoCLI() func(ctx *cli.Context) error {
	return func(cliCtx *cli.Context) error {
		...
		cfg, err := NewFetcherConfig(cliCtx)
		if err != nil {
			return err
		}

		results, err := FetchChainInfo(cliCtx.Context, lgr, cfg)
		if err != nil {
			return fmt.Errorf("failed to validate: %w", err)
		}
		for chainId, result := range results {
			if result.Error != nil {
				lgr.Error("failed to fetch chain info", "chainId", chainId, "error", result.Error)
				continue
			}
			err = script.WriteChainConfigToFile(result.Output, result.ChainName, chainId)
			if err != nil {
				return fmt.Errorf("failed to write chain info for %d: %w", chainId, err)
			}
			lgr.Info("wrote chain info to file", "chainName", result.ChainName, "chainId", chainId)
		}
		lgr.Info("completed fetching chain info")
		return nil
	}
}
...
func FetchChainInfo(ctx context.Context, lgr log.Logger, cfg *FetcherConfig) (map[uint64]FetchChainInfoResult, error) {
	lgr.Info("initializing fetcher", "numChains", len(cfg.ChainConfigs))
	...
	resultCh := make(chan FetchChainInfoResult, len(cfg.ChainConfigs))
	...
	for chainId, chainConfig := range cfg.ChainConfigs {
		...
		go func() {
			...
			lgr.Info("fetching chain info", "chainName", chainConfig.ChainName, "chainId", chainId)
