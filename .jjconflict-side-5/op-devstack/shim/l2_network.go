...
type presetL2Network struct {
	...
	batchers    locks.RWMap[stack.ComponentID, stack.L2Batcher]
	proposers   locks.RWMap[stack.ComponentID, stack.L2Proposer]
	challengers locks.RWMap[stack.ComponentID, stack.L2Challenger]

	els locks.RWMap[stack.ComponentID, stack.L2ELNode]
	cls locks.RWMap[stack.ComponentID, stack.L2CLNode]

	conductors       locks.RWMap[stack.ComponentID, stack.Conductor]
...
func NewL2Network(cfg L2NetworkConfig) stack.ExtensibleL2Network {
	...
	require.Equal(cfg.T, cfg.ID, eth.ChainIDFromBig(cfg.NetworkConfig.ChainConfig.ChainID), "chain config must match expected chain")
	require.Equal(cfg.T, cfg.L1.ID(), eth.ChainIDFromBig(cfg.RollupConfig.L1ChainID), "rollup config must match expected L1 chain")
	require.Equal(cfg.T, cfg.ID, eth.ChainIDFromBig(cfg.RollupConfig.L2ChainID), "rollup config must match expected L2 chain")
	return &presetL2Network{
		id:            eth.ChainIDFromBig(cfg.RollupConfig.L2ChainID),
...
func (p *presetL2Network) ID() eth.ChainID {
	return p.id
}
...
func (p *presetL2Network) L2Batcher(m stack.L2BatcherMatcher) stack.L2Batcher {
	v, ok := findMatch(m, p.batchers.Values())
...
func (p *presetL2Network) AddL2Batcher(v stack.L2Batcher) {
	id := v.ID()
	p.require().Equal(p.id, v.ChainID(), "l2 batcher %s must be on chain %s", id, p.id)
...
func (p *presetL2Network) Conductor(m stack.ConductorMatcher) stack.Conductor {
	v, ok := findMatch(m, p.conductors.Values())
...
func (p *presetL2Network) L2Proposer(m stack.L2ProposerMatcher) stack.L2Proposer {
	v, ok := findMatch(m, p.proposers.Values())
...
func (p *presetL2Network) AddL2Proposer(v stack.L2Proposer) {
	id := v.ID()
	p.require().Equal(p.chainID, v.ChainID(), "l2 proposer %s must be on chain %s", id, p.chainID)
...
func (p *presetL2Network) L2Challenger(m stack.L2ChallengerMatcher) stack.L2Challenger {
	v, ok := findMatch(m, p.challengers.Values())
...
func (p *presetL2Network) AddL2CLNode(v stack.L2CLNode) {
	id := v.ID()
	p.require().Equal(p.chainID, v.ChainID(), "l2 CL node %s must be on chain %s", id, p.chainID)
...
func (p *presetL2Network) AddL2ELNode(v stack.L2ELNode) {
	id := v.ID()
	p.require().Equal(p.chainID, v.ChainID(), "l2 EL node %s must be on chain %s", id, p.chainID)
...
func (p *presetL2Network) OPRBuilderNode(m stack.OPRBuilderNodeMatcher) stack.OPRBuilderNode {
	v, ok := findMatch(m, p.oprBuilderNodes.Values())
...
func (p *presetL2Network) RollupBoostNode(m stack.RollupBoostNodeMatcher) stack.RollupBoostNode {
	v, ok := findMatch(m, p.rollupBoostNodes.Values())
