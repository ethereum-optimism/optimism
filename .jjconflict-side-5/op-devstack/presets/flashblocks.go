...
func NewSingleChainWithFlashblocks(t devtest.T) *SingleChainWithFlashblocks {
	...
	sequencerCL := l2.L2CLNode(match.AssumeComponent(t, match.WithSequencerActive(t.Ctx())))
	sequencerEL := l2.L2ELNode(match.AssumeComponent(t, match.EngineFor(sequencerCL)))
	...
	out := &SingleChainWithFlashblocks{
		L2OPRBuilder:  dsl.NewOPRBuilderNode(l2.OPRBuilderNode(match.AssumeComponent(t, match.FirstOPRBuilderNode)), orch.ControlPlane()),
		L2RollupBoost: dsl.NewRollupBoostNode(l2.RollupBoostNode(match.AssumeComponent(t, match.FirstRollupBoostNode)), orch.ControlPlane()),
		Minimal: &Minimal{
			Log:              t.Logger(),
			T:                t,
			...
			L2Batcher:        dsl.NewL2Batcher(l2.L2Batcher(match.AssumeComponent(t, match.FirstL2Batcher))),
			...
			FaucetL2:         dsl.NewFaucet(l2.Faucet(match.AssumeComponent(t, match.FirstFaucet))),
