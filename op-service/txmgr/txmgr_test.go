...
func (g *gasPricer) sample() (*big.Int, *big.Int) {
	...
	// fmt.Println("sample called", "epoch", g.epoch, "gasTip", epochGasTipCap, "feeCap", epochGasFeeCap)
...
// TestIncreaseGasPriceUseLargeIncreas asserts that if the suggest gas tip
// returned from L1 is much larger than the required price bump the L1 value
// is used instead of the price bump
func TestIncreaseGasPriceUseLargeIncreas(t *testing.T) {
	t.Parallel()
