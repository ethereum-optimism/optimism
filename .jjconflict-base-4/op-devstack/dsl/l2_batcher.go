...
func (b *L2Batcher) Stop() {
	...
	require.NoError(b.t, err, fmt.Sprintf("Expected to be able to call StopBatcher API on batcher %s, but got error", b.inner.ID()))
}
...
func (b *L2Batcher) Start() {
	...
	require.NoError(b.t, err, fmt.Sprintf("Expected to be able to call StartBatcher API on batcher %s, but got error", b.inner.ID()))
}
