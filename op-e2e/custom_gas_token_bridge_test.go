...
func TestCustomGasTokenLockAndMint(t *testing.T) {
	...
	// Activate the custom gas token
	systemConfig, err := bindings.NewSystemConfig(cfg.L1Deployments.SystemConfig, l1Client)
	require.NoError(t, err)

	systemConfigOwnerOpts, err := bind.NewKeyedTransactorWithChainID(sys.Cfg.Secrets.SysCfgOwner, cfg.L1ChainIDBig())
	require.NoError(t, err)

	tx, err = systemConfig.SetGasPayingToken(systemConfigOwnerOpts, wethAddress)
	require.NoError(t, err)
	_, err = wait.ForReceiptOK(context.Background(), l1Client, tx.Hash())
	require.NoError(t, err)
