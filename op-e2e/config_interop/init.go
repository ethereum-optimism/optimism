...
var (
	...
	L1Deployments_2 *genesis.L1Deployments
	// DeployConfig represents the deploy config used by the system.
	DeployConfig *genesis.DeployConfig
	DeployConfig_2 *genesis.DeployConfig
...
func init() {
	var l1AllocsPath, l1DeploymentsPath, l1DeploymentsPath_2, deployConfigPath, deployConfigPath_2, externalL2 string
	...
	defaultL1DeploymentsPath_2 := filepath.Join(root, ".devnet", "addresses-2.json")
	defaultDeployConfigPath := filepath.Join(root, "packages", "contracts-bedrock", "deploy-config", "devnetL1.json")
	defaultDeployConfigPath_2 := filepath.Join(root, "packages", "contracts-bedrock", "deploy-config", "devnetL1-2.json")

	flag.StringVar(&l1AllocsPath, "l1-allocs-interop", defaultL1AllocsPath, "")
	flag.StringVar(&l1DeploymentsPath, "l1-deployments-interop", defaultL1DeploymentsPath, "")
	flag.StringVar(&l1DeploymentsPath_2, "l1-deployments-interop-2", defaultL1DeploymentsPath_2, "")
	flag.StringVar(&deployConfigPath, "deploy-config-interop", defaultDeployConfigPath, "")
	flag.StringVar(&deployConfigPath_2, "deploy-config-interop-2", defaultDeployConfigPath_2, "")
	flag.StringVar(&externalL2, "externalL2-interop", "", "Enable tests with external L2")
	flag.IntVar(&EthNodeVerbosity, "ethLogVerbosity-interop", int(log.LvlInfo), "The level of verbosity to use for the eth node logs")
	...
	if err := allExist(
		l1AllocsPath, l1DeploymentsPath, l1DeploymentsPath_2,
		deployConfigPath, deployConfigPath_2,
	); err != nil {
		return
	}
	...
	L1Deployments_2, err = genesis.NewL1Deployments(l1DeploymentsPath_2)
	if err != nil {
		panic(err)
	}
	...
	DeployConfig_2, err = genesis.NewDeployConfig(deployConfigPath_2)
	if err != nil {
		panic(err)
	}
	...
	DeployConfig_2.L1UseClique = false
	...
	DeployConfig_2.FundDevAccounts = true
	// Speed up the in memory tests
	DeployConfig.L1BlockTime = 2
	DeployConfig_2.L1BlockTime = 2
	DeployConfig.L2BlockTime = 1
	DeployConfig_2.L2BlockTime = 1

	if L1Deployments != nil {
		DeployConfig.SetDeployments(L1Deployments)
	}
	if L1Deployments_2 != nil {
		DeployConfig_2.SetDeployments(L1Deployments_2)
	}
