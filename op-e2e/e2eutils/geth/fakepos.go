...
func (f *FakePoS) Start() error {
	...
	f.sub = event.NewSubscription(func(quit <-chan struct{}) error {
		// Turn the quit channel into a context, which is more composable with the library
		...
		for {
			...
			case now := <-t.Ch():
				...
				if isAmsterdam {
					_, err = f.engineAPI.NewPayloadV5(ctx, *envelope.ExecutionPayload, blobHashes, &parentBeaconBlockRoot, make([]hexutil.Bytes, 0))
				} else if isPrague {
					_, err = f.engineAPI.NewPayloadV4(ctx, *envelope.ExecutionPayload, blobHashes, &parentBeaconBlockRoot, make([]hexutil.Bytes, 0))
				} else if isCancun {
					_, err = f.engineAPI.NewPayloadV3(ctx, *envelope.ExecutionPayload, blobHashes, &parentBeaconBlockRoot)
				} else {
					_, err = f.engineAPI.NewPayloadV2(ctx, *envelope.ExecutionPayload)
				}
				if err != nil {
