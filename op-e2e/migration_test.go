...
func TestMigration(t *testing.T) {
	...
	require.NoError(t, backoff.Do(10, backoff.Exponential(), func() error {
		forkedL1RPC, err = rpc.Dial("http://127.0.0.1:8545")
		if err != nil {
	...
	require.NoError(t, err)
	_, err = hh.GetDeployment("L2OutputOracle")
	require.NoError(t, err)

	nodeCfg := defaultNodeConfig("geth")
	...
	rollupNodeConfig := &node.Config{
		L1: &node.L1EndpointConfig{
			L1NodeAddr: "http://127.0.0.1:8545",
			L1TrustRPC: false,
		},
		L2: &node.L2EndpointConfig{
			L2EngineAddr: gethNode.HTTPEndpoint(),
		},
