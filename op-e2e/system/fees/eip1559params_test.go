...
func TestEIP1559Params(t *testing.T) {
	...
	h, err := l2Seq.HeaderByNumber(context.Background(), big.NewInt(0))
	...
	e, d := eip1559.DecodeHoloceneExtraData(h.Extra)
	t.Log(e, d)
	require.Equal(t, eip1559.EncodeHoloceneExtraData(expectedDenom, expectedElasticity), h.Extra)
	...
	e, d = eip1559.DecodeHoloceneExtraData(b.Header().Extra)
	t.Log(e, d)
	require.Equal(t, eip1559.EncodeHoloceneExtraData(expectedDenom, expectedElasticity), b.Header().Extra)
}
