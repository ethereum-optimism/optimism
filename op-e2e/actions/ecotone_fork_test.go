...
func TestEcotoneNetworkUpgradeTransactions(gt *testing.T) {
	...
	require.True(t, scalar.Cmp(dp.DeployConfig.GasPriceOracleScalar.ToInt()) == 0, "must match deploy config")
	...
	require.NoError(t, err)
	require.True(t, uint64(basefeeScalar) == dp.DeployConfig.GasPriceOracleScalar.ToInt().Uint64(), "must match deploy config")
