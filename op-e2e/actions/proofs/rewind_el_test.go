...
func runRewindELTest(gt *testing.T, testCfg *helpers.TestCfg[any]) {
	...
	fcs := &eth.ForkchoiceState{
		HeadBlockHash:      env.Sequencer.RollupCfg.Genesis.L2.Hash,
		SafeBlockHash:      env.Sequencer.RollupCfg.Genesis.L2.Hash,
		FinalizedBlockHash: env.Sequencer.RollupCfg.Genesis.L2.Hash,
	}

	res, err := env.Sequencer.Eng.ForkchoiceUpdate(context.Background(), fcs, nil)
