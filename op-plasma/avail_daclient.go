...
import (
	...
	"github.com/ethereum-optimism/optimism/op-plasma/types"
)
...
func (c *AvailDAClient) SetInput(ctx context.Context, img []byte) ([]byte, error) {
	...
	avail_Blk_Ref, err := c.submitDataAndWatch(img)

	if err != nil {
		return []byte{}, fmt.Errorf("cannot submit data:%v", err)
	}

	ref_bytes_data, err := avail_Blk_Ref.MarhalToBinary()
...
func (c *AvailDAClient) submitDataAndWatch(data []byte) (types.AvailBlockRef, error) {
    return interface
}
