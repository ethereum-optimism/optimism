...
func TestBatchSubmitter_computeSyncActions(t *testing.T) {
	...
	block101 := queue.NewBlockWithEstimatedSize(types.NewBlockWithHeader(&types.Header{Number: big.NewInt(101)}))
	block102 := queue.NewBlockWithEstimatedSize(types.NewBlockWithHeader(&types.Header{Number: big.NewInt(102)}))
	block103 := queue.NewBlockWithEstimatedSize(types.NewBlockWithHeader(&types.Header{Number: big.NewInt(103)}))
	...
	block104 := queue.NewBlockWithEstimatedSize(types.NewBlockWithHeader(&types.Header{Number: big.NewInt(104)}))
	...
	testCases := []TestCase{
		...
		{name: "gap between safe chain and state",
			...
			blocks:        queue.New(block102, block103), // note absence of block101
		...
		{name: "unexpectedly good progress",
			...
			prevCurrentL1: eth.BlockRef{Number: 1},
			blocks:        queue.New(block101, block102, block103),
			channels:      []channelStatuser{channel103},
			expected: syncActions{
				clearState:   &eth.BlockID{Number: 1},
		...
		{name: "failed to make expected progress (unsafe=safe)",
			...
			prevCurrentL1: eth.BlockRef{Number: 1},
			blocks:        queue.New(block102, block103),
			channels:      []channelStatuser{channel103},
			expected: syncActions{
				clearState: &eth.BlockID{Number: 1},
		...
		{name: "happy path + multiple channels",
			...
			prevCurrentL1: eth.BlockRef{Number: 1},
			blocks:        queue.New(block101, block102, block103, block104),
