...
func ChannelManagerReturnsErrReorg(t *testing.T, batchType uint) {
	...
	require.Equal(t, queue.New(queue.NewBlock(a), queue.NewBlock(b), queue.NewBlock(c)), m.blocks)
}
...
func TestChannelManager_TxData(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			blockA := queue.NewBlock(derivetest.RandomL2BlockWithChainId(rng, 200, defaultTestRollupConfig.L2ChainID))
			m.blocks = queue.New(blockA)
...
func TestChannelManager_handleChannelInvalidated(t *testing.T) {
	...
	rng := rand.New(rand.NewSource(99))
	blockA := queue.NewBlock(derivetest.RandomL2BlockWithChainId(rng, 10, defaultTestRollupConfig.L2ChainID))
	blockB := queue.NewBlock(derivetest.RandomL2BlockWithChainId(rng, 10, defaultTestRollupConfig.L2ChainID))

	// This is the snapshot of channel manager state we want to reinstate
	// when we requeue
	stateSnapshot := queue.New(blockA, blockB)
	...
	metrics.RecordL2BlockInUnsafeQueue(blockA.Block)
	metrics.RecordL2BlockInUnsafeQueue(blockB.Block)
...
func TestChannelManager_PruneBlocks(t *testing.T) {
	cfg := channelManagerTestConfig(100, derive.SingularBatchType)
	cfg.InitNoneCompressor()
	a := queue.NewBlock(types.NewBlock(&types.Header{
		Number: big.NewInt(0),
	}, nil, nil, nil, types.DefaultBlockConfig))
	b := queue.NewBlock(types.NewBlock(&types.Header{
		Number:     big.NewInt(1),
		ParentHash: a.Hash(),
	}, nil, nil, nil, types.DefaultBlockConfig))
	c := queue.NewBlock(types.NewBlock(&types.Header{
		Number:     big.NewInt(2),
		ParentHash: b.Hash(),
	}, nil, nil, nil, types.DefaultBlockConfig))
	...
	for _, tc := range []testCase{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := range tc.initialQ.Len() {
				require.NoError(t, m.AddL2Block(tc.initialQ.MustPeekN(i).Block))
			}
