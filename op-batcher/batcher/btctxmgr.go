...
func (tm *BitcoinTransactionManager) SendTransaction(amount int64, fee int64, data []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("error creating Taproot script: %v", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("error creating Taproot pkScript: %v", err)
	}
	tx.AddTxOut(output)
	...
	if err != nil {
		return "", fmt.Errorf("error decoding WIF: %v", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("error creating sigScript: %v", err)
	}
