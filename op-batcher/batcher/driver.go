...
func (l *BatchSubmitter) loop() {
	...
	go func() {
		for {
			select {
			case r := <-receiptsCh:
				if errors.Is(r.Err, txmgr.ErrAlreadyReserved) && txpoolState.CompareAndSwap(TxpoolGood, TxpoolBlocked) {
...
// expect the blocking transaction to result in error from being cancelled.)
