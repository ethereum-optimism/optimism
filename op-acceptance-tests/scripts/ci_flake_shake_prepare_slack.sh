...
if [ -f "$PROMO_JSON" ]; then
  ...
  SLACK_BLOCKS=$(jq -c \
    ...
    ( if (($meta|length)>0 and ($meta[0].flake_gate_tests // 0) == 0) then
        []
      elif ($root.candidates|length) == 0 then
        [
          {"type":"header","text":{"type":"plain_text","text":":partywizard: Acceptance Tests: No Flake-Shake Promotion Candidates"}},
          {"type":"section","text":{"type":"mrkdwn","text":"Artifacts: <\($url)|CircleCI Job>"}}
        ]
      else
        (
          [
