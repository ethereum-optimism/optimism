...
// RunDeepCanonicalBlockQueryTest verifies that the kona-interop fault proof program
// correctly uses EIP-2935 for block hash lookups outside the 8,191-block history window.
//
// It sends a cross-chain init message early in chain history, time-travels L1 to force
// ~10,000 L2 blocks of history, then executes the message on the other chain. The FPP
// must verify the init message's canonical block hash via a ~9,900 block lookback.
// Without EIP-2935, this lookback would be too expensive to complete.
func RunDeepCanonicalBlockQueryTest(t devtest.T, sys *presets.SimpleInterop) {
	t.Require().NotNil(sys.SuperRoots, "supernode is required for this test")
	...
	// --- Stage 1: Send init message early on ChainA ---
	aliceA := sys.FunderA.NewFundedEOA(eth.OneEther)
	aliceB := aliceA.AsEL(sys.L2ELB)
	sys.FunderB.Fund(aliceB, eth.OneEther)
	...
	// --- Stage 2: Time travel to generate ~10,000 L2 blocks ---
	// Advance in 200s increments to stay under FakePoS's 5-minute skip threshold,
	// ensuring L1 produces multiple blocks per increment (~17 at 12s L1 block time).
	// This gives the batcher enough L1 blocks to submit batches.
	const totalL2Seconds = 17_000 // 8,500 blocks * 2s block time (> 8,191 EIP-2935 window)
	const incrementSeconds = 200  // well under 5min FakePoS skip threshold
	const increments = totalL2Seconds / incrementSeconds

	targetTimestamp := sys.L2ELA.BlockRefByLabel(eth.Unsafe).Time + totalL2Seconds

	for i := 0; i < increments; i++ {
		sys.AdvanceTime(incrementSeconds * time.Second)
	}

	// Wait for both L2 chains to produce blocks up to the target time.
	sys.L2ELA.WaitForTime(targetTimestamp)
	sys.L2ELB.WaitForTime(targetTimestamp)

	// --- Stage 3: Execute the old message on ChainB ---
	execMsg := aliceB.SendExecMessage(initMsg)

	// --- Stage 4: Wait for validated super root ---
	endTimestamp := sys.L2ChainB.TimestampForBlockNum(bigs.Uint64Strict(execMsg.BlockNumber()))
	startTimestamp := endTimestamp - 1
	...
	// --- Stage 5: Build transition states and run FPP ---
	start := superRootAtTimestamp(t, chains, startTimestamp)
	end := superRootAtTimestamp(t, chains, endTimestamp)
	...
	// Test only the consolidation step — this is where the FPP must verify the
	// canonical block hash of the init message's block (~9,900 blocks back).
	// Without EIP-2935, this step would fail (too expensive to walk back linearly).
	tests := []*transitionTest{
		{
